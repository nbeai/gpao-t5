...
- Phase 0-2 반영(근거: `P0-2-WEB-SEARCH`, 실측 조사 + 오너 계정 라이브): §6.32 Web Search — URL 없이도
  찾는다. 3층(모델 내장 → 무키 → 키 연결), **작동하는 경로가 있으면 연결을 절대 권하지 않는다**(테스트 강제).
  말귀 보정: "정리" 같은 일반어로 파일 도구를 부르지 않고, 모델이 스스로 검색하면 T5 가 중복 실행하지 않는다.
- Phase 0-1 반영(근거: `P0-1-LOCAL-FILE`, v3.1 §21, 오너 실사용 검증): §6.31 Local File Hands —
  스텁이던 `local.file` 을 실제 손발로. 범위(scope) 축 신설(링크 탈출 차단), 되돌리기, 작업별 권한
  판정(도구 단위 kind 고정이 삭제를 승인 없이 통과시키던 결함 수정), 스텁 등록 금지 게이트.
- P-ID-1 반영(근거: `P-ID-1-OPERATIONAL-SELFHOOD`, 오너 지시 + 실계정 재현 검증): §6.30 Operational
  Selfhood — **어떤 모델이 붙든** 자기가 GPAO-T5(AI 운영체제)임을 알고, 무엇을 어디까지 할 수 있는지
  계속 인지한다. 정체·능력을 사용자 경로 문서(SOUL.md·CAPABILITIES.md)에 두고, 상시로는 요약만
  싣고 물어봤을 때만 대목을 꺼낸다(계획서 Phase 2 다이어트와 헌법 §5 자기파악을 동시에 만족).
...
- P-ONB-2 반영(근거: `P-ONB-2-FIRST-RUN-WELCOME`, 오너 지시(T5 마무리) + T3·Hermes·OpenClaw 실측 조사 +
  ...
  저장 정책 보정: **확실한 무효만 거절**(불확실은 저장하되 미검증). 첫인사는 하드코딩이 아니라
  숨은 규격 + 모델 생성, 미연결이면 인사를 지어내지 않는다(fail-closed).
...
### 6.27 First-Run Onboarding & Welcome (P-ONB-2, 구현됨 — 연결부터 첫인사까지)

근거: 오너 지시(T5 마무리) "설치 완료 후 즉시 전개 → 자연스럽게 연결 → 이용. 건너뛰어도 설정에서
언제든. 연결 후 첫 응답·웰컴도 자연스럽게." + T3·Hermes·OpenClaw 실측 조사(`P-ONB-2-FIRST-RUN-WELCOME`).

- **필요 판정은 서버측 단일 진실**: `needed = 연결 0개 && 건너뛴 적 없음`. 연결이 생기면 자동으로
  불필요해진다(완료 클릭 없음). **URL 쿼리로 판정하지 않는다** — T3 는 `?setup=1` 만 보고 저장 상태를
  아무도 읽지 않아 재설치마다 다시 떴다. 상태는 `onboarding.json` **한 파일**(T3 는 4곳 분산).
- **영속 탈출구**: "나중에 할게요"(`/onboarding/skip`)는 기록되어 다시 조르지 않는다. 건너뛰어도
  앱은 그대로 쓰이고, 모델이 없으면 stub 이 정직하게 답한다(차단하지 않는다).
- **온보딩 = 상시 설정과 같은 경로**(Hermes 원리): 새 화면을 만들지 않고 기존 칩 패널의 모델 연결
  블록을 펼친다. 그래서 나중에 연결해도 같은 표면·같은 계약이다(§5.5 안티 대시보드).
- **저장 정책 보정 — 확실한 무효만 거절**: `auth_failed`·`model_missing`·`billing_blocked` 만 저장
  거부하고, `unreachable`·`rate_limited` 는 **저장·활성하되 검증됨이라 말하지 않는다**(healthState 유지
  → 칩 "모델 확인 필요"). 근거: T3·Hermes 모두 라이브 프로브 하드블록이 사내 프록시·지역 차단·일시
  혼잡의 정상 사용자를 막았다고 기록. 거짓 초록 금지(§6.23)는 그대로.
- **웰컴/첫 응답**: 인사말을 하드코딩하지 않는다. `buildWelcomeContext` 가 **규격만**(한국어 1~3문장,
  실제 가능한 일에 근거, 내부 단계·파일·도구 id·지시문 언급 금지, 마지막에 한 번 묻기) 지시하고 문장은
  모델이 만든다. 숨은 지시는 **사용자 발화로 위장하지 않는다**(transcript 엔 assistant 결과만).
  **미연결이면 인사를 지어내지 않고** 상태를 정직하게 말한다(fail-closed, OpenClaw 원리).
  1회성 표식은 **실제로 인사한 경우에만** 남긴다 — 라이브 실측에서 발견한 결함(미연결로 한 번 열면
...
근거: v3.1 §21 Phase 0-4, 계획서 v3.0 Phase 7("다음 같은 요청에서 **질문이 줄어든다**").

**구조는 완성인데 효과가 없었다**: 감지→후보→replay→승인→admitted 를 다 거쳐도 `canInfluence` 가
**화면 표시에만**(`/skills` 응답) 쓰여 대화는 그대로였다. 상태 기계만 있고 소비 지점이 없었다.

- **소비 지점**: `applicableSkill(skills, text)` 이 **canInfluence 통과분만** 고르고,
  `skillInfluence(sk)` 가 계획에 줄 사실(도구·단계)을 만든다. turn 이 그 도구를 계획에 넣는다.
- **말귀를 넓힌다**: 스킬 조회를 fast_chat 판정 **앞에** 둔다. 일반 규칙이 못 알아듣는 표현이라도
  배운 작업의 트리거와 맞으면 그것 자체가 실행 신호다 — 그게 "배웠다"의 뜻이다.
  (뒤에 두면 스킬이 구경도 못 한다 — 테스트에서 실제로 그랬다.)
- **영향만, 실행은 아니다**(불변): `canAutoExecute()` 는 계속 false. 스킬이 외부 전송 도구를 골라도
  그 도구의 등급이 승인을 정한다(slack.post → A2). 스킬은 승인 게이트를 우회하지 못한다.
- **조용히 바뀌지 않는다**: 스킬이 도왔으면 결과에 `usedSkill`(id·label)을 남긴다 — reply·clarify
  양쪽 모두. 사용자가 "왜 이렇게 했지"를 알 수 있어야 한다.
- **게이트는 커널에 하나만**: 서버는 스킬을 전부 넘기고 커널이 canInfluence 로 거른다(이중 진실 금지).
- **반대 검증**: canAutoExecute 를 true 로 바꾸면 "스스로 실행하지 않는다" 테스트가, canInfluence
  필터를 빼면 "미승인 스킬 영향 0" 테스트가 각각 실제로 실패한다.

### 6.32 Web Search (Phase 0-2, 구현됨 — 주소 없이도 찾는다, 되는데 설정을 요구하지 않는다)

근거: v3.1 §21 Phase 0-2("사용자는 URL 을 들고 오지 않는다"), §16-D 능력 완결, §17 의존성 0.
`P0-2-WEB-SEARCH`. 오너 지시: **작동하는 경로가 있으면 "검색 연결하세요"를 절대 말하지 않는다.**

- **3층 구조**: ①모델 내장 검색(ChatGPT 계정 — 실측 확인) → ②무키 폴백(덕덕고·사용자 SearXNG) →
  ③키 연결(Tavily). **위에서 되면 아래로 가지 않는다.**
- **연결 권유의 절대 규칙**: `searchConnectionSuggestion` 이 유일한 생성 지점이고, 1층이 되거나
  2층이 성공하거나 이미 키가 있으면 **null 을 돌려준다**. 테스트가 이걸 강제한다("되는데 권유가
  나오면 실패"). 되는데 설정을 요구하면 §15("설정을 공부하지 않아도")를 정면으로 어긴다.
- **찾으면 실제로 읽는다**: 검색 스니펫으로 답하지 않고 T5 가 그 페이지를 fetch 해서 출처를
  만든다(출처 원장 계약). 첫 후보가 막히면(로그인벽·봇벽) **다음 후보로** 넘어간다 — 실사용에서
  첫 결과가 봇벽이라 막다른 답이 됐다.
- **차단을 성공으로 읽지 않는다**: 덕덕고가 anomaly/challenge 페이지를 주면 결과 0으로 처리하고
  다음 층으로 간다. 리다이렉트 주소를 못 풀면 그 결과는 버린다(가짜 출처 금지).
- **말귀 보정**(오너 지적 "적절하게 움직여야 한다"): ①"정리" 같은 일반어로 파일 도구를 부르지
  않는다(파일·폴더가 명시돼야) ②모델이 내장 검색을 하면 T5 가 같은 검색을 또 하지 않는다
  (OpenClaw 도 내장 검색이 있으면 관리형 도구를 억제 — 같은 원리). 실사용에서 이 둘 때문에
  원장에 "그 폴더는 비어 있어요"·"로그인이 필요한 페이지예요"가 남았다.
- **능력 표시**: 내장 검색 지원 여부는 **확인된 provider 만**(chatgpt_oauth) true — 추정으로
  능력을 부풀리지 않는다.
- **라이브 실증**(오너 계정): "요즘 AI 뉴스 조사해서 정리해줘"(URL 없음) → 최신 사건까지 담긴
  정리 + 원장 깨끗(엉뚱한 도구 실행 0).
- **추출 품질**(Phase 0-2b, 기준: Crawl4AI "LLM-ready Markdown" — 오너 결정): 이전 추출은 모든
  태그를 지우고 **앞 500자**를 잘라 네비게이션·쿠키 배너가 본문이 됐다(위키 실측: "Jump to content
  Main menu Navigation…"). 이제 껍데기(script·nav·header·footer·aside)를 걷고 본문 컨테이너를 찾아
  제목·문단·목록·표·인용을 **마크다운 구조로** 남긴다. 1칸짜리 표 행(인포박스·"v t e")은 버리고,
  짧은 조각 문단도 버린다. 구조를 못 건지면 통짜 텍스트로라도 주되 `blocks:0` 으로 **정직하게**
  표시한다. 의존성은 0 그대로(파서를 들이지 않는다 — §17·§20).
  **채택하지 않은 것**(오너 결정): Playwright·Crawlee 도입, Firecrawl 벤치마크. 근거 — 실측에서
  읽기 실패의 원인은 렌더링이 아니라 robots·로그인벽·봇차단이었고(렌더러로 안 풀린다), 의존성 0을
  깨뜨리며, 남의 기준(스크래핑 품질)으로 우리를 재는 것이기 때문.
- **실측 기록**: 무키 경로는 환경·시점에 따라 막힌다(덕덕고 anomaly, 공개 SearXNG 429/403/비JSON).
  큰 사이트는 스크래핑도 막는다(robots·로그인벽) — 그래서 1층이 선택이 아니라 주 경로다.

### 6.31 Local File Hands (Phase 0-1, 구현됨 — 선언한 능력을 실제로 만든다)

근거: v3.1 보강안 §21 Phase 0-1(즉시), §22 로컬 지배력(범위 축 먼저), §16-C·D. `P0-1-LOCAL-FILE`.

**거짓말이었다**: `local.file` 핸들러가 `{scanned:true}` 만 반환하는 스텁인데 레지스트리에 등록돼
있어 CAPABILITIES.md 는 "읽고 정리한다"고 적고 T5 는 사용자에게 그렇게 말했다. §6.30 의 문서 파생은
레지스트리에서 뽑으므로 **선언과 구현의 어긋남은 못 잡았다**.

- **범위(scope) 축**(`file-scope.js`): 허용 루트 안에서만. 기본은 좁게(`~/GPAO-T5`), env 로 확장.
  `../` 뿐 아니라 **심볼릭 링크 탈출도 막는다**(realpath 재판정). 루트 자체가 링크인 환경(macOS
  `/var`→`/private/var`)도 같은 기준으로 비교한다. 범위 밖은 막다른 답 대신 "어디까지 되는지"를 준다.
- **손발**(`local.file.js`): 보기·읽기·만들기·덮어쓰기·옮기기·지우기·되돌리기. 큰 파일은 통째로 읽지
  않는다. 실패는 종류별 사용자 언어(없음·권한·폴더·공간).
- **되돌리기**: 덮어쓰기·삭제는 원본을 휴지통으로 옮기고 되돌리기 표를 남긴다. 되돌린 뒤 **내용까지
  복구**되는지 테스트로 고정(§18).
- **작업별 권한 판정**(`fileKind`): 같은 도구가 읽기도 삭제도 한다. 도구 단위로 kind 를 고정하면
  **삭제가 organize 로 새어 승인을 건너뛴다** — 오너 실사용에서 실제로 샜고, 회귀 테스트로 고정했다.
  이제 write·delete 는 안전 바닥이라 항상 승인을 받고, 읽기는 막지 않는다.
- **인자 파싱**(`file-parse.js`): "어떤 파일에 무엇을"을 문장에서 분리한다(parseSend 와 같은 자리).
  도구만 만들면 커널이 무엇을 하라고 말해줄 수 없다는 것이 실사용에서 드러났다. 애매하면 실행 전에
  **한 가지만** 묻는다.
- **스텁 금지 게이트**(§16-C): 라이브 레지스트리에 fixture 가 등록되면 테스트가 실패한다.
- **조용한 덮어쓰기 금지**(P0-1b, 감사 지적): `move` 대상이 이미 있으면 **막고 확인을 요구한다**.
  `copyFile` 은 대상을 말없이 덮어쓰는데 그러면 undo(대상→원본)로도 대상의 원래 내용은 영영
  사라진다 — `write` 는 휴지통 백업이 있었는데 `move` 만 빠져 있었다. 자체 §16-A 점검이 놓친 것을
  감사가 잡았다. 회귀 테스트: 기존 b.txt 가 있을 때 a.txt→b.txt 이동은 **둘 다 그대로 남는다**.
- **라이브 실증**(오너 경로): "메모.md 만들어서 '중요한 내용' 적어줘" → **승인** → 파일 생성 →
  "지워줘" → **승인** → 삭제 → "되돌려줘" → 내용까지 복구.

### 6.30 Operational Selfhood (P-ID-1, 구현됨 — 모델이 바뀌어도 자기가 누구인지 안다)

근거: 오너 지시(2026-07-26) "어떤 모델이 연결되더라도 자기가 어떤 AI 운영체제와 함께 하고, 그
운영체제로 무엇을 어디까지 할 수 있는지 **계속 인지**해야 한다" + 헌법 §5 자기파악 헌법 +
계획서 v3.0 §0·§0.1. `P-ID-1-OPERATIONAL-SELFHOOD`.

**무너져 있었다(오너 실사용 실측)**: "넌 이름이 뭐야?"→"저는 ChatGPT예요"(하부 모델을 정체로),
"어떤 운영체제야?"→Linux/Windows 이야기(자기가 OS 인 줄 모름), "지파오티파이브?"→GPT-5 설명,
지어 준 이름은 다음 대화에서 소실. 원인: 모델에게 준 자기 소개가 한 줄뿐이라 빈칸을 모델이
자기 출신으로 채웠다.

- **정체(매 턴, 짧게)**: `buildIdentityFacts` 가 프롬프트 맨 앞에 — 너는 AI 모델이 아니라 GPAO-T5
  라는 AI 운영체제다 / 모델은 추론·문장을, 너는 요청 보존·맥락·자기 상태·권한·승인·기록·복구를
  맡는다 / **지금 쓰는 모델은 두뇌일 뿐 정체가 아니다**(제공자 이름을 자기 이름처럼 말하지 마라) /
  지금 할 수 있는 범위 개수 요약. 상시 블록은 700자 미만으로 묶어 Phase 2 다이어트를 지킨다.
- **문서(사용자 경로)**: `SOUL.md`(이름·정체·말투 — **계획서 v3.0 §0·§0.1 오너 원문 그대로 시드**,
  이후 덮어쓰지 않음) · `CAPABILITIES.md`(파생 구역은 레지스트리에서 매번 재생성, 사람이 쓴 구역은
  보존). 사용자가 도구를 늘리면 파생 구역이 자동으로 늘어난다. 재설치로 덮이지 않는다.
- **필요할 때만 꺼낸다(오너 결정 (a) 방식)**: `selfhoodLookup` 이 발화를 보고 능력·한계·정체를
  물을 때만 해당 대목을 싣는다. 평범한 대화엔 안 싣고, 못 찾으면 **지어내지 않고** 상시 요약으로
  답한다. 모델에게 검색 도구를 주는 (b) 방식은 부족할 때 얹는다.
- **헌법 §5 다섯 항목을 모두 담는다**(필요한 걸 빼지 않는다): 모델(+강점·한계) · 도구와 실행
  가능성 · 승인 필요 · **위험한 실행 종류** · **막혔을 때 다음 안전 행동**. 내부 도구 id 는 새지 않는다.
- **이름**: 명시적 지시만 잡아(추정≠반영 §6.18) 이번 턴부터 반영하고 `SOUL.md` 에 남긴다 —
  재시작해도 유지된다(OpenClaw SOUL/IDENTITY 이중 영속화 원리 흡수).
- **모델 무관**: 5개 provider 셰이프에 대해 같은 정체 사실이 실리는지 회귀로 고정(프롬프트만으로는
  보장되지 않는 항목이라 테스트가 계약이다).
- **라이브 실증(오너 실계정 gpt-5.5)**: 무너졌던 네 질문 전부 정정 — "내 이름은 GPAO-T5야" /
  "GPAO-T5라는 개인 AI 운영체제로 움직이고, 내 두뇌로 쓰는 모델은 gpt-5.5" / "지파오티파이브,
  즉 GPAO-T5는 내가 쓰는 이름이야" / 이름 지정 후 **재시작해도 "아이기스"** 유지.
...
- **전 provider 확장**(Phase 0-3): 와이어는 spec 이 선언하고 읽기는 공통(`streamSse`). anthropic 은
  같은 endpoint 에 `stream:true`(텍스트는 `content_block_delta` 만 — message_start·ping 은 안 흘림),
  gemini 는 **다른 엔드포인트**(`:streamGenerateContent?alt=sse`). **beai V1 은 스트리밍 미지원**
  (실키 실측: 400 "Streaming is not supported in External API V1") — OpenAI 와이어를 물려받으면
  켜지면서 응답 자체가 깨진다. `streaming:false` 로 **명시**하고 단발로 돌린다(선언 없이 조용히
  빠지면 "왜 이 모델만 느리지"를 아무도 설명 못 한다). 테스트가 이 명시를 강제한다.
  라이브: gemini 실키로 조각 흐름 확인, beai 실키로 단발 정상 확인.
