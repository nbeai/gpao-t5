...
test('실패 영수증에 실행신분이 없으면 같은 창의 fresh observe도 재시도 근거가 아니다', async () => {
  const p = 화면판({ 실패실행신분: null });
  await runTurn(
    { text: '계산기 창을 앞으로 가져오고 실제로 그렇게 됐는지 확인해줘' },
    p.ctx(순서모델([초점, 관찰, 초점])),
  );
  assert.equal(p.calls(), 1,
    '무엇을 실행했는지 묶을 신분이 없는데 뒤 화면 모양만으로 동일 focus를 다시 실행했다');
});
