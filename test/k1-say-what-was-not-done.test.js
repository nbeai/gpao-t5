...
// ── ③ 시제 — 앞 턴에 한 것을 이번 턴에 한 것처럼 말한다 ─────────────────
// 라이브(2026-08-07 · 노드 K 첫 판 뒤): 원장이 **완전히 빈** 턴(손 0회)에 T5 가 이렇게 답했다.
// ```
// "윤님, 방금 실제 파일 다시 열어서 확인해 봤어요.
//  - 위치: /Users/jyp/GPAO-T5/지난달 정산 파일/6월_정산_통합요약.csv"
// ```
// `[이번 턴 실행 사실] 없음` 블록은 **프롬프트에 실렸다**(밟아서 확인). 모델이 안 따랐다.
//
// 그 이유가 재료에 있다. 모델이 보는 것은 둘뿐이었다 —
//   ① 이번 턴에 아무것도 안 했다        ② 대화 이력에 정산 파일 내용이 있다
// **앞 턴에 그것을 읽었다는 사실이 어디에도 없다.** 그래서 ②를 ①의 빈자리에 끌어다 놓고
// *"방금 열어봤다"* 로 말한다. 금지 문구를 더 세게 쓰는 길은 막혀 있다(⛔ · F-12) —
// **구분할 재료를 준다.** 앞 턴에 한 것을 앞 턴 것이라고 적어 주면 시제가 갈린다.
test('앞 턴에 한 것을 앞 턴 것으로 준다 — 이번 턴 빈자리에 끌어다 쓰지 않게', () => {
  const 글 = 태우기({
    priorReceipts: [{
      failureState: 'none', lifecycle: 'delivered',
      actualCall: { tool: 'local.file', action: 'read' }, result: { ok: true },
      userSafeSummary: '6월_정산_통합요약.csv 를 읽었어요.',
    }],
  });
  assert.match(글, /6월_정산_통합요약\.csv/,
    `**앞 턴 사실이 안 간다** — 모델은 이력만 보고 "방금 열어봤다"로 말한다: ${글.slice(-400)}`);
  const 자리 = 글.indexOf('6월_정산_통합요약.csv');
  assert.match(글.slice(Math.max(0, 자리 - 260), 자리), /앞[선서]?\s*턴|이번 턴이 아니|지난 턴/,
    `**앞 턴 것이라고 안 적는다** — 시제가 안 갈리면 그대로 "방금"이 된다:\n${글.slice(Math.max(0, 자리 - 260), 자리)}`);
});

test('이번 턴 0 과 앞 턴 사실이 함께 선다 — 둘을 나란히 놓아야 시제가 갈린다', () => {
  const 글 = 태우기({
    priorReceipts: [{
      failureState: 'none', lifecycle: 'delivered',
      actualCall: { tool: 'local.file', action: 'read' }, result: { ok: true },
  ...
  assert.match(글, /도구를 한 번도 부르지 않았다/, '이번 턴 0 이 사라졌다');
});

test('앞 턴이 없으면 그 블록을 안 만든다 — 없는 과거를 지어내지 않는다', () => {
  assert.doesNotMatch(태우기({}), /앞선 턴에서 한 것/, '없는 과거를 이야기한다');
});

test('앞 턴 것도 확인 못 한 것은 그렇게 적는다 — 실패를 성공으로 승격하지 않는다', () => {
  const 글 = 태우기({
    priorReceipts: [{
      failureState: 'blocked',
      userSafeSummary: '정산 파일을 찾지 못했어요.',
    }],
  });
  const 자리 = 글.indexOf('정산 파일을 찾지 못했어요');
  assert.ok(자리 >= 0, `앞 턴 실패가 안 간다: ${글.slice(-300)}`);
  assert.match(글.slice(자리, 자리 + 80), /미확인|못 함|확인 못/,
    `**막힌 것이 해낸 것처럼 보인다**: ${글.slice(자리, 자리 + 120)}`);
});

// **"진짜 됐어?" 에는 다시 확인하는 길을 준다.**
//
// 라이브 두 번째 판(2026-08-07): 앞 턴 사실을 주니 답의 **내용**은 참이 됐다 —
// 파일 경로도 맞고 `~/Desktop/6월_정산_통합정리.csv` 가 **실제로 생겨 있었다**(78B).
// 남은 거짓은 한 문장이다: *"방금 다시 확인해 봤어요."* 확인은 안 했다(원장 0).
//
// **모델이 확인하고 싶어 하는 것은 옳다.** 사용자가 *"그거 진짜 됐어?"* 라고 물으면
// 다시 보는 것이 맞는 행동이고, 못 하게 막는 것은 제품을 나쁘게 만든다.
// 막을 게 아니라 **길을 준다** — 오늘 노드 R 에서 세운 구조 그대로다.
test('앞 턴에 만든 것은 다시 볼 수 있다고 말한다 — 말로 때우지 말고 진짜 보게', () => {
  const 글 = 태우기({
    priorReceipts: [{
      failureState: 'none', lifecycle: 'delivered',
      actualCall: { tool: 'local.file', action: 'write' }, result: { ok: true },
      userSafeSummary: '6월_정산_통합정리.csv 를 바탕화면에 만들었어요.',
    }],
  });
  assert.match(글, /다시 (보|읽|확인)/,
    `**확인하는 길이 없다** — 모델은 확인하고 싶어 하고, 길이 없으면 말로 때운다: ${글.slice(-500)}`);
});
