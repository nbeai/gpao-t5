...
test('운영 기준: 사용자의 부탁을 T5가 먼저 조사·실행하고, 사용자는 자기 권한만 판단한다', () => {
  const c = judgmentCharter();
  assert.match(c, /경로·명령·설정·연결은 T5가 다룬다/);
...
test('압축: 헌장이 다시 산문으로 부풀지 않는다(밀도 유지)', () => {
  ...
  // 1700 → 1800 (2026-08-01, 오너 구조 지시의 산출물 의무 계약): 기억 채널 줄과 같은 성질의
  // **채널 배선 한 줄**("파일 산출물 판단은 work.deliverable 로 선언")이 <도구>에 들어갔다.
  // 무엇이 파일이어야 하는지의 판단은 문장에 없다 — 판단은 모델, 대조는 원장이 한다.
  assert.ok(c.length <= 1800, `헌장이 ${c.length}자로 늘었다 — 한 줄=한 판단으로 압축할 것`);
