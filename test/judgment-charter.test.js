...
test('운영 기준: 사용자의 부탁을 T5가 먼저 조사·실행하고, 사용자는 자기 권한만 판단한다', () => {
  const c = judgmentCharter();
  assert.match(c, /경로·명령·설정·연결 방식은 T5가 먼저 다룰 현실/);
  assert.match(c, /직접 확인할 수 있는 것은 사용자가 대신 확인하게 하지 않는다/);
  assert.match(c, /사용자 몫은 승인·동의·비밀 직접 입력·선택/);
});
...
test('압축: 헌장이 다시 산문으로 부풀지 않는다(밀도 유지)', () => {
  const c = judgmentCharter();
  assert.ok(c.length <= 1650, `헌장이 ${c.length}자로 늘었다 — 한 줄=한 판단으로 압축할 것`);
