...
// **절단 시험이 찾은 두 번째 구멍**(2026-08-11): 능력 줄을 옛 `join(', ')` 으로 되돌려도
// S1~S5 가 전부 초록이었다. 그런데 S1 이 스키마 설명을 봐주는 근거가 바로 이 분리다 —
// 뭉침이 돌아오면 S1 의 전제가 무너진다. 전제 자체를 봉인한다.
test('S6 — 한 줄에 손을 둘 담지 않는다 (뭉치면 부정이 다시 경계를 넘는다)', async () => {
  const { 판, descriptors } = await 능력질문턴();
  const 위반 = 부정정의역위반({ descriptors, systemPrompt: 판.system, tools: 판.tools })
    .filter((v) => v.조항.startsWith('S6'));
  assert.deepEqual(위반, [], `**손 여럿이 한 줄에 뭉쳐 있다 — 그 줄의 진술은 누구 것인지 모른다:**${보기(위반)}`);
});
