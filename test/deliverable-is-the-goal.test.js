...
test('옮긴 것도 완료다 — "정리"의 정답은 move 다', () => {
  // **손이 실제로 내는 모양으로 잰다**(F-108 · 2026-08-13). 예전엔 여기서 `{ path }` 를 줬는데
  // `local-file.js:1134` 의 move 는 `{ from, to }` 를 낸다 — 그 칸을 안 낸다. 검사가 손을 안 보고
  // **계약이 바라는 모양**을 지어내 넣고 있었고, 그래서 실물이 안 맞는데도 내내 초록이었다.
  // 이 저장소가 이미 데인 자리다(J1: *"검사가 「패킷에 필드가 있는가」까지만 재서 내내 초록이었다"*).
  assert.equal(unsatisfiedDeliverables(계약, [영수증('move', { from: '/집/받은것/견적서.pdf', to: '/집/문서/견적서.pdf' })]).length, 0,
    'move 를 완료로 안 세면 T5 는 쓸데없는 파일을 만들어 계약을 채운다(실측)');
  // 옛 모양(`path`)도 계속 받는다 — 다른 손이 그 칸을 낼 수 있다.
