...
for (const [이름, 모델만들기] of 경로들) {
  ...
  test(`③ **probe 를 못 돌리면 모르는 대로 둔다** — 미상은 카드 없이 실행 제외 — ${이름}`, async () => {
    const 실행된인자 = [];
    const tools = demoTools({
      localTerminal: {
    ...
    assert.notEqual(r.kind, 'approval', `${이름}: 미상이 승인카드로 바뀌었다`);
    assert.equal(실행된인자.length, 0, `${이름}: 미상인데 실행됐다`);
  });

  // ── ④ **본 것과 한 것이 같다** ────────────────────────────────────────────
  //
  // 승인과 실행 사이 현실은 바뀔 수 있으므로 재개 때 fresh probe를 한 번 더 돈다.
  // 명령·자리·효과가 그대로면 현재 관측으로 실행하고, 효과가 달라지면 다시 묻는다.
  test(`④ **사용자가 승인한 그 판정으로 실행된다**(두 진실 금지) — ${이름}`, async () => {
    ...
    const 실행때본것 = p.probe한것.at(-1);
    assert.ok(실행때본것.회차 > 승인때본것.회차, `${이름}: 승인 뒤 현재 현실을 다시 재지 않았다`);
    assert.equal(실행.probeResult, `결과${실행때본것.회차}`,
      `${이름}: **fresh 판정한 사실과 실행된 사실이 다르다.**\n`
      + `  재개 판정 때: 결과${실행때본것.회차}\n  실행 때: ${실행.probeResult}`);
  });
}
