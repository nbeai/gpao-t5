...
test('preflight: 허용 파일 목록이 슬라이스 범위와 같다', () => {
  ...
  assert.deepEqual(허용파일, [
    ...
    // F-66b 독립 감사 차단 — 서명 본문과 durable Receipt 한 벌을 보존한다.
    ...
    // 적용은 겹침·덮음 두 축, 비적용은 겹침 한 축. 겹침은 작은 쪽 기준이라 요청을 통째로 품은
    // 비적용 본보기가 무조건 1.000 → 동점으로 이겼다. 오너 실물 5건 중 3건(검증 통과 2건 전부)이
    // 자기 appliesWhen 으로도 입장 불가였다. 크기를 함께 보는 자(`shapeSimilarity`)를 더한다 —
    // 문턱·관문은 불변이므로 그물은 안 넓어진다(반대시험 ③).
    ...
    'scripts/audit-project-entry.mjs',
    // 자격 파일의 문 하나(2026-08-14) — 키가 아닌 사실만 내주는 통로. 제품은 여전히
    // `src/surface/model-connection.js` 로만 자격을 읽는다. 개발 손이 "공급자·모델 id 만
    // 보려고" 자격 파일을 여는 것을 대신한다(2026-08-12 키 유출 → 오너 키 회전 사고).
