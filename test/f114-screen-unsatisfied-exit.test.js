...
test('다른 앱·창의 goal_verified는 unsatisfied focus를 회복하지 않는다', () => {
  const failed = 실패한화면행동();
  const success = {
    intended: 'desktop.act 실행',
    actualCall: { tool: 'desktop.act', args: { action: 'focus', app: '다른앱' } },
    failureState: 'none', lifecycle: 'delivered',
    result: { 단계: 'goal_verified', 행동: 'focus', 실행신분: { 창: 900, pid: 18888 } },
    userSafeSummary: '다른 창을 앞으로 가져왔어요.',
  };
  const v = 완료주장검증({ reply: '화면 작업을 끝냈어요.', receipts: [failed, success] });
  assert.equal(v.일치, false);
  assert.match(String(v.모델에게), /desktop\.act\|focus/);
});
