...
// ── 여기가 이 검사의 핵심 ────────────────────────────────────────────────
test('확인되지 않은 것을 사실인 척 라벨에 올리지 않는다', () => {
  const 라벨 = 기록라벨({ confirmed: [], unconfirmed: ['메일을 보냈어요.'], estimated: [] });
  assert.ok(!/보냈어요/.test(라벨), `미확인을 접힌 줄에 사실처럼 적었다 — 펼치지 않는 사용자는 됐다고 믿는다: ${라벨}`);
  assert.match(라벨, /도구 1개/, '사실로 말할 게 없으면 세는 쪽이 정직하다');
});

test('추정도 마찬가지다 — 확인된 것이 없으면 사실을 만들지 않는다', () => {
