...
// ── **따라온 계약 이동** (A3 · 2026-08-12) ───────────────────────────────────
// 이 칸이 재던 축은 *"되는 것만 말하면 모델이 제안을 실행으로 착각한다"* 이고 그 축은 그대로다.
// 옮긴 것은 **무엇이 경계인가**뿐이다.
//
// 옛 문구(`승인 전에는 아무 일도 예약되지 않는다`)는 이 설명서가 겨냥하는 바로 그 경우에
// **이제 거짓이다** — 사용자가 시점을 말한 명시 예약은 그 자리에서 켜진다
// (`design/T5-AUTOMATION-CLOSE-ko.md` §4 넓힘 1번 · 오너 2026-08-12
//  *"불필요한 승인카드는 모두 없애야해."*). 그 넓힘이 *"승인 전 job 0"* 을 재던 mainline
// 검사 10건을 이미 옮겼는데, 이 칸은 **job 수가 아니라 설명서 글자**를 읽어서 그물에 안 걸렸다
// — 같은 가족의 11번째다.
//
// **축을 무르게 하지 않는다**(C5): 경계 주장은 그대로 두 개 요구하고, 둘 다 지금 참인 것이다.
//   ① 추론 레인은 여전히 후보로만 남는다 — 자동으로 켜지는 것은 명시 레인뿐이다
//   ② 선 예약도 T5 가 꺼져 있으면 안 돈다 — 데몬이 아니다
test('안 되는 것도 그대로 말한다(짐작한 예약은 안 켜지고, 꺼져 있으면 안 돈다)', () => {
  const d = String(채널('automation.propose')?.description ?? '');
  assert.match(d, /짐작한 것이면 후보로만|확정해야 선다/,
    '되는 것만 말하면 모델이 제안을 실행으로 착각한다 — 반대 방향의 거짓이다.'
    + ' 자동으로 켜지는 것은 사용자가 시점을 말한 명시 레인뿐이라는 경계가 빠졌다');
  assert.match(d, /켜져 있어야|데몬/,
    '**선 예약이 안 도는 조건이 설명서에 없다.** T5 는 상시 서버가 아니라 사용자가 여는 앱이고'
    + ' 꺼져 있으면 안 돈다 — 그 사실이 없으면 「켜 뒀어요」는 절반만 참이다'
