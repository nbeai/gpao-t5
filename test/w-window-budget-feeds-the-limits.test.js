...
test('결과몫걸음은 turn.js 의 MAX_TOOL_STEPS 와 같다 — 두 진실 금지', async () => {
  const src = await readFile(new URL('../src/kernel/turn.js', import.meta.url), 'utf8');
  const m = src.match(/const MAX_TOOL_STEPS = (\d+)/);
  assert.ok(m, 'turn.js 에서 MAX_TOOL_STEPS 를 못 읽었다');
  assert.equal(결과몫걸음, Number(m[1]),
    '걸음 상한이 갈라졌다 — 한 턴의 결과 전부가 창 안에 들어온다는 계산이 무너진다');
});
