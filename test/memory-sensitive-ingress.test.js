...
// 선빨강(2026-08-12): 본선 전수 검사가 3~6회 중 1회 빨개지던 원인을 못박는다.
// sha256 지문 안에는 13~19자리 연속 숫자열이 심심찮게 생기고, 그중 일부는 **정말로**
// Luhn 을 통과한다. 아래 두 값은 실제 회차에서 잡힌 원문이다 — 옛 경계(`[^\d]`)는 이걸
// 카드번호로 읽었고, 그 지문을 담은 자동화 job 이 화면 목록에서 통째로 사라졌다.
// 기대를 느슨하게 한 것이 아니다: 진짜 카드번호는 아래 두 줄에서 그대로 잡혀야 한다.
test('sha256 기계 지문은 카드번호로 오인하지 않고 함께 있는 실제 카드번호는 잡는다', () => {
  for (const digest of [
    '2759a99b360e101eec0938111019692bdaaacc1b308557f7bfdbd68af9319a55', // 0938111019692 → Luhn 통과
    '9d279712818168936eed4d03d858ccc927e8c5002464c289ddc0fa0493ce4416', // 279712818168936 → Luhn 통과
  ]) {
    assert.equal(containsSensitiveValue(digest), false,
      `지문 내부 숫자 조각을 카드번호로 오인했다 — ${digest}`);
    assert.equal(containsSensitiveValue(`${digest} 카드 4111 1111 1111 1111`), true,
      '지문을 통과시키면서 실제 카드번호까지 놓쳤다');
  }
  // 구분자·라벨·한글 뒤에 오는 진짜 카드번호는 경계를 바꿔도 그대로 잡힌다.
  for (const value of ['4111111111111111', 'card=4111111111111111', 'card-4111111111111111',
    '카드번호는 4111111111111111이야', '결제 4111 1111 1111 1111 완료']) {
    assert.equal(containsSensitiveValue(value), true, value);
  }
});
