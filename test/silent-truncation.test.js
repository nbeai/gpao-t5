...
test('이름이 더 필요할 때 **문**을 알려 준다(막다른 길로 두지 않는다)', () => {
  const out = compactResult({ path: '/집/다운로드', items: 파일들(437) });
  // 예전엔 "더 좁은 폴더를 따로 보거나 명령을 쓴다"는 **말**이 길이었다. 그건 길의 이름이지
  // 길이 아니다 — 사고 당시 모델은 그 문장을 받고도 나머지를 가져오지 못했다(인자가 없었다).
  // S3 에서 진짜 문(`offset`·`limit`)이 났으므로, 재는 것도 **인자**로 옮긴다.
  assert.match(out, /offset=\d+/, '나머지를 가져올 인자가 없으면 그 턴은 계획으로 끝난다');
  assert.match(out, /limit/, '몇 개씩 받을지 정할 수 없으면 문이 반쪽이다');
});
