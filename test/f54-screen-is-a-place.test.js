...
// ── **태도 층 봉인 — 동반 세 계단이 헌장에 실려 모델 입력까지 닿는다** (오너 승인 문안 · 2026-08-09) ──
//
// 재료 층 4갈래(행동 0/8)·모델 층(5.1/5.5 동일 멈춤) 소진 뒤의 마지막 층. 문안은 오너 승인
// 원문 그대로 한 글자도 다르면 안 된다 — 다듬는 순간 태도가 아니라 조종이 된다(PM).
const 동반세계단 = '답하기 전에 스스로 본다 — 가진 손으로 볼 수 있는 것을 다 보았는가. '
  + '한 자리에서 막히거나 부족하면 옆 자리로 간다. 그래도 안 되면, 무엇을 보았고 무엇을 '
  + '못 보았는지를 함께 말한다.';
test('동반 세 계단 — 오너 문안 원문이 헌장에 있고 모델 입력까지 닿는다', async () => {
  const { buildModelMessages } = await import('../src/runtime/model-provider.js');
  const { system } = buildModelMessages({ currentRequest: '이번 달 얼마 벌었지?' });
  assert.ok(String(system).includes(동반세계단),
    '오너 승인 문안이 모델 입력에 없다 — 헌장에서 빠졌거나 한 글자라도 달라졌다');
});
