...
// 실측: 로컬 읽기 명령에는 카드가 없었다. 2026-08-18에는 자동 `reach`가 POST까지 승인 없이
// 보낸 사실이 재현돼, 임의 셸 네트워크는 카드 뒤에서만 실행하는 것으로 경계가 바로잡혔다.
//
...
test('① 로컬 읽기는 자동이고 임의 셸 네트워크는 카드 뒤에서 실행된다', async () => {
  ...
  assert.equal(네트워크.카드, true, '외부 효과를 구분 못 하는 셸 네트워크가 자동으로 갔다');
  assert.equal(네트워크.kind, 'unknown_kind', '외부 효과를 로컬 read/write로 꾸몄다');
});
...
// ── ④ 선언이 기계와 같은 말을 한다 ────────────────────────────────────────
test('④ 능력 줄이 로컬 읽기 자동과 셸 네트워크 승인 경계를 함께 말한다', () => {
  assert.ok(능력줄, '터미널 능력 줄이 모델 방에서 사라졌다');
  assert.match(능력줄, /바꾸지 않는 명령은 확인 카드 없이/,
    '읽기 명령에 마찰이 없다는 사실이 다시 빠졌다 — 없는 사실은 모델이 짐작으로 메운다');
  assert.match(능력줄, /바깥 네트워크[^.]*한 번 확인/,
    '실행기는 묻는데 모델에게는 네트워크가 자동이라고 말하는 두 진실이다');
  assert.match(스키마글, /바깥 네트워크[^.]*카드 뒤/,
    '도구를 고르는 스키마에서 외부 효과 경계가 사라졌다');
  assert.match(스키마글, /web\.collect가 승인 없이 맡는다/,
    '자동 공개 읽기의 남은 손을 모델에게 주지 않았다');
