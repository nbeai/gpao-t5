...
// ── ② 승격층 렌더 정규화(§7-cm-2 · 오너 (A) 확정) ────────────────────────────
// 저장은 **원문 그대로**다(makeAutoReversible 계약·H04 되돌리기 전제 — 0줄 무접촉).
// 바꾸는 것은 **모델에게 보여줄 때의 문형**뿐: 명령형 원문이 지시처럼 서지 않게 사실문 틀에
// 넣고 원문은 인용으로 보존한다. 「규칙문 금지 · 출처 병기」의 집행 자리다.
test('★ 선빨강 — 선호는 사실문 틀로 렌더된다(명령형이 지시로 서지 않는다 · 원문은 인용 보존)', () => {
  const m = buildModelMessages({
    ...기본,
    admittedContext: [선호문장],
    admittedRich: [{ statement: 선호문장, kind: 'preference' }],
  });
  const 몸 = 블록몸(m.system, '[사용자에 대해 알고 있는 것');
  assert.ok(몸, '전제 붕괴 — 사실 블록이 안 실렸다');
  assert.ok(몸.includes(`"${선호문장}"`),
    '**선호가 맨몸 명령형으로 렌더된다** — 「앞으로 …해줘」가 지시처럼 선다. 사실문 틀에 넣고 '
    + '원문은 인용으로 보존해야 한다(저장은 원문 불변 · 렌더만 정규화).');
  assert.match(몸, /사용자가 정한 선호/, '출처 병기(어디서 온 것인지)가 없다');
});

test('닻 — 원문 문자는 인용 안에 그대로 산다(저장·되돌리기 전제 불변)', () => {
  const m = buildModelMessages({
    ...기본,
    admittedContext: [선호문장],
    admittedRich: [{ statement: 선호문장, kind: 'preference' }],
  });
  assert.ok(m.system.includes(선호문장), '원문이 한 글자라도 바뀌면 철회 지목이 깨진다(H04 0/5 재발 경로)');
});

test('닻 — 선호가 아닌 사실(user_fact)은 문형을 안 바꾼다(균일 배선 · 특례 아님)', () => {
  const m = buildModelMessages({
    ...기본,
    admittedContext: ['나는 회계팀에서 일한다.'],
    admittedRich: [{ statement: '나는 회계팀에서 일한다.', kind: 'user_fact' }],
  });
  const 몸 = 블록몸(m.system, '[사용자에 대해 알고 있는 것');
  assert.ok(몸.includes('- 나는 회계팀에서 일한다.'), '사실 진술까지 선호 틀로 감쌌다 — 신분을 안 본 것이다');
  assert.ok(!몸.includes('사용자가 정한 선호: "나는 회계팀에서 일한다."'), '신분 구분이 무너졌다');
});
