...
// ── ④ 외부효과 뒷단 · 벽시계 · 취소 ────────────────────────────────────────
test('④ 변경 없는 터미널 읽기는 손 선언만으로 좁은 예산을 먹지 않는다', async () => {
  ...
  const localTerminal = {
    ...
    async handler(a) { 돈것.push(a.command); return { result: { command: a.command, exitCode: 0, stdout: '', cwd: dir, applied: false }, userSafeSummary: '확인했어요.' }; },
  };
  let 냈나 = false;
  const model = {
    async respond(tc, opts = {}) {
  ...
  assert.equal(돈것.length, 8, `변경 없는 읽기가 위험 상한에서 잘렸다: ${돈것.join(', ')}`);
});
