...
// **S1a(2026-08-05): 같은 사실을 지키되 자리를 옮겼다.**
// 예전엔 이 블록을 `user` 에서 찾았다. 그게 오너가 "안녕"에 능력을 읊는 답을 받은 원인이었다 —
// 모델은 그걸 **사용자가 한 말로 읽는다**(S0 계측). 사실은 옳고 자리가 틀렸다.
// 그래서 검사도 `system` 에서 찾는다. **사실이 사라졌으면 여전히 빨개진다** — 그게 이 검사의 일이다.
test('분야를 분류하지 않아도 T5가 먼저 맡을 수 있는 엔지니어링 현실이 모델 입력에 간다', () => {
  const { system, user } = promptFor();
  assert.match(system, /\[네가 먼저 맡을 수 있는 일\]/);
  ...
  // 그리고 **사용자 입으로는 말하지 않는다.**
  assert.doesNotMatch(user, /먼저 맡을 수 있는 일/);
});
...
test('외부 서비스 현실이 없어도 운영 현실은 남는다', () => {
  const { system, user } = promptFor('이 컴퓨터 왜 느린지 봐줘');
  assert.match(system, /먼저 맡을 수 있는 일/);
