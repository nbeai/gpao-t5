 ...
 */
/** 무대는 **한 번만** 세운다 — 검사마다 서버를 새로 띄우면 같은 사실을 네 번 사서 CPU 만 태운다. */
let 한번;
const 되돌림턴 = () => (한번 ??= 되돌림턴만들기());

async function 되돌림턴만들기() {
...
// ── **되돌림만이 아니다** — 무관한 답이 계산되면 그것도 대체다 ──────────────
//
// F-8 을 출구 검증 경로에서 닫은 뒤에도, 라이브에서 같은 모양이 또 나왔다
// (사람 사용시험 `approval_lifecycle`, 2026-08-04): 저장된 답 1378자 안에
//   "…안전 때문에 `rm -rf` 실행은 보류했어." + "지금 내가 직접 볼 수 있는 범위(fixture…"
// 두 답이 이음새 없이 붙었다. 이번엔 되돌림이 아니라 **다른 재작성 경로**였다.
//
// 뿌리는 같다: `미리보기정렬` 의 마지막 갈래가 *"화면을 물릴 수 없으므로 답이 화면을
// 따라온다"* 며 **무관한 답을 앞 말 뒤에 잇는다.** 그 전제는 이제 참이 아니다.
//
// 잃으면 안 되는 이어감(중간에 흐른 말 + 최종 답)은 그대로 둔다 — 재는 것은
// **앞 말과 무관한 답이 계산된 경우** 하나뿐이다.
import { 미리보기정렬 } from '../src/kernel/turn-surface.js';

const 무대 = (shown) => {
  const 지운적 = [];
  const pv = {
    shown, emit(p) { pv.shown += String(p ?? ''); }, retract() { 지운적.push(pv.shown); pv.shown = ''; },
  };
  return { pv, 지운적 };
};

test('앞 말과 **무관한 답**이 계산되면 앞 말을 물리고 대체한다', () => {
  const { pv } = 무대('실행은 보류했어.');
  const 결과 = 미리보기정렬('지금 내가 볼 수 있는 범위 안에는 그 폴더가 없어.', pv);
  assert.doesNotMatch(결과, /보류했어/,
    `무관한 두 답이 한 답으로 이어붙었다 — 사용자는 자기모순인 답을 받는다: "${결과}"`);
  assert.match(결과, /범위 안에는/, '새 답이 사라졌다');
});

test('**이어지는 답**은 그대로 이어 간다(잃으면 안 되는 말)', () => {
  const { pv } = 무대('앞부분 ');
  assert.equal(미리보기정렬('앞부분 뒷부분', pv), '앞부분 뒷부분', '자연스러운 이어감을 끊었다');
});

test('흐른 말이 최종 답으로 끝나면 흐른 말이 답이다(중간 말 보존)', () => {
  const { pv } = 무대('중간에 한 말. 최종 답');
  assert.equal(미리보기정렬('최종 답', pv), '중간에 한 말. 최종 답');
});
