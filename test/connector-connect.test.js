...
test('탐색 뒤에 나온 선언되지 않은 연결도 승인 카드로 만들지 않는다', async () => {
  ...
  assert.ok(r.ledger.unconfirmed.some((x) => /바로 시작할 수 있는 연결 방식이 없어요/.test(x)));
});

test('선언만 있고 실행 방식이 없는 연결도 승인 카드로 만들지 않는다', async () => {
  ...
  // 자격 저장소를 볼 수 있어야 해서 async 다(끊기 갈래가 저장된 자격을 확인한다).
  const result = await connect.approvalEligibility({ connector: '준비 중 서비스' });
