...
// ── 2차 수리 선빨강 (§7-ch-4·ch-5 · 라이브 3/3 목록 뒤) ──────────────────────
// 1차 수리(기억 블록에 충돌 문장 한 줄)는 **모델에게 도달했는데 행동을 안 바꿨다**(수리후1 미달).
// 측정에서 드러난 것: 같은 선호가 **두 채널**로 실린다 — ⓐ carryableWork → `[현재 합의]`
// (work-state.js · 충돌 문법 0글자) · ⓑ admittedContext 사실 블록(1차 수리가 얹힌 자리).
// 그리고 공통 규약 자리인 **판단 헌장 `<맥락>`** 에 「이전 **대화**는 지금 발화를 덮지 않는다」가
// 이미 있었다 — 낱말이 **대화**뿐이라 저장된 기억·합의를 이름으로 덮지 않는다.
// 블록마다 문장을 복제하지 않고(배선은 깔끔하게) **헌장 한 절에서 범위를 정확히 한다.**
//
// 등급(§7-ch-5 ㉯ — 이름으로 닫기 차단): 아래 ①은 **배선 확인**이고 ②가 **주 닻**이다.
// **종료 판정은 라이브 재측정뿐** — 이 둘이 초록이어도 「수리됨」이라 적지 않는다.
import { judgmentCharter } from '../src/kernel/judgment-charter.js';

test('★ 선빨강 ①(배선 확인 · 종료 판정 아님) — 헌장이 「덮지 않는다」의 범위에 기억·합의를 이름으로 넣는다', () => {
  const c = judgmentCharter();
  const 맥락 = c.slice(c.indexOf('<맥락>'), c.indexOf('</맥락>'));
  assert.ok(맥락.includes('덮지 않는다'), '전제 붕괴 — <맥락> 절에 「덮지 않는다」 줄이 없다');
  assert.match(맥락, /기억/,
    '**헌장의 「덮지 않는다」가 「이전 대화」만 말한다** — 저장된 기억·합의는 그 범위에 이름으로 '
    + '없다. 라이브에서 저장된 선호가 지금 요청을 3/3 이겼다(§7-ch-1·ch-4).');
  assert.match(맥락, /합의/, '「합의」도 범위에 있어야 한다 — [현재 합의] 채널이 그 이름으로 실린다');
});

test('★ 선빨강 ②(주 닻) — 저장된 선호가 실린 실제 조립 결과에 그 범위 문장이 함께 간다', () => {
  const m = buildModelMessages(선호실림);
  assert.ok(m.system.includes(선호문장), '전제 붕괴 — 선호가 안 실렸다');
  const 맥락 = m.system.slice(m.system.indexOf('<맥락>'), m.system.indexOf('</맥락>'));
  assert.ok(맥락.length > 0, '조립 결과에 헌장 <맥락> 절이 없다 — 헌장이 아예 안 실렸다');
  assert.match(맥락, /기억[\s\S]*덮지 않는다|덮지 않는다[\s\S]*기억/,
    '**사용자 경로에 실제로 가는 문자열에 「기억은 지금 발화를 덮지 않는다」가 없다** — '
    + '함수층만 고치고 조립을 안 보면 이름으로 닫는 것이다(§7-ch-5 ㉯).');
});

test('닻 — 헌장 길이 상한을 넘지 않는다(문턱 약화 0 · 순증 0 압축)', () => {
  assert.ok(judgmentCharter().length <= 1955,
    `헌장이 상한 1955 를 넘었다(${judgmentCharter().length}) — 순증 0 압축 계약이 깨졌다`);
});

test('닻 — 「받아들인 기준」 줄은 문자 그대로 산다(가설 (라)의 자리 · 이번 수리 접촉 0)', () => {
  assert.ok(judgmentCharter().includes('사용자가 받아들인 기준만 조용히 반영'),
    '선등록이 접촉 0 이라 선언한 줄이 바뀌었다 — 범위 이탈이다');
});
