...
//
// 터미널 결과는 예전엔 위 갈래에 안 걸려 마지막 갈래(JSON 통짜)로 떨어졌다. 그래서 잘릴 때
// **JSON 문자열 한가운데**가 끊겼고, stdout 의 줄바꿈은 `\n` 리터럴로 눌려 행 구조가 사라졌다.
// 이제 터미널 갈래가 선다(task-context `③-b` · 2026-08-14) — **재는 것은 그대로다:**
// 조용히 자르지 않고, 나머지로 갈 값을 준다. 표식만 그 갈래의 말로 바뀐다.
test('명령: 긴 출력이 잘리면 **전체 크기·뺀 양·이어 받을 문**이 함께 온다', () => {
  ...
  assert.match(글, /전체 \d+줄 \d+자/, '명령 출력이 조용히 잘렸다 — 모델은 받은 것이 전부인 줄 안다');
  assert.match(글, /가운데 \d+줄\(\d+자\)은 이 답에 없다/, '뺀 양을 안 밝히면 조용한 절단이다');
  assert.match(글, /sed -n '\d+,\d+p'/, '잘렸다는 말만 하고 나머지로 갈 값이 없다');
  // 줄 한가운데서 끊으면 모델은 반토막을 값으로 읽는다 — 앞도 뒤도 온전한 행이어야 한다.
  assert.match(글, /^src\/a0\.js:0:TODO 고치기$/m, '앞줄이 한가운데서 끊겼다');
  assert.match(글, /^src\/a399\.js:399:TODO 고치기$/m, '결론(마지막 줄)이 한가운데서 끊겼다');
});
