...
test('명령: 긴 출력이 잘리면 **전체 크기·실은 범위·다음 위치**가 함께 온다', () => {
  // §5-3 b(2026-08-12): 표식이 "생략"(산문)에서 전체 크기·실은 범위·다음 위치(값)로 올라갔다.
  // 재는 것은 같다 — 조용히 자르지 않는다는 사실.
  const 글 = compactResult({
    command: 'grep -rn TODO .', exitCode: 0, applied: true,
    stdout: Array.from({ length: 400 }, (_, i) => `src/a${i}.js:${i}:TODO 고치기`).join('\n'),
  });
  assert.match(String(글), /잘림: 전체 \d+자/, '명령 출력이 조용히 잘렸다 — 모델은 받은 것이 전부인 줄 안다');
  assert.match(String(글), /다음 위치 \d+/, '잘렸다는 말만 하고 나머지로 갈 값이 없다');
});
