...
async function executePlan(intent, plan, selfState, ctx, ledger, summary, admitted = [], sendArgs, requestText = intent.currentRequest, 앞선막힘 = [], 첫응답나머지 = [], 이월된것 = new Set(), 계획호출신분 = {}) {
  ...
  const 목적미달 = () => {
    ...
    // ── **정직한 미완 고지가 판정을 끄지 않는다** (라이브 실측 2026-08-14 · gpt-5.1) ──────
    //
    // `빈손으로끝났나` 는 첫 줄에서 `미완료를밝혔나` 면 무조건 false 를 낸다. **그 함수 자리
    // 에서는 옳다** — 정직한 미완료는 빈손 심문도 약속도 아니다. 그런데 이 갈래는 그것을
    // 「목적에 닿았나」의 자로 썼고, 그래서 답에 `아직` 한 낱말이 들어가면 손이 쥐여 준 사실을
    // **한 번도 안 세고** 턴이 닫혔다(`evidence/live5-2026-08-14/run-125806/과업2-*.json`):
    //   원장  local.locate → {candidates:[], canWiden:true, suggestDepth:5, placesToLook:[셋]}
    //   답    "…순매출 계산 자체를 **아직** 못 하고 있어. … 어느 자리 아래에 있는지 짚어 줄 수 있어?"
    //   결과  되부름 0 — 사용자는 실재하는 폴더를 자기가 짚어 달라는 말을 받았다
    // 구조가 *"정직하게 못 했다고 말하면 멈춘다"* 였다. 뒤집으면 *"계속 가려면 거짓말해야
    // 한다"* 이고, **정직한 미완 고지와 목적 미달은 한 턴에 함께 있는 것이 정상**이다(오너 정의
    // 2026-08-14: 말귀는 목적이 달성될 때까지 이어가는 루프다).
    //
    // 그래서 **배타를 끊는다 — 그물은 안 넓힌다.** 미완 고지는 「목적에 안 닿았다」는 답 쪽
    // 신호로 옆에 설 뿐이고, 되부름을 실제로 여는 재료는 아래 그대로 **영수증의 사실**이다
    // (안 연 후보 · canWiden·placesToLook 을 낸 얕은 찾기). 그 사실이 없으면 — 손이 다 훑었고
    // 더 볼 자리가 없으면 — 예전처럼 아무것도 안 선다. 그쪽이 이 수리의 **반대편 위험**이다:
    // 할 일을 다 하고 정직하게 끝낸 턴까지 끌고 가면 사용자는 같은 말을 두 번 듣고 예산만 탄다.
    const 목적에안닿은답 = 빈손으로끝났나(답글원문) || 미완료를밝혔나(답글원문);
    if (!읽었다 && 목적에안닿은답) {
      if (후보들.length) {
    ...
    // ── **파악을 마친 물음은 빈손이 아니다**(J12 · 사슬 22개 실측 §7-m·§7-n) ────────────
    //
    // 같은 발화·같은 걸음 순서인데 답글의 물음표 하나로 되부름이 갈렸다. 손을 불러 목록을
    // 정확히 답하고 "먼저 볼 파일 있어?"를 붙인 답이 빈손으로 읽혀 재생성이 돌았고, 그
    // 재생성은 35턴에서 **한 번도 채택되지 않았다**(0/35 — 왕복만 남았다).
    //
    // 오너 정의(2026-08-14 · 말귀): *"되물음은 감점이 아니다. **파악 없이 묻는 것**이
    // 떠넘김이다."* 그 「파악」의 자를 새로 만들지 않는다 — F-107 이 세운 `빈손으로돌아왔나`
    // 한 벌을 그대로 쓴다(껍데기·thin·blocked 는 사실을 가져온 걸음이 아니다).
    // 약속 갈래는 안 끈다: 사실을 가져오고도 "이어서 해볼게요"로 닫으면 그대로 빈손이다.
    const 사실을가져온걸음 = 부른것들.some((r) => (r.failureState ?? 'none') === 'none' && !빈손으로돌아왔나(r));
    const 걸음이말하는미달 = 부른것들.length
      && (막힌것.length
        || 빈손으로끝났나(답글원문, null, { 심문은빈손아님: 사실을가져온걸음 })
        || 안밟은수단.length);
    if (걸음이말하는미달 || 한걸음도안뗐다) {
