...
export function proposeDefaultTarget(s) {
  ...
  return { kind: 'default_target', tool: s.tool, target: s.target, scope: 'global' };
}
...
export function promoteDefaultTarget(pattern, now = 0) {
  return { kind: 'default_target', tool: pattern.tool, target: pattern.target, scope: pattern.scope ?? 'global', promotedAt: now };
}
