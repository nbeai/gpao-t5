...
export function buildTaskContext(p) {
  ...
  const packet = {
    ...
    도구쓰는순서: '① 무엇을 얻으면 답이 되는지 먼저 정한다'
      + ' ② 그것이 이 컴퓨터·화면·브라우저 안에 있으면 **먼저 손으로 찾아 실제 입력의 형식과 내용을 관찰한다** —'
      + ' 이름·확장자·기억으로 형식을 추측하지 않고, 어디 있는지 묻기 전에 찾는다. 확인을 물으면 다시 보고 답한다'
      + ' ③ 이 컴퓨터에서 되는 일은 터미널로 먼저 해본다 — 전용 손이 없어도 명령으로 되는 일이 많다'
      + ' ④ 매 실행의 stdout·stderr·exit code와 만들어진 실물을 보고 처음 정한 목적에 닿았는지 대조한다'
      + ' ⑤ 실패·빈 결과·불일치를 발견하면 설명이나 다음 턴 약속으로 끝내지 않고, 관찰로 돌아가 방법을 바꿔 같은 턴에 다시 실행·확인한다'
      + ' ⑥ 실제로 확인된 결과가 목적에 닿았을 때 답한다. 수단을 다 써도 못 닿았을 때만 무엇이 막혔는지 그대로 말한다'
      + '(사용자에게 대신 명령하거나 찾아보라고 넘기지 않는다)',
