...
export function buildTaskContext(p) {
  ...
  const packet = {
    ...
    // **오래 기억할 말이 있어 보인다는 사실 하나.** 판정이 아니라 신호다 — 선언인지 질문인지는
    // 모델이 정한다(감사 TG5-CX-01: OS 가 문자열로 의도를 판정하려다 질문을 선호로 저장했다).
    // 이 사실이 없으면 모델이 후보를 낼지 말지가 그때그때 달라지고, 그 습관 차이가 곧 사용자
    // 마찰 차이가 된다 — 같은 말인데 어떤 날은 카드가 뜨고 어떤 날은 안 뜬다.
    memoryHint: p.memoryHint ?? null,
