...
const WHY_APPROVAL = {
  ...
  // **모르면 승인으로 가되, 모른다고 말한다.** 안 밟은 사실을 카드에 적으면 사용자는
  // 일어나지도 않을 변경에 승인을 누른다 — 실측(2026-08-15 · §7-w): 아무것도 안 바꾸는
  // `find … 2>/dev/null | head` 가 "내용을 남기거나 덮어쓰는 일이라"는 카드로 갔고,
  // 사용자는 답을 못 받았다. 아래 fallback 문장("되돌리기 어렵거나 밖으로 나가는 일이라")도
  // 같은 단정이라 미상에는 못 쓴다.
  [UNKNOWN_KIND]: '무엇을 바꾸는지 시험만으로 확정하지 못해서 실행 전에 확인받아요.',
