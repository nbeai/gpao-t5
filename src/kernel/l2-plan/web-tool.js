...
export function defineWebTool(d = {}) {
  ...
  const base = defineTool({
    ...
    // P2-9: **못 하는 것도 능력의 일부다.** 브라우저로 열어 버튼·탭·스크롤을 다루는 손이 없다는
    // 사실이 여기 없으면, 모델은 리뷰탭이나 무한스크롤 뒤를 못 읽은 이유를 스스로 지어낸다
    // (실측: "수집이 제한돼서" — 제한된 적 없다). 능력 부재는 **실패가 아니라 한계**다.
    capability: d.capability
      ?? '주소를 주면 그 페이지를 읽고, 주소가 없으면 찾아서 읽는다(읽기 전용). 읽은 내용은 출처와 함께 준다.'
        + ' 로그인이 필요하거나 수집을 막은 페이지는 읽지 못한다.'
        + ' 페이지를 **열어서 눌러 보지는 못한다** — 자바스크립트로 그려지거나 버튼·탭·더보기·스크롤 뒤에 있는 내용은'
        + ' 들어오지 않는다(브라우저를 띄워 조작하는 손은 아직 없다). 그런 곳은 주소를 직접 받으면 그 주소로 읽을 수 있다.',
