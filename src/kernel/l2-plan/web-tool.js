...
export function defineWebTool(d = {}) {
  ...
  const base = defineTool({
    ...
    보는것: '웹',
    needsApproval: false, // 인증/사용자 개방 세션은 읽기 자체의 승인 사유가 아니다
    ...
    capability: d.capability
      ?? '주소를 주면 그 페이지를 읽고, 주소가 없으면 찾아서 읽는다(읽기 전용). 읽은 내용은 출처와 함께 준다.'
        ...
        // **칸 1 정정.** 예전엔 여기서 *"…스크롤 뒤에 있는 내용은 이 도구로는 들어오지 않는다"* 로
        // `scroll` 을 부정했다. 그건 이 손이 안 가진 동사이고(브라우저·화면 손의 것),
        // 능력 문장들은 프롬프트에서 뭉쳐 실리므로 그 부정에 경계가 없었다 —
        // 바로 위 주석이 세운 규칙(*"한 도구의 능력 설명은 다른 도구의 부재를 주장하지 않는다"*)을
        // 이 문장이 스스로 어기고 있었다. 부정은 아래 `limits` 로 내리고 여기서는 손을 가리킨다.
        + ' 화면을 열어 눌러 보거나 밀어 봐야 나오는 내용은 **브라우저로 화면 보기**·**브라우저 화면 넘기기**가 맡는다.'
    ...
    limits: d.limits ?? [
      ...
      { says: '자바스크립트로 그려지거나 버튼·탭 뒤에 있는 내용은 못 읽는다', 동사: 'snapshot', coveredBy: 'browser.observe' },
      // 칸 1: `scroll` 은 이 손에 없다 — 부정하지 말고 **가진 손을 가리킨다.**
      { says: '아래로 내려야 나오는 내용은 이 손으로 안 들어온다', 동사: 'scroll', coveredBy: 'browser.act' },
    ],
    ...
    schema: {
      description: '주소의 원문을 직접 읽는다(빠르다).'
        ...
        // **찾기와 읽기가 나뉘었다**(2026-08-05). 이 문장이 없으면 모델은 예전처럼 검색어를
        // 이 손에 넣고, 그러면 **첫 결과가 자동으로 읽히면서 고를 자리가 다시 사라진다.**
        + ' **어디를 읽을지 아직 모르면 먼저 `web.search` 로 후보 목록을 받아 고른다** —'
        + ' 그 목록의 요약을 보고 목적에 맞는 출처를 정한 다음 그 주소를 여기로 읽는다.'
        + ' 한 곳이 막히거나 원하는 내용이 없으면 **질의 문구를 바꾸지 말고 다른 후보 주소로 간다**'
        + ' (막힌 결과에도 `다른후보`·`다음수단` 이 함께 온다).'
...
/**
 * **찾는 손의 선언.** 읽는 손(`web.collect`)에서 갈라 나온다(오너 라이브 2026-08-05).
 *
 * 왜 따로 서는가 — `web.collect{request}` 는 한 칸에 "찾을 것"과 "읽을 주소"가 섞여 있고
 * 검색하면 자동으로 첫 결과를 읽는다. 그래서 **모델이 "후보만 보여 줘"를 부를 수 없었다.**
 * 비교군은 목록을 보고 출처를 고르는데 T5 는 첫 결과를 읽고 막히면 질의 문구를 바꿨다.
 * 손을 나누는 것이 그 순서를 여는 유일한 길이다.
 *
 * **출처 원장 계약은 안 건다.** 아무것도 안 읽었으므로 "읽은 근거"가 없는 게 정직하다.
 * 대신 결과가 `읽은상태: '후보만'` 으로 무엇을 얻었는지 스스로 밝힌다.
 */
export function defineWebSearchTool(d = {}) {
  return defineTool({
    id: d.id ?? 'web.search',
    label: d.label ?? '웹에서 찾기',
    owner: 'core',
    ...
    capability: '웹에서 무엇이 있는지 **후보를 찾아 목록으로 준다**(제목·주소·짧은 요약).'
      + ' 페이지를 열어 읽지는 않는다 — 어디를 읽을지는 목록을 보고 고른 다음 웹 자료 수집으로 읽는다.',
    operatorFact: '웹에서 후보를 찾아 목록으로 낸다(읽기는 하지 않는다).',
    limits: [
      { says: '찾기만 하고 페이지 내용은 읽지 않는다', coveredBy: 'web.collect' },
    ],
    schema: {
      description: '무엇을 찾을지 주면 **후보 목록**(제목·주소·요약)을 돌려준다. 페이지는 열지 않는다.'
        + ' **어디를 읽을지 네가 고르는 자리다** — 요약을 보고 목적에 맞는 출처를 골라'
        + ' `web.collect` 로 그 주소를 읽는다. 한 곳이 막히거나 원하는 내용이 없으면'
        + ' **질의 문구를 바꾸지 말고 목록의 다른 후보로 간다.**'
        + ' 이미 읽을 주소를 알고 있으면 이 손을 거치지 말고 `web.collect` 로 바로 읽는다.',
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: '찾을 말(주소가 아니라 검색어)' },
        },
        required: ['query'],
      },
    },
