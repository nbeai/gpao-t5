...
export function parseCompletionCriteria(text) {
  ...
  const count = /(\d+)\s*(?:건|개|개의|건의|명|줄|행|row|rows|items?)/i.exec(t);
  if (count) checks.push({ type: 'count', expected: Number(count[1]) });
  if (/중복\s*(?:이|은|는)?\s*없/.test(t) || /no\s*duplicate/i.test(t)) checks.push({ type: 'no_duplicate' });
  if (/누락\s*(?:이|은|는)?\s*없|빠진\s*(?:것|게)?\s*없|missing.*(?:없|no)/i.test(t)) checks.push({ type: 'no_missing' });
  ...
  const sections = [];
  for (const m of t.matchAll(secRun)) {
