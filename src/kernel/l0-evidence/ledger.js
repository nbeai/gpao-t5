...
export function projectReceipts(entries) {
  ...
  for (const e of entries) {
    if (확인된사실(e)) {
      ...
      //
      // 손이 **실제로 쥔 다음 수**(`다음수단` — tool-receipt 계약)만 여기로 온다.
      //
      // **없으면 안 준다.** 사람용 문장으로 메우면 그건 옵트인이 되고, 채워야 할 손 목록이
      // 생긴다 — 오늘 하루 종일 앓은 병이 그것이다(만든 것과 닿은 것). 비면 모델은
      // *"막혔다"* 만 알고 스스로 판단한다. **비어 있는 것보다 틀린 것이 나쁘다.**
      const 길 = (e.다음수단 ?? []).map((x) => x?.왜 ?? x?.방법).filter(Boolean).slice(0, 3);
      const 다음 = 길.length ? `이어서 할 수 있는 것: ${길.join(' · ')}` : null;
