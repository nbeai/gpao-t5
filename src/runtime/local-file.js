 ...
 *
 * 안 읽은 CSV 가 남은 폴더는 공백 대조용으로 낸다. **다 읽은 폴더(표 2개 이상)도 낸다** —
 * "대조할 공백이 없다"던 첫 판은 6단계 ⑫ R1 에서 반증됐다: 네 표를 **다 읽고도** 실물의
 * 합은 절반의 파일에서만 왔고, 빠진 파일들은 **이름으로만** 불렸다(호명 ≠ 반영 · PM 라벨확정
 * 2026-08-09). 폴더의 모든 CSV 합을 알 때만 열별 전체 합을 낸다(하나라도 모르면 전체 합을
 * 지어내지 않는다 — 그때는 빠진 파일의 개별 합 사실만 남는다). 판단은 없다 — 원장 영수증의
 * 기계 사실 재배열이다.
...
export function 표맥락에서(receipts) {
  ...
  const 맥락 = [...폴더들.entries()].map(([폴더, f]) => {
    ...
    let 전체합 = null;
    // 다 읽었거나(공백 0) 빠진 것의 합을 전부 알 때만 — 모르는 채 전체를 지어내지 않는다.
    ...
    // 대조할 것이 있는 폴더만: 공백(안 읽은 이웃)이 남았거나, 다 읽은 표가 둘 이상이라
    // 부분합↔전체 합이 갈릴 수 있는 폴더. 표 하나뿐인 폴더는 그 파일 합이 곧 전체라 공백이 없다.
  }).filter((x) => x.안읽은.length || Object.keys(x.부분합).length >= 2);
 ...
 *
 * 붙는 조건 전부: 이번 턴 표 맥락이 있고(안 읽은 CSV 가 남았거나 읽은 표가 둘 이상인 폴더) ·
 * 쓴 글에 숫자와 총·전체·합계 명명이 있고 · 글의 숫자에 부분합은 있는데 전체 합은 없다.
 * 안 읽은 파일은 이름으로 지명하면 통과(범위를 밝힌 것). **읽은 파일은 이름이 통과가 아니다**
 * — 6단계 ⑫ R1 패배(PM 라벨확정 2026-08-09): 실물이 네 파일을 전부 "기준"이라 부르면서
 * 합은 절반의 파일에서만 냈다. 호명 기반 → **숫자 기반**: 읽은 표들의 기계 합이 쓴 숫자
 * 집합에 없으면 안 실린 것이고, 그 파일을 근거로 부르고 있으면 모순 사실로 되부른다.
 * 전체 합을 모르면 주장하지 않는다 — 빠진 파일의 개별 합 사실만 남긴다(지어내지 않는다).
 */
function 쓴숫자대조(text, 표맥락) {
  ...
  for (const f of 표맥락) {
    ...
    } else if (어긋난열.length && Object.keys(f.부분합 ?? {}).length >= 2) {
      // **다 읽은 폴더 — 숫자 기반 판정**(⑫ R1 수리). 이름을 불렀는지는 묻지 않는다.
      const 근거로부름 = Object.keys(f.부분합).filter((n) => 글.includes(n));
      조각.push(`${폴더이름}의 읽은 표 ${Object.keys(f.부분합).length}개 ${어긋난열.slice(0, 2)
        .map(([열, v]) => `${열} 전체 합 ${v.toLocaleString('ko-KR')}`).join(' · ')}이 쓴 숫자에 없음`
        + (근거로부름.length ? ` — 글은 ${근거로부름.slice(0, 4).join('·')} 을(를) 근거로 부르면서 그 합은 안 실었다` : ''));
    }
  }
