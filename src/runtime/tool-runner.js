    ...
    try {
      ...
      if (out && out.blocked) {
        return receipt({
          ...
          // **막혔어도 손이 쥔 것은 떨어뜨리지 않는다.** 안 가 본 후보는 검색기가 실제로 준 사실이다.
          //
          // ── **막힌 갈래도 `result` 를 싣는다**(기본 ③ · 2026-08-14) ─────────────────
          //
          // 예전 이 자리는 *"내용(`result`)은 여전히 안 싣는다 — 못 본 것은 못 본 것이다"* 였다.
          // 그 문장이 **두 가지를 한 칸에 묶어** 놓고 있었다: 「못 본 페이지의 본문」과
          // 「막히는 동안 우리가 실제로 밟은 기계 사실」. 웹에서는 둘이 같아서 티가 안 났는데,
          // 터미널에서는 갈린다 — `local-terminal.js` 의 막힌 갈래는 `probe.exitCode`·
          // `probe.stderr`·`blockedBy`·`blockReason`·`probeChangedNothing` 을 **낸다.**
          // 그건 못 본 내용이 아니라 **돌려 보고 알아낸 것**이다. 여기서 통째로 버렸다.
          // 쓰기 명령은 전부 이 경로라, 「저장해줘」 계열의 실체가 매번 원장에서 사라졌다.
          //
          // **승격이 아니다.** `failureState` 는 그대로 blocked 라 `확인된사실`(ledger.js)은
          // 여전히 false 이고, 모델 교환에서도 `data` 로 안 간다(task-context 의 실패 갈래) —
          // 원문은 `확인안됨` 표식을 단 `실패원문` 으로만 간다. 원장이 무엇에 막혔는지 아는 것과
          // 그것을 확인된 사실로 세는 것은 다른 일이고, 그 둘을 가르는 자리는 이미 서 있다.
          //
          // 출처 원장 필수 도구(웹)는 애초에 여기 못 온다 — `assertWebEvidence` 가 위에서
          // 「내용 담은 실패」를 계약 위반으로 떨어뜨린다(web-tool.js). 그 봉인은 그대로다.
          result: out.result,
          다음수단: out.다음수단,
          다른후보: out.다른후보,
          막힌곳: out.막힌곳,
