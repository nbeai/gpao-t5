...
//
//   ⚠ **강제 경계는 이보다 넓다 — 두 진실이다**(2026-08-06 밟음 · 장부 F-46).
//     `local-file.js:216·331` 이 실행·자격 판정에서 `[...roots, 홈자리]` 로 **홈 전체를 더한다.**
//     오너 결정이다(같은 파일 주석에 원문이 있다) — *"울타리는 승인·휴지통·발화밖파괴 위에 덧댄
//     중복이었다. 남는 절대선은 보호 영역 하나이고, 터미널은 이미 그 규칙 하나로 돈다."*
//     즉 실제 경계는 **홈 전체 − 보호 영역**이고, 위 선언 넷은 그 안의 이름표다.
//
//     이 줄이 원래 *"기본값이 홈 전체가 되지 않는다"* 라고 적혀 있었다. 코드가 오너 결정을 따라
//     넓어졌는데 주석만 안 따라와서, 읽은 사람이 **T5 가 없는 능력을 주장한다고 오판했다**(실측).
//     선언과 강제를 한 벌로 만들지 못한 것은 F-45 로 남긴다 — 격리 증명이 선언 쪽을 세므로
//     여기서 혼자 합치면 그 증명이 깨진다(F-46).
//
