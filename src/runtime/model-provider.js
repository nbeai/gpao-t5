...
// **종속 한 줄 — 관문마다 같은 글자로**(순서 7 ④ · 감사 확정 · §7-cl).
// 기억·선호의 신분이 채널마다 달라 모순된 신호를 만들었다: 집 지침에는 우선순위 라벨이 있고
// 자기 소개·이어받을 작업·현재 합의에는 없었다. 라이브 4회가 전부 저장 선호에 졌고, 용의자
// 줄·두 벌 가설은 시험으로 반증됐다. **하나의 문장을 하나의 자리에서 정의해 다섯 관문이 같은
// 글자를 쓴다** — 특례 0 · 조립층이라 와이어 5종이 자동으로 싣는다(모델 오버레이 금지: 규칙이
// 특정 공급자 오버레이에 살면 모델을 바꿀 때 조용히 사라진다).
// 라벨은 **사실 진술이지 금지문이 아니다** — 기억은 여전히 답하고, 명시 지시가 없으면 선호는
// 여전히 조용히 반영된다(반대시험 ①②가 그 자리를 문다).
export const 종속한줄 = '이번 요청과 충돌하면 이번 요청이 우선한다(나머지는 그대로 쓴다).';
...
export function buildModelMessages(tc) {
  ...
  if (tc.admittedContext?.length) {
    ...
    if (사실들.length) {
      ...
      // **선호는 사실문 틀로 낸다**(순서 7 ⑤ ② · 오너 (A) 확정 · §7-cm-2).
      // 저장은 원문 그대로다(makeAutoReversible 계약·철회 지목 전제 — 0줄 무접촉). 바꾸는 것은
      // **보여줄 때의 문형**뿐: 「앞으로 …해줘」가 맨몸으로 서면 그 자체가 지시처럼 읽힌다.
      // 사실문 틀에 넣고 원문은 인용으로 보존한다 — 규칙문 금지 · 출처 병기(어디서 온 것인지).
      // 신분을 본다: 선호만 감싸고 사용자에 대한 사실(user_fact 등)은 문형을 안 바꾼다.
      const 신분표 = new Map((tc.admittedRich ?? []).map((e) => [e?.statement, e?.kind]));
      커널블록.push('[사용자에 대해 알고 있는 것]\n'
        + '사용자가 알려 준 사실이다 — 물으면 이걸로 답한다. 여기 없는 것은 지어내지 않는다.\n'
        + `${종속한줄}\n`
        + 사실들.map((c) => (신분표.get(c) === 'preference'
          ? `- 사용자가 정한 선호: "${c}"`
          : `- ${c}`)).join('\n'));
    }
  }
