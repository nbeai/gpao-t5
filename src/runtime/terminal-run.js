...
/**
 * **셸이 실행 중에 값을 만들어 넣는가.** 그러면 명령 문자열과 실제 자리를 **대조할 수 없다** —
 * 못 맞춘 것이 아니라 잴 수가 없는 것이다(§7-al-4 (ㄱ)).
 *
 * 값 치환만 본다: `$(…)` · 백틱 · `$VAR` · `$$`. **글롭(`*`)은 일부러 뺐다** — 글롭은 이미 있는
...
export function executionBlock(r) {
  ...
  // ── **대조 불능은 대조 음성이 아니다** (§7-al-4 (ㄱ) · 라이브 실측 2026-08-16) ──────
  //
  // 바로 위 대조가 거짓인 이유는 **둘**인데 한 칸으로 굴러떨어지고 있었다:
  //   (ㄴ) 대조를 했고 결과가 「지목 안 함」  — 순회하다 만난 자리다. **이 판정은 옳다**
  //   (ㄱ) 대조 자체가 **불가능**했다        — 셸이 실행 중에 이름을 만들었다
  // 라이브가 밟은 것은 (ㄱ)이다. 오너 부탁("이 폴더 전체를 압축해서 백업본 하나 만들어줘")에
  // 모델이 고른 명령이 이랬다:
  //     cd 작업 && tar -czf 작업-backup-$(date +%Y%m%d-%H%M%S).tar.gz 작업
  // 셸이 뱉은 자리(`작업-backup-20260816-004213.tar.gz`)는 **명령 문자열에 있을 수가 없다** —
  // 그 이름은 명령이 도는 동안 만들어졌으니까. 그런데 그 「못 맞춤」을 「안 지목함」으로 읽어
  // `code/failed` 로 떨어뜨렸고, 그러면 `looksBlocked` 가 false 라 **승인 카드도 사다리도 안 열리고
  // 영수증이 성공(failureState none)으로 남았다.** 막힌 걸음이 장부에 성공으로 적힌 것이다.
  //
  // 이건 이 저장소가 이미 쓰는 축이다 — **「안 잰 0」과 「잰 0」은 다르다**(F-105).
  // 못 쟀으면 못 쟀다고 말한다: 위 :325 갈래가 같은 자리에서 쓰는 그 `blocked_unproven` 이고,
  // 그 문구는 **무엇을 바꾸려던 것인지 모른다고 스스로 말한다.** 안 밟은 사실을 카드에 안 적는다.
  //
  // **(ㄴ)은 열지 않는다.** 폴백을 통째로 복사하면 확장 없는 순수 읽기 실패
  // (`awk -f /없는/x.awk` → `can't open file …`)까지 승인으로 가고, 승인해도 결과가 같은 일에
  // 사용자가 헛클릭한다 — 이 수리가 없애려는 병의 거울상이다(손 관리자 실측 2026-08-16).
  //
  // ⚠️ **이 갈래는 어법으로 알아맞히는 마지막 판이다.** 입구(`열지못한자리` 정규식)가 여전히
  //    문자열 어법이라 못 잡는 것이 남아 있다 — 실측된 미탐: `sqlite3 … unable to open database file`.
  //    말투를 하나 더 얹는 다음 수리는 **하지 마라.** 그때는 재설계 후보로 올린다:
  //    「어법으로 알아맞히지 말고 **커널에게 물어본다**」(`sandbox.js` 가 처음부터 쓴 축).
  if (열지못한자리 && 실행중에이름이정해졌나(r.command)) {
    return {
      kind: 'sandbox',
      why: 'blocked_unproven',
      userWhy: '이 자리에서 막힌 게 있는데 무엇을 바꾸려던 것인지는 시험만으로 확정 못 했어요 — 확인만 받으면 바로 실행해요. 아직 아무것도 안 바뀌었어요',
