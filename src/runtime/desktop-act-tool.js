...
export function makeDesktopActTool(deps = {}) {
  ...
  return {
   ...
   previewOf(args = {}) {
     ...
     // **엔터의 실질은 신고 유무와 무관하게 탐침이 읽은 칸 내용에서만 선다**
     // (F-58 (a) · PM 조건 ① 2026-08-10: "모델 신고(기대.값)를 열쇠 재료로 쓰지 않는다").
     // 리허설 실측: 모델이 `바깥으로` 는 신고하고 `기대.값` 을 비우는 회차가 실재했고, 그때
     // 신고 분기가 빈 내용으로 열쇠를 못 세워 같은 방·같은 문구 두 번째에도 카드가 남았다.
     // 반대로 `기대.값 ≠ 칸내용` 이면 카드가 A 를 보여 주고 엔터가 B 를 보내는 어긋남(F-62)
     // 이 된다 — 엔터 걸음은 기계가 읽은 그 내용이 카드에도 열쇠에도 실린다.
     const 엔터키인가 = 행동 === 'press_key' && /^(return|enter)$/i.test(키);
     const probed칸내용 = typeof args.눌러본사실?.칸내용 === 'string' ? args.눌러본사실.칸내용.trim() : '';
     // **바깥으로 나가는 걸음은 무엇이 나가는지 말한다.** 사용자는 이 한 줄을 보고 허락한다.
     if (args.기대?.바깥으로 === true) {
       // 엔터는 **표시와 열쇠를 가른다**(PM 조건 ①): 열쇠는 기계(칸내용)만 — 아래에서 센다.
       // 표시는 기계가 읽었으면 그것(진실이 이긴다), 못 읽었으면 신고값이라도 보여 준다 —
       // 그때는 열쇠가 없어 **카드가 반드시 뜨므로** 검증 안 된 주장을 조용함의 근거로
       // 쓰는 일은 없다. 아예 안 보여 주면 "무엇이 나가는지 없는 카드"(F-44 병)로 돌아간다.
       const 내용 = 엔터키인가 ? (probed칸내용 || String(args.기대?.값 ?? '').trim())
         : String(args.기대?.값 ?? '').trim();
       ...
       return {
         ...
         ...((() => {
           // 엔터의 **열쇠**는 표시와 달리 기계(칸내용)만이다(PM 조건 ①) — 신고값으로
           // 세운 열쇠는 카드 A·실행 B 어긋남(F-62)을 조용함으로 승격시킨다.
           const 열쇠내용 = 엔터키인가 ? probed칸내용 : 내용;
           const 신분 = 자리로짚음 || !열쇠내용 ? null : 발신실질(어디, 창, 열쇠내용);
     ...
     // **엔터의 실질은 탐침이 읽은 칸 내용에서만 선다**(F-58 (a) · PM 조건 ① 2026-08-10).
     // 모델 신고(기대.값)는 열쇠 재료가 아니다 — 신고와 실행이 어긋나는 모양(위험 기록
     // F-62)을 이 걸음에서는 애초에 만들지 않는다. 못 읽었으면 칸이 없다 → 매번 카드.
     const 칸내용 = probed칸내용;
     const 엔터실질 = (엔터키인가 && 칸내용 && !자리로짚음) ? 발신실질(어디, 창, 칸내용) : null;
     // **카드는 무엇이 나가는지 보여야 한다**(보여 준 것 = 기억한 것 · PM 조건 ①).
     // 열쇠에 내용이 들어가는 두 걸음(type·엔터)은 카드 문안에도 그 내용이 실린다 —
     // 문안 없는 열쇠는 사용자가 허락한 적 없는 것을 조용하게 만든다.
     const 내용문안 = 글자넣기실질 ? `${무엇 || '그 칸'} 에 "${String(args.값 ?? args.기대?.값 ?? '').trim().slice(0, 60)}" 넣기`
       : 엔터실질 ? `"${칸내용.slice(0, 60)}" 이(가) 담긴 칸에서 ${키} 누르기` : null;
     return {
       impact: 내용문안 ? [자리, 내용문안].filter(Boolean).join(' · ')
         : 자리 && (행동 === 'click' || 행동 === 'type') ? `${자리} · ${말}` : 말,
       ...(글자넣기실질 ?? 엔터실질 ? { 발신실질: 글자넣기실질 ?? 엔터실질 } : {}),
       scope: '이번 요청',
       duration: '이번 한 번',
   ...
   async probe(args) {
     ...
     if (창신분만) {
       if (!본창) return 모름;
       // **그 칸의 실제 내용도 기계 사실이다**(F-58 (a) · PM 판정 2026-08-10 조건 ①②).
       // 엔터 걸음의 실질(무슨 글자가 이대로 실행되는가)을 모델 신고(기대.값)가 아니라
       // 탐침이 읽은 값에서 세운다. fail-closed 셋:
       //   · 글자칸이 **정확히 하나**이고 값이 있을 때만 — 둘이면 어느 칸이 실행될지 모른다
       //   · **보안 칸(Secure)은 값을 들지 않는다** — 비밀값 불가침(헌장 ①). 아래 정규식이
       //     AXSecureTextField 를 애초에 안 잡고, Secure 가 낀 창은 내용 자체를 안 세운다
       //     (그 칸이 실행될 수 있는 창에서 열쇠를 만들면 비밀이 열쇠 문자열에 스민다)
       //   · 못 읽으면 칸을 안 만든다 → 미상 그대로 → 카드
       const 요소전부 = Array.isArray(요소들) ? 요소들 : [];
       if (요소전부.some((e) => /secure/i.test(String(e?.role ?? '')))) return { 찾음: false, 본창 };
...
async handler(args) {
      ...
      // 증명할 수 없다. 실행에 실제로 쓰는 마지막 관찰에서 다시 찾힌 때만 진행한다.
      if (typeof 드라이버.verify === 'function'
        && 요소짚는것.has(행동) && !눈으로짚음 && !커서에친다) {
        const 신분축 = Boolean(args?.대상?.토큰) || args?.대상?.번호 != null || Boolean(args?.대상?.id);
        if (신분축 && !신분찾기(마지막본것?.elements ?? [], args?.대상)) {
          return {
            blocked: true,
            userSafeSummary: '짚어 주신 것이 실행 직전 화면에서는 보이지 않아 누르지 않았어요. 지금 화면을 다시 보고 짚을게요.',
            다음수단: [{ 방법: 'observe', 왜: '실행에 쓸 fresh 화면 신분을 다시 얻는다' }],
          };
        }
      }
      ...
      if (typeof 드라이버.verify === 'function' && 누르는것.has(행동)) {
        ...
        // **행동에 실제로 쓴 마지막 fresh 관찰의 신분**(라벨·역할·창·pid)으로 옮긴다.
        // 여기서 다시 관찰하면 실행 대상과 검증 대상이 다른 스냅샷/창이 된다(F-113).
        const 그것 = (마지막본것?.elements ?? []).find((e) => e?.id === args?.기대?.요소) ?? null;
        const 실행대상 = 신분찾기(마지막본것?.elements ?? [], args?.대상);
        // **누른 버튼 자체가 남아 있는 것은 클릭 목적의 사후조건이 아니다.**
        // 실모델은 `7` 버튼을 누르며 기대에도 같은 `7` 버튼을 넣었다. verify_state 의
        // `exists:true` 는 클릭 전부터 참이라 버튼을 누르지 않아도 `satisfied` 가 된다.
        // 같은 관찰 요소를 양쪽에 쓴 순환 기대는 술어로 보내지 않고 unknown+그림으로 남겨
        // 모델이 실제 화면 결과를 보고 다음 손을 고르게 한다. 앱/문구별 의미 판정은 하지 않는다.
        const 순환기대 = Boolean(그것 && 실행대상 && 그것 === 실행대상);
        let 답 = null;
        try {
          답 = await 드라이버.verify({
            // **기대가 없을 수 있다**(계산기처럼 기대를 말할 수 없는 대상). 그때도 그림은 받는다 —
            // 여기서 터지면 판정이 통째로 null 이 되고, 그림도 함께 사라진다(실제로 그랬다).
            ...(!순환기대 && args?.기대?.값 !== undefined ? { 값: args.기대.값 } : {}),
            ...(!순환기대 && 그것?.label ? { 라벨: 그것.label } : {}),
            ...(!순환기대 && 그것?.role ? { 역할: 그것.role } : {}),
            창: 마지막본것?.본창?.id ?? args?.window ?? args?.대상?.창 ?? null,
            pid: 마지막본것?.본창?.pid ?? null,
          });
        } catch { 답 = null; }
