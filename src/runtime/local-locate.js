...
/** 낱말과 이름을 같은 자로 쪼갠다 — 자가 두 벌이면 언젠가 벌어진다(§4-6). */
const 낱말경계 = /[\s,./\\_-]+/;

/**
 * **한 글자도 낱말일 수 있다 — 글자 하나가 뜻을 통째로 담는 문자체계에서는**
 * (라이브 실측 · 실모델 gpt-5.1 · 2026-08-14).
 *
 * 사용자: *"**표 폴더**에 이번 달 매출이랑 환불이 들어 있어…"* — 격리 집에 `일감/표/` 가
 * **실재했다.** T5: *"홈 전체, `일감` 안을 5단계까지 뒤졌는데 후보가 안 나와."*
 * 후보 0이라 과업이 시작조차 못 했다. 원인은 아래 `낱말()` 의 `length >= 2` 한 줄이다 —
 * `표` 가 통째로 지워지니 낱말 목록이 비고, 빈 목록은 **어떤 이름과도 안 맞는다.**
 * 한국어는 한 글자 명사가 흔하다(표·집·글·돈·책·방·짐·차). 한국형 AI OS 가 그 이름들을
 * 전부 못 찾는 것이다.
 *
 * 그렇다고 그 규칙을 지우지 않는다 — **목적이 있었다.** 영어의 `a`·`i`·`x` 한 글자는
 * 소음이고, 낱말로 받으면 후보가 폭발한다. 목록(`표·집·글…`)으로 풀지도 않는다 —
 * 목록은 항상 뚫린다. **성질로 가른다:** 라틴 글자는 여럿이 모여야 낱말이 되고(한 글자는
 * 조각이다), 한글 음절·한자는 **글자 하나가 이미 음절이자 뜻**이다. 그 성질만 통과시킨다.
 */
const 한글자로뜻인가 = (w) => /^[\p{Script=Hangul}\p{Ideographic}]$/u.test(w);
...
function 이름맞음종류(name, 부른말, 낱말들) {
  ...
  return 낱말들.length > 0 && 낱말들.some((w) => 낱말이든가(이름, w)) ? 'partial' : null;
}

/**
 * 부른 낱말이 이 이름 안에 있는가. **한 글자 낱말은 조각과 같을 때만 센다.**
 *
 * 한 글자를 낱말로 받아 놓고 `.includes` 로 풀면 위에서 고친 것을 도로 무너뜨린다 —
 * 이 저장소만 세어도 이름에 `표` 가 든 항목이 17개인데 이름이 `표` 인 것은 0개다.
 * 그러면 「이름이 맞아요」가 열일곱 번 거짓말이 되고, 그게 이 파일이 두 번 데인 바로 그 병이다
 * (`md` 가 모든 `.md` 를 잡던 자리 · `지침` 이 《… 설계 지침》.md 를 잡던 자리).
 *
 * 성질의 문제다: 한 글자가 긴 이름 **속에** 있는 것은 뜻이 아니라 **우연**이다
 * (`발표자료`·`표준약관` 의 `표`). 이름을 부른 것이면 그 글자가 이름의 **조각**으로 서 있다
 * (`표` · `표 모음` · `표.tsv`). 낱말을 쪼갠 자와 **같은 자**로 이름을 쪼개 견준다.
 */
function 낱말이든가(이름, w) {
  if (!한글자로뜻인가(w)) return 이름.includes(w);
  return 이름.split(낱말경계).includes(w);
}
...
export function makeLocalLocateTool(deps = {}) {
  ...
  return {
    ...
    async handler(args = {}, executionContext = {}) {
      ...
      //
      // **다만 이 절단이 무엇을 흔드는지는 회차마다 다르다**(실물 홈 실측 2026-08-12).
      // 색인이 전수를 셌으면 걸음의 절단은 **전수를 안 흔든다** — 흔드는 것은 「보여 줄 후보
      // 고르기」뿐이다. 그런데 문장은 정의역을 안 밝힌 채 붙어서 이렇게 나왔다:
      //   *"모두 **133개**예요 … 거기서 **2032개는 못 봤어요**"*
      // 둘 다 참인데(133 은 색인이 센 것, 2032 는 걸음이 못 본 항목 수) 한 답에 나란히 서면
      // 사용자는 한 사실로 읽고 **133 을 못 믿는다.** 참인 문장 둘로 거짓 인상을 만든 것이다.
      //
      // 지우지 않는다 — **무엇에 대한 절단인지**를 말한다. 오픈클로 `find` 도 잘린 것이
      // 「결과 목록」임을 밝히고 다음 길을 준다(`sessions-DIwWArcp.js:6349`).
      const 못본총 = 잘린폴더.reduce((n, f) => n + f.unseen, 0);
      const 색인이셌다 = 세기?.countedBy === 'index+walk';
      const 잘림말 = !잘린폴더.length ? ''
        : 색인이셌다
          // 전수는 색인이 쟀다. 걸음이 덜 본 것은 **후보로 올릴 자리**를 덜 본 것뿐이다.
          ? ` 다만 항목이 아주 많은 폴더 ${잘린폴더.length}곳은 앞 ${MAX_ENTRIES_PER_DIR}개만 봐서,`
            + ` 위 개수는 그대로지만 아래 후보는 그 안을 덜 보고 골랐어요(더 좁혀 주시면 그 안을 볼게요).`
