...
export function makeServer(deps = {}) {
  ...
  async function runAndPersistTurn(session, input, emit, onAnswerDelta, timingEntry, onAnswerReset) {
    ...
    // ── §5-5 승격 집행부: **통로는 하나다**(병합 판정 2026-08-12) ──────────────────────
    //
    // 브랜치 `worktree-agent-a63f71265aa07dc7a`(69a4678)가 같은 자리에 두 번째 승격 통로
    // (`autoPromoteEligible` 정규식 문)를 넣으려 했다. 그 브랜치는 낡은 기준(ca47832)에서
    // 갈라져 나와, 본선이 그 뒤 S1(74be435)에서 **같은 기능을 이미 세운 것을 못 봤다.**
    // 둘을 나란히 두면 이 파일이 스스로 경고한 자리로 간다 — *"승격 통로가 둘이면 replay·
    // 원장·필드가 한쪽에만 붙는 날 다시 갈라진다"*(context-mesh.js). 그래서 아래 한 통로만 둔다.
    //
    // 어느 쪽이 옳은가는 **자로 갈랐다**: 범위("앞으로"인가 "이번만"인가)는 의미 판단이고,
    // 본선은 그것을 모델이 말하게 하고(`evidence.appliesTo`) 런타임은 그 주장이 이번 턴
    // 원문에 실제로 붙어 있는지만 기계로 대조한다(`자동반영가능`). 브랜치는 같은 범위를
    // 낱말 정규식으로 짐작한다. 바로 위 `자동반영가능` 주석이 그 방식이 낸 P0 을 적어 뒀다.
    // 민감정보도 본선이 더 넓다 — 값 탐지(`containsSensitiveValue`)가 낱말 목록보다 앞선다.
    // 브랜치가 홀로 가졌던 `supersedes`(새 지시가 과거를 이김)도 본선엔 모델이 인용과 함께
    // 제출하는 `agreement_superseded` 레인으로 이미 서 있다(같은 원리: 모델이 말하고 런타임이 대조).
    //
  ...
  // 승인이 필요한 일은 채널에서 **자동으로** 실행하지 않는다 — 카드가 서고, 결재 표면은 둘이다:
  // T5 화면 버튼(직접 id), 그리고 채널의 명시 결재(모델이 approval.decide 로 지목 + 코드가
  // 턴 전 동결값으로 자격 판정 — 아래 채널 갈래). 밖에서 "네" 한 마디가 **무자격으로** 외부
  // 효과를 내면 안 된다는 경계는 그대로다 — 자격은 언제나 코드가 판정한다.
  /**
