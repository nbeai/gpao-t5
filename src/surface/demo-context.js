...
const DESCRIPTORS = [
  ...
  defineTool({
    ...
    capability: '이 컴퓨터에서 실행 파일 하나를 argv 인자 배열로 직접 실행해 읽기·탐색·계산한다.'
      + ' 셸 문자열·파이프·리다이렉션은 이 단계에서 실행하지 않는다. 파일 적용은 local.file을 고른다.'
      + ' 실행은 승인 없이 한 번만 하며 파일 쓰기·자식 프로세스·네트워크·호스트 IPC·외부 시그널·비밀 읽기는 격리된다.',
    ...
    schema: {
      description:
        '실행 파일 하나를 provider-neutral executable과 argv 배열로 한 번 실행한다.'
        ...
        + ' 셸 문자열을 조립하지 않는다. 파이프·리다이렉션·파일 변경·네트워크·자식/백그라운드 프로세스는 이 단계 범위가 아니다.'
      ...
      parameters: {
        type: 'object',
        properties: {
          executable: { type: 'string', description: '실행 파일의 절대 경로' },
          argv: { type: 'array', items: { type: 'string' }, description: '실행 파일에 그대로 전달할 인자 배열' },
