  ...
  defineTool({
    ...
    schema: {
      description:
        ...
        + ' 먼저 `head`·`sed`·`cat` 등으로 실제 형식과 대표 내용을 보고, 그 관측에 맞춰 집계한다.'
        + ' 결과 파일이 목적이면 먼저 명령의 stdout으로 정확한 내용을 계산·확인하고, 그 내용을 `local.file` write로 저장한다.'
        + ' 그러면 가역 파일 쓰기로 자동 진행되며, 셸 리다이렉션 승인에 멈추지 않고 같은 턴에 readback까지 할 수 있다.'
