...
function 화면행동선언() {
  return defineTool({
    ...
    // **모델에게 실제로 가는 한 줄은 이것뿐이다**(`capability` 는 안 간다 · 실측).
    // 여기 없는 손은 모델에게 **없는 손**이다 — 좌표 손을 붙여 놓고 이 줄을 안 고쳐서
    ...
    limits: [{ says: '비밀번호 칸에는 입력하지 못한다' },
      { says: '메시지 전송처럼 바깥으로 나가는 것은 사람에게 한 번 물어본 뒤에 한다' }],
    schema: {
