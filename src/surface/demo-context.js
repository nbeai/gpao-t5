...
const DESCRIPTORS = [
  ...
  defineTool({
    ...
    capability: '이 컴퓨터에서 명령을 직접 실행한다. 위치를 모르면 직접 찾는다. 확인·검사·테스트·빌드는 바로 하고,'
      ...
      + ' 결과 파일을 만드는 일은 명령으로 계산한 stdout을 확인한 뒤 로컬 파일 손으로 저장하면 승인 없이 되돌릴 수 있게 끝난다.'
      + ' 작업 폴더에서 되돌릴 수 있는 결과 파일 생성·덮어쓰기는 승인 없이 직접 실행한다.'
      + ' 셸 계산에 중간파일이 필요하면 `/tmp` 고정 경로가 아니라 실행마다 격리된 `$TMPDIR` 안에 만든다.'
      + ' 헌장의 네 경계에 해당하면 실행 직전에 확인 카드가 한 번 뜨고, 승인되면 네가 이어서 실행한다 — 사용자에게 명령어를 적어 주지 않는다.'
    ...
    schema: {
      description:
        ...
        + ' 작업 위치는 명령 문자열 안에서 `cd`하지 말고 `cwd` 인자에 정확한 절대경로로 넣는다.'
        + ' 그래야 샌드박스가 실제 실행 위치와 결과 파일 위치를 같은 사실로 검증한다.'
        + ' 파일 내용에 따라 계산하는 일은 이름이나 확장자로 형식을 추측하지 않는다.'
        + ' 먼저 `head`·`sed`·`cat` 등으로 실제 형식과 대표 내용을 보고, 그 관측에 맞춰 처리한다.'
        + ' 작업공간 안의 되돌릴 수 있는 파일 생성·덮어쓰기는 승인 없이 실행된다.'
        + ' 터미널이 직접 결과 파일을 만들어도 되고 `local.file`로 저장해도 된다. 방법은 목적과 관찰 결과에 맞춰 고른다.'
        + ' 실행 뒤 stdout·stderr·exit code와 결과 실물을 확인한다. 실패·빈 결과·불일치를 발견하면'
        + ' 설명하거나 다음 턴으로 미루지 말고 입력을 다시 보고 방법을 바꿔 같은 턴에 수정·재확인한다.'
        + ' 한 셸 명령 안에 중간파일이 꼭 필요하면 `/tmp/name`을 만들지 말고 `$TMPDIR/name`을 쓴다.'
        + ' `$TMPDIR`은 이 실행만을 위해 열리고 끝나면 폐기되므로 최종 결과 파일만 가역 대상으로 남는다.'
        + ' 헌장의 네 경계가 아닌 작업은 확인 카드를 요구하지 않는다. 오래 걸리는 서버 실행은 아직 이 도구로 하지 않는다.'
