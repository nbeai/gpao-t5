...
const DESCRIPTORS = [
  ...
  defineTool({
    ...
    capability: '보고 있는 화면을 더 본다 — 아래로 내리거나, 탭을 바꾸거나, 더보기를 편다.'
      + ' 몇 번 내렸는지와 왜 멈췄는지를 남긴다.'
      + ' 칸에 글자를 넣거나 보내는 일은 「화면 다루기」 손이 맡는다.',
    행위: {
      scroll: { 말: '아래로 내리기', 별칭: ['스크롤'] },
      click: { 말: '탭·더보기 누르기', 별칭: ['누르', '클릭'] },
    },
    // 자기 동사(`click`)에 건 한계와, 남의 동사를 가리키는 이관(`coveredBy`)을 함께 적는다.
    limits: [{ 동사: 'click', says: '링크는 누르지 않는다 — 주소를 알면 browser.observe 로 연다' },
      { 동사: 'type', coveredBy: 'desktop.act', says: '이 손으로는 칸에 글자를 넣지 않는다 — 칸에 글자 넣기는 「화면 다루기」 손이 한다' }],
    schema: {
