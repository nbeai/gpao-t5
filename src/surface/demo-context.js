...
import { DEFAULT_TIMEOUT_MS } from '../runtime/terminal-run.js';
...
function 화면선언() {
  return defineTool({
    ...
    // **무엇을 보는 손인가**(노드 ③ · 커널이 읽는 축). 글은 모델이 읽고, 축은 커널이 읽는다 —
    // 한 손이 막혔을 때 **같은 것을 보는 다른 손**을 가리키려면 비교할 수 있어야 한다.
    // **축이 둘이다**(F-107 · 오너 정본 2026-08-12: *"뭐가 막든간에 시각적 효과까지
    // 발휘하면 리뷰 읽을 수 있어. 그게 컴퓨터유즈잖아."*). 브라우저 창은 화면에 있고,
    // 그 안의 웹 페이지를 이 손이 본다. 네이버가 막은 것은 HTTP 층이지 **화면에 그려진
    // 픽셀은 못 막는다.** 바로 아래 operatorFact 가 이미 *"로그인 뒤에 있으면(카드사·
    // 배달앱·플레이스) 여기뿐이다"* 라고 적어 뒀다 — 그 사실을 커널이 축으로 읽는다.
    보는것: ['화면', '웹'],
    ...
    capability: '지금 이 컴퓨터 화면에 무엇이 떠 있는지 본다 — 앞에 있는 앱, 열린 창들,'
      ...
      // **칸 1 정정.** 예전엔 *"앞으로 가져오지 않고"* · *"누르거나 입력하지 않는다"* 였다.
      // 셋 다 **이 손이 안 가진 동사**(`desktop.act` 의 focus·click·type)를 부정한 것이고,
      // 능력 문장들은 프롬프트에서 **한 줄로 뭉쳐** 실리므로 그 부정에 손 경계가 없었다 —
      // 모델은 "칸에 글자를 넣는다"와 "입력하지 않는다"를 한 화면에서 읽었다(성질 1).
      // 같은 사실을 **긍정으로** 말하고, 그 동사를 가진 손은 `limits.coveredBy` 가 가리킨다.
      + ' 앞에 없는 앱도 이름을 대면 **보고 있는 창을 그대로 둔 채** 그 창 안을 본다.'
      + ' 이 손은 눈이다 — 화면을 만지는 일은 「화면 다루기」 손이 맡는다.'
    ...
    operatorFact: '이 컴퓨터의 앞 앱·창 목록을 보고, 창 안 요소의 글자(대화·문서 본문·목록 값)까지 읽는다.'
      + ' 앱 이름을 대면 **앞에 없는 창도 보고 있는 창을 그대로 둔 채** 그 안을 본다.'
      + ' 서비스 연결이 없어도 화면에 보이는 것이면 읽는다.'
      + ' 글자로 안 읽히는 창은 **화면을 그대로 보내 준다** — 그 그림을 보고 읽고, 자리를 짚어 만지면 된다.'
      // **무엇을 보는 손인가**(노드 ③ · 오너 정본 2026-08-06). 터미널과 보는 대상이 다르다 —
      // 같은 것을 더/덜 보는 게 아니라 **다른 것**을 본다. 그리고 이 손은 느리고 비싸다.
      + ' 이 손이 보는 것은 **앱이 사람에게 보여주는 화면**이다 —'
      + ' 앱 안 자료가 암호화돼 있거나(카톡 대화) **로그인 뒤에 있으면**(카드사·배달앱·플레이스)'
      + ' 여기뿐이다. 대신 **느리고 비싸다** — 파일·설정으로 되는 일은 그쪽이 낫다.',
    행위: {
      status: { 말: '볼 준비가 됐는지 확인하기' },
      observe: { 말: '앞 앱·열린 창과 창 안 글자 보기' },
    },
    // 칸 1 · 동사 표식 — 이 한계는 `observe` 에 걸린다(`status` 는 권한 없이도 답한다).
    // 그리고 **이 손이 안 하는 일**은 부정이 아니라 `coveredBy` 로 그 손을 가리켜 적는다.
    limits: [{ 동사: 'observe', says: '권한이 없으면 창 목록을 읽지 못한다' },
      { 동사: 'click', coveredBy: 'desktop.act', says: '이 손은 누르지 않는다 — 누르는 것은 「화면 다루기」 손이 한다' },
      { 동사: 'type', coveredBy: 'desktop.act', says: '이 손은 칸에 글자를 넣지 않는다 — 칸에 글자 넣기는 「화면 다루기」 손이 한다' },
      { 동사: 'focus', coveredBy: 'desktop.act', says: '이 손은 창을 앞으로 띄우지 않는다 — 앞으로 띄우기는 「화면 다루기」 손이 한다' }],
    schema: {
      description: '이 컴퓨터 화면 상태를 본다. `status` 는 볼 준비가 됐는지와 앞 앱을,'
        ...
        + ' `app` 을 주면 **앞에 없는 창도 보고 있는 창을 그대로 둔 채** 그 안을 본다 —'
      ...
      parameters: {
        ...
        properties: {
          ...
          app: {
            type: 'string',
            description: '어느 앱의 창을 볼지. **보고 있는 창을 그대로 둔 채 그 앱 창 안을 본다.**'
...
function 화면행동선언() {
  return defineTool({
    ...
    // **주석 정정(칸 1 · 2026-08-11).** 여기 있던 *"모델에게 실제로 가는 한 줄은 이것뿐이다
    // (`capability` 는 안 간다 · 실측)"* 는 **틀렸다.** 기계로 갈랐다 —
    // `self-state.js:readyCapabilities`(= `toolCapabilityLine`) → `task-context.js:readyTools`
    // → `model-provider.js` 의 `네가 지금 바로 쓰는 손:` 으로 **capability 도 간다**
    // (능력을 물은 턴은 `fast_chat` 이 아니라 이름만 실리는 다이어트에 안 걸린다).
    // 그 주석을 믿고 `capability` 를 자유 문장으로 쓴 자리들이 그대로 모델에게 실려 있었다.
    //
    // 여기 없는 손은 여전히 모델에게 **약한 손**이다 — 좌표 손을 붙여 놓고 이 줄을 안 고쳐서
    // 모델이 관찰만 네 번 하고 *"버튼을 누르거나 글자를 입력하는 동작은 못 한다"* 고 답했다.
    // **이제 동사 목록은 산문이 아니라 `행위` 표에서 생성된다** — 아래를 고쳐도 enum 에 있는
    // 동사는 빠지지 않는다(칸 1 S4).
    operatorFact: '창·앱 상태를 바꾸고, **버튼을 누르고 칸에 글자를 넣고 키를 누르고 화면을 민다(스크롤)** —'
      + ' 하고 나서 전후 값으로 실제 변화를 확인한다.'
      // **미는 손이 없으면 어제 대화는 영영 못 읽는다**(라이브 2026-08-06). `scroll` 은
      // 스키마 enum 에 있었지만 모델이 읽는 줄은 이것뿐이라, T5 는 시도조차 않고
      // *"윤님이 위로 스크롤한 화면을 캡처해서 보내 주시면"* 이라고 사람에게 넘겼다.
      + ' 창은 보이는 만큼만 준다 — **화면 밖의 것은 밀어서 본다**(이전 대화·긴 목록).'
      + ' 글자로 안 읽히는 창은 **화면에서 본 자리를 `대상.x`·`대상.y` 로 짚어** 누르고 입력한다'
      + '(관찰이 그 창의 자리·크기를 함께 준다).',
    // **동사 하나하나의 사용자 말**(칸 1). `action` enum 과 1:1 이고, 모델이 읽는 동사 목록은
    // 산문이 아니라 **여기서 생성된다** — 동사를 더하면 말이 따라 늘고, 빼면 말도 빠진다.
    행위: {
      focus: { 말: '창을 앞으로 띄우기', 별칭: ['앞으로 가져오', '앞으로 띄우'] },
      scroll: { 말: '화면 밀기(스크롤)', 별칭: ['스크롤', '아래로 내리'] },
      move: { 말: '창 옮기기' },
      resize: { 말: '창 크기 바꾸기' },
      launch: { 말: '앱 켜기' },
      quit: { 말: '앱 끄기' },
      click: { 말: '누르기', 별칭: ['누르', '클릭'] },
      // 닫는 문장이 겨누는 그 동사다 — *"웹 검색창에 글자 칠 수 있어?"*
      type: { 말: '칸에 글자 넣기', 별칭: ['글자를 넣', '글을 쓰', '입력', '타이핑'] },
      double_click: { 말: '두 번 누르기' },
      right_click: { 말: '오른쪽 버튼으로 누르기' },
      drag: { 말: '끌어 옮기기' },
      press_key: { 말: '키 누르기(엔터·탭·방향키)', 별칭: ['엔터'] },
      hotkey: { 말: '단축키 누르기', 별칭: ['단축키'] },
      menu: { 말: '앱 메뉴에서 고르기' },
      copy: { 말: '복사하기' },
      paste: { 말: '붙여넣기', 별칭: ['붙여넣'] },
      wait: { 말: '잠깐 기다리기' },
    },
    // **"못 한다"는 선언은 모델을 멈춰 세운다.** 손이 할 수 있게 됐는데 이 줄이 남아 있어서
    // 모델은 입력까지만 하고 *"아직 전송은 안 된 상태"* 로 끝냈다(라이브 2026-08-06).
    // 이제 못 하는 것이 아니라 **한 번 물어보고 하는 것**이다.
    //
    // **칸 1 · 동사 표식.** 한계는 이제 **어느 동사에 걸리는지**를 함께 밝힌다. 밝히지 않으면
    // 모델은 그 부정을 손 전체로 읽는다 — 그게 성질 1 의 얼굴이었다.
    limits: [{ 동사: 'type', says: '비밀번호 칸에는 입력하지 못한다 — 그건 사람이 직접 넣는다' },
      { 동사: 'press_key', says: '메시지 전송처럼 바깥으로 나가는 걸음은 사람에게 한 번 물어본 뒤에 한다' }],
    schema: {
      description: '창·앱 상태를 바꾼다. `focus`(앞으로 띄우기) · `scroll` · `move` · `resize`'
        ...
        + ' 밝히면 **사람에게 한 번 물어본 뒤 실행한다.** 숨기지 마라 — 밝힌다고 못 하게 되는 것이 아니다.'
      ...
      parameters: {
        ...
        properties: {
          ...
          // **창을 좁히는 축은 보는 손과 행동 손이 같아야 한다**(라이브 2026-08-06).
          // 이 칸이 없어서, 모델이 `창제목:'박종윤'` 으로 입력칸을 정확히 짚어 놓고도
          // 행동 손은 목록 창을 보고 *"지금 화면에 없어요"* 로 막혔다.
          창제목: {
            type: 'string',
            description: '창 제목의 일부(대화 상대 이름 등). **`desktop.screen` 에서 이걸로 골랐으면'
              + ' 여기에도 똑같이 줘라** — 안 주면 같은 앱의 다른 창에 하게 된다.',
          },
          대상: {
            type: 'object',
            description: '누를 것(관찰이 준 요소 그대로 — id·label·지문·토큰).'
              + ' **글자로 안 읽히는 창은 화면을 보고 `x`·`y` 로 짚되, 그 관찰이 준 `그림스냅샷`을'
              + ' `스냅샷`으로 함께 줘라**. 계보 없는 좌표는 실행하지 않는다.'
              + ' 토큰이 있으면 토큰이 이긴다 — 좌표는 그것이 없을 때의 길이다.'
            ...
            properties: {
              ...
              번호: { type: 'number' },
              x: { type: 'number', description: '방금 관찰한 창 그림 안의 가로 좌표' },
              y: { type: 'number', description: '방금 관찰한 창 그림 안의 세로 좌표' },
              스냅샷: { type: 'string', description: '그 좌표를 짚은 관찰의 `그림스냅샷` 신분' },
              비밀칸: { type: 'boolean' },
            },
          },
          기대: {
            ...
            properties: {
              ...
              바깥으로: {
                type: 'boolean',
                description: '메시지 전송·결제처럼 바깥으로 나가는 일이면 true.'
                  + ' **밝히면 사람에게 물어본 뒤 실행한다** — 막히는 것이 아니다.'
                  + ' 무엇이 나가는지 `기대.값` 에 적어라(카드에 그대로 보인다).',
              },
            },
          },
          // **행동마다 무엇을 넣는지 말한다.** 예전엔 *"type 일 때 넣을 글자"* 뿐이라
          // 모델이 `press_key` 에 **빈 값**을 보냈고, 아무 키도 안 눌린 채 턴이 끝났다
          // (라이브 2026-08-06). 값이 필요한 행동인데 비어 있으면 이제 실행하지 않는다.
          값: {
            description: '행동마다 넣는 것이 다르다.'
              + ' `type`/`paste` — 넣을 글자 · `press_key` — 키 이름(`return`·`escape`·`tab`·`up`).'
              + ' **메시지 보내기는 `return` 이다.**'
              + ' `hotkey` — 조합(`cmd+s`) · `menu` — 메뉴 차례(`["파일","저장"]`) ·'
              + ' `scroll` — 방향(`up`/`down`) 또는 `{방향, 양}` · `wait` — 초.'
              + ' **값이 필요한 행동에 안 주면 실행하지 않는다.**',
          },
...
const DESCRIPTORS = [
  ...
  // **셸이 앞에 선다**(2026-08-11). 배치는 선언에서 한다(`tool-schema.js`) — 여기 적는 순서가
  // 곧 모델이 보는 순서다. 예전엔 의미별 손(파일·흔적 찾기)이 셸보다 앞에 있었고, 모델은
  // 이름으로 먼저 잡히는 손을 골랐다. 그래서 「엑셀 만들어줘」가 전용 손이 없다는 이유로
  // 죽었다 — 셸에서는 되는 일이었다. 한 번만 앞으로 옮긴다(런타임 정렬이 아니다 · 불변식 A).
  ...
  defineTool({
    ...
    //
    // ── **선언이 실물보다 무섭게 적혀 있었다** (오너 대조 2026-08-14 · F-46 의 모델판) ──────
    //
    // 같은 질문(*"작업 폴더에서 제일 긴 문서가 뭐야?"*)을 두 곳에 넣은 결과:
    //   비교군(더 약한 모델) 셸 두 줄로 전부 재고 1·2·3위를 답했다
    //   T5(더 강한 모델)     `local.file` 9회 · 두 개만 재고 *"겉보기로는 짧을 가능성이 큽니다"*
    // **차이는 터미널을 썼느냐 하나다.** `wc -l` 한 줄이면 끝날 일이다.
    //
    // 원인은 이 문장이었다. 두 손을 모델 눈으로 나란히 놓으면 —
    //   터미널  *"…명령은 그냥 실행하면 된다 — 실행 직전에 **확인 카드**가 한 번 뜨고…"*
    //   파일손  *"지우거나 덮어쓴 것은 **되돌릴 수 있다**."*
    // 읽기 명령(`ls`·`wc`·`grep`)에 붙을 마찰이 하나도 없는데 **이 줄에는 그 사실이 없다.**
    // 없는 사실은 모델이 짐작으로 메운다 — 안전해 **보이는** 손을 고른다.
    // 라이브 9회차 실측: 터미널 승인 카드 0~1장 · 읽기 명령에는 한 번도 안 떴다.
    //
    // **강제를 더하지 않는다**(「터미널을 먼저 써라」는 F-88 이 막는 자리다). 고치는 것은 하나,
    // **빠진 사실을 채우고 틀린 범위를 사실로 좁힌다.** 승인은 이름 목록이 아니라 probe 결과가
    // 정한다(`tool-boundary.js:실행전판정` → `action-plan.js` `changes` → `authority.js`).
    // 그리고 「인터넷에 연결하는 명령」도 틀렸다 — 읽기성 네트워크는 오너 결정(2026-08-06)으로
    // 이미 카드에서 빠져 있다(`local-terminal.js` 의 `reach` 재시도 ·
    // `test/terminal-read-network-is-automatic.test.js`). 선언만 그 결정을 못 따라갔다.
    capability: '이 컴퓨터에서 명령을 직접 실행한다. 위치를 모르면 직접 찾는다. 확인·검사·테스트·빌드는 바로 하고,'
      ...
      + ' **이 컴퓨터를 바꾸지 않는 명령은 확인 카드 없이 바로 실행된다** —'
      + ' 목록·세기·찾기·읽기, 그리고 밖에서 읽어 오기만 하는 것까지 그렇다.'
      + ' 파일을 바꾸거나 설치하는 명령도 그냥 실행하면 된다 — 실제로 무언가를 바꾸는 명령일 때만'
      + ' 실행 직전에 확인 카드가 한 번 뜨고, 승인되면 네가 이어서 실행한다 — 사용자에게 명령어를 적어 주지 않는다.'
      + ' 어느 쪽인지는 이름으로 정하지 않는다 — 실행 전에 바꾸는 것을 막아 둔 채 한 번 돌려 본 결과가 정한다.'
      + ' 그러니 사용자에게 명령어를 보여주며 대신 치라고 하지 않는다. 비밀 자리는 승인해도 읽지 않는다.'
      // ⑧ 유도 재료(§7-bd) — capability 축이 늘 실리는 쪽이다(:668 주석). 사실+행선지, 숫자는 단일 근원.
      + ` 한 번의 실행은 ${DEFAULT_TIMEOUT_MS / 1000}초 안에 끝나는 일까지다 — 시간이 다 되면 실행을 멈추고, 그때까지 돌던 것(서버·워치)은 함께 꺼진다.`
      + ' 계속 돌아야 하는 것을 켜 두고 관리하는 손은 local.process 다.'
      // ⑥ 유도 재료(§7-bv · 오너 문안) — 정체성 델타 한 문장: 요청 자체가 허락 구하기다.
      // 산문 선허락(§7-bu R2 u7 실물)이 안 덮인 갈래 — 카드 기제 재진술이 아니라 이 델타만.
      + ' 실행 요청이 곧 허락 구하기다 — 승인이 필요하면 카드가 먼저 뜬다.',
    operatorFact: '이 컴퓨터의 상태·설정·설치된 도구를 직접 확인하고 필요한 명령을 실행한다.'
      // **무엇을 보는 손인가**(노드 ③). 화면 손과 보는 대상이 다르다.
      + ' 이 손이 보는 것은 **컴퓨터가 저장한 것**이다 — 파일·프로세스·설정.'
      + ' 싸고 빠르며 **많은 것을 한 번에** 다룬다(파일 1,000개 이름 바꾸기·로그 100만 줄).',
    ...
    schema: {
      description:
        ...
        // ── F-117 짝 — **경계 사실은 골라야 할 손 쪽에도 적는다** ─────────────────────
        //
        // capability(시스템 프롬프트)에는 "여러 파일을 한꺼번에 다루는 일도 명령 하나로
        // 끝난다"가 이미 있었는데 **도구를 고르는 순간의 스키마에는 없었다** — 또 두 축이
        // 다른 말을 한 것이다. 와이어 재생 실측(2026-08-15 · 제품 프롬프트·이력·25도구
        // 원판 그대로, 이 줄 하나만 넣고 5회): 터미널 첫 수 0/5 → **4/5**.
        // 파일 쪽(집계한계채우기)에만 적으면 안 산다(절제5 0/5) — 경계는 양쪽이 말해야 선다.
        + ' **여러 파일을 한꺼번에 세거나 줄 수를 재거나 내용을 훑는 일은 명령 하나로 끝난다**'
        + '(wc·grep·find·sort) — 파일을 하나씩 열 필요가 없다.'
        ...
        // **주어가 빠져 있어서 제한으로 읽혔다.** 옛 문장은 *"먼저 안전하게 시험해 보고 필요하면
        // 사용자에게 확인을 받는다"* 였다 — 시험도 확인도 **모델이 할 일**로 읽힌다. 모델이 확인을
        // 받으려면 명령어를 보여 주는 수밖에 없고, 그게 이 손이 처음부터 막으려던 떠넘김이다
        // (같은 병을 위 `capability` 에서 이미 한 번 고쳤다 — 여기만 남아 있었다).
        // 시험하는 것도 카드를 띄우는 것도 **커널**이고, 모델은 그냥 부르면 된다.
        ...
        // 옛 문장("오래 걸리는 서버 실행은 아직 이 도구로 하지 않는다")은 두 재판 기준선에
        // 있었는데 2/3 이 무시했다 — 기제 사실도 행선지도 없는 금지문이라서다(손 관리자 규명).
        // 실측: 모델이 서버를 여기로 띄웠고 granted 실행이 상한을 꽉 채운 뒤 SIGTERM 으로
        // 서버를 죽였다(126s · 최종답 시점 pids 0 · 완성재판 R2·R3). process 손이면 9s·생존(R1).
        // 숫자는 DEFAULT_TIMEOUT_MS 단일 근원 — 상수가 바뀌면 이 문장도 같이 바뀐다(드리프트 금지).
        + ` 한 번의 실행은 ${DEFAULT_TIMEOUT_MS / 1000}초 안에 끝나는 일까지다 — 시간이 다 되면 실행을 멈추고, 그때까지 돌던 것(서버·워치)은 함께 꺼진다.`
        + ' 계속 돌아야 하는 것을 켜 두고 관리하는 손은 local.process 다.'
        // ⑥ 유도 재료(§7-bv) — capability 사본과 같은 문장(두 사본 균일 · 단어경계 두 사본 선례).
        + ' 실행 요청이 곧 허락 구하기다 — 승인이 필요하면 카드가 먼저 뜬다.'
        // ── **미래 시각은 이 손의 일이 아니라는 사실**(A3 · 2026-08-12 · 라이브 2/5 실측) ──
        //
        // 같은 발화 5회 중 2회가 `automation.propose` 를 **아예 안 부르고** 사용자에게 zsh
        // 스크립트와 `~/Library/LaunchAgents/…plist` 를 짜 줬다(원장 jobs 0건). 오늘 이
        // 저장소가 **세 번째로** 밟는 같은 병이다 — 능력은 섰는데 모델이 그걸 모르고 옛 방식을
        // 흉내 낸다(`local.locate` 형식 세기 3/3 · 진짜 xlsx 생성 7/7). 앞의 둘 다 **설명서
        // 한 줄**로 닫혔다.
        //
        // **자리가 핵심이다**: 예약 채널 쪽이 아니라 **셸 손** 설명서에 적는다. 모델이 셸을
        // 잡으려는 바로 그 순간에 보여야 하기 때문이다. 오픈클로가 같은 자리에 심어 뒀다
        // (`$(npm root -g)/openclaw/dist/bash-tools.descriptions-B8J5snix.js:24`):
        //   *"Do not use exec sleep or delay loops for reminders or deferred follow-ups;
        //    use cron instead."*
        // 그리고 그 줄은 `params?.hasCronTool` 일 때만 실린다 — **없는 능력을 가리키지 않는다.**
        // 여기서 그 조건은 구조적으로 늘 참이다: `automation.propose` 는 턴 경로에서 무조건
        // 열리고(`server.js` `ctx.modelControls`), `modelSchemasFor` 가 그것을 거두는 유일한
        // 조건은 `hands.length === 0` 인데 이 설명이 보인다는 것 자체가 손이 있다는 뜻이다.
        //
        // **금지문을 안 쓴다**(오늘의 규율 · F-88): 「~하지 마라」가 아니라 무엇이 참인가 —
        // 예약으로 걸면 무엇이 따라오고, 셸로 흉내 내면 그 셋이 왜 없는가.
        + ' **미래 시각에 스스로 일어나는 일은 이 손의 일이 아니다.** 사용자가 시점을 말한'
        + ' 반복·예약("매일 아침 9시에…", "내일 아침에…")은 예약 채널(`automation.propose`)로'
        + ' T5 가 직접 건다 — 그러면 그 시각에 T5 가 스스로 실행하고, 결과가 이 대화로 오고,'
        + ' 안 도는 조건(앱 꺼짐·따라잡기 한도·권한창)도 함께 알려진다.'
        + ' cron·launchd·sleep 루프를 사용자에게 짜 주면 그 셋이 전부 없고, T5 는 그것이 도는지'
        + ' 안 도는지조차 모른다.',
      parameters: {
        type: 'object',
        properties: {
  ...
  defineTool({
    ...
    capability: "이 컴퓨터에서 지금 무엇이 돌고 있고 무엇이 자원을 많이 쓰는지, 그리고 저장 공간이"
      + " 얼마나 남았는지 직접 본다. 아무것도 바꾸지 않는다. 터미널이 못 보는 자리(setuid 도구)라"
      + " 별도 손으로 본다.",
    operatorFact: "컴퓨터가 느리거나 용량이 궁금할 때 무엇이 자원을 쓰는지와 남은 저장 공간을 직접 확인한다.",
    schema: {
      description: "**이 컴퓨터가 지금 어떤 상태인가**를 한 번에 본다 — 돌고 있는 것을 자원 사용"
        + " 순서로, 그리고 **남은 저장 공간**(전체·여유)까지 함께 돌려준다. 축을 고를 필요가 없다."
        + " \"컴퓨터가 느려\", \"왜 이렇게 버벅여\", \"뭐가 돌고 있어?\" 뿐 아니라"
        + " \"디스크 여유 공간 얼마나 남았어?\", \"용량 얼마 남았지?\", \"저장 공간 부족해?\""
        + " 같은 말에도 이 손을 쓴다. 읽기만 하므로 승인이 필요 없다.",
      parameters: {
        type: "object",
  ...
  defineTool({
    ...
    schema: {
      description:
        ...
        // **이 자리에 예전엔 "짧게 보여주고 고르게 한다" 가 있었다**(2026-08-07 고침).
        // 그래서 판 ⑤에서 T5 가 정산 폴더 다섯을 정확히 찾아 놓고 **목록만 보여준 뒤 멈췄다** —
        // 모델이 잘못한 게 아니라 이 문장이 시킨 대로 한 것이다. 오늘 세 번째 같은 병이고
        // (앞의 둘은 `userSafeSummary`·`nextSafeAction`), 이번엔 **손 설명서** 안에 있었다.
        // 이력은 여기 주석으로 둔다 — 설명서는 모델이 읽는 글이라 옛 문장을 인용만 해도 샌다.
        + ' 후보가 하나면 그대로 쓴다. **여럿이면 가장 그럴듯한 하나를 골라 진행하고,'
        + ' 무엇을 왜 골랐는지 말한다** — 사용자가 아니라고 하면 그때 바꾼다.'
        + ' 못 찾으면 depth 를 늘리거나 from 을 바꿔 다시 부를 수 있다. 파일을 열지는 않는다.'
        // **형식으로 부르면 세는 손이 된다**(콘솔 라이브 2026-08-12). 손은 이미 전수와
        // 자리별 분포를 내는데(`formatTotal`·`formatByPlace`) 여기에 안 적혀 있어서 모델이
        // 그 능력을 못 봤다: 「내 컴퓨터에 PDF 있어?」 세 회차 전부 이 손 대신 **캡슐을 직접 짜서**
        // Desktop·Documents·Downloads 만 훑었다. 모델이 잘못한 게 아니라 우리가 안 말한 것이다
        // (이 설명서가 같은 병으로 이미 한 번 고쳐진 자리다 — 위 2026-08-07 주석).
        + ' **"엑셀 파일"·"PDF"·"텍스트 파일"처럼 형식으로 물으면 몇 개인지와 어느 자리에'
        + ' 몇 개씩인지를 함께 준다**(OS 색인을 쓰므로 depth 에 안 갇힌다) —'
        + ' "있어?"·"몇 개야?"·"어디 있어?" 는 그 개수를 묻는 말이다.'
        ...
        + ' 사용자에게 경로를 복사해 오라고 하지 말고 **그 이름 중 하나로 from 을 바꿔 다시 부른다**'
        + '(사용자가 "외장하드요" 라고 짚어 주면 물론 그 이름을 쓴다).',
      parameters: {
        type: 'object',
        properties: {
  ...
  defineTool({
    ...
    // **웹 축**(F-107) — 자바스크립트로 그려진 화면을 본다 — web.collect 가 못 읽는 그 자리다.
    // 축이 없으면 web.collect 가 껍데기를 물고 와도 이 손이 대안으로 안 보인다.
    보는것: '웹',
    availability: [{ kind: 'connected' }], toolKind: 'read', reversible: true,
    ...
    capability: '주소를 브라우저로 열어 **실제로 그려진 화면**을 본다. 자바스크립트로 그려지거나 탭 뒤에 있는 내용도 볼 수 있고,'
      ...
      // ★ 국면 5 슬라이스 1 실측(선빨강 c0): 이 사실이 없어서 모델이 「로컬 주소는 막혀 있다」고
      // **보수적으로 오추정**해 손을 아예 안 댔다(코드는 통과시킨다 — validateWebInput 실측).
      // 로그인 거짓 약속(2026-07-27)과 같은 기제, 방향만 반대 — 능력 문장에 없는 것은 모델이 지어 채운다.
      + ' 이 컴퓨터 안 주소(127.0.0.1 같은 로컬 서버)도 열 수 있다.'
    ...
    행위: {
      open: { 말: '주소 열기' },
      snapshot: { 말: '그려진 화면 그대로 뜨기' },
    },
    // 동사가 아니라 **손 전체**에 걸리는 사실이라 `동사` 표식이 없다 — 어떤 손의 동사도
    // 말하지 않으므로 봉인(S2)이 표식을 요구하지 않는다.
    // **어느 자리에서 봤는지는 영수증이 말한다**(2026-08-11 · 프로필이 둘이 됐다).
    // 선언은 정적이고 프로필은 회차마다 다르므로, 여기서 단정하면 한쪽에서 반드시 거짓이 된다 —
    // 그래서 「기본은 이렇고, 이번 회차의 사실은 영수증에 있다」로 적는다.
    limits: [{ says: '기본은 새 브라우저 자리라 로그인 상태가 따라오지 않는다 — 이번 회차가 어느 자리였는지는 영수증의 profile 이 말한다' }],
    schema: {
      description: '주소를 브라우저로 열어 실제 화면을 본다. `web.collect` 로 읽었는데 내용이 비어 있거나'
        + ' 자바스크립트로 그려지는 화면일 때 쓴다. 본 범위와 못 본 범위를 함께 돌려준다.'
        + ' 화면을 더 보거나 누르려면(내리기·링크·버튼·탭·더보기) `browser.act` 를 쓴다.',
      parameters: {
        type: 'object',
        properties: {
  ...
  defineTool({
    ...
    // **웹 축**(F-107) — 그 화면을 넘겨 뒤에 있는 것을 꺼낸다.
    // 축이 없으면 web.collect 가 껍데기를 물고 와도 이 손이 대안으로 안 보인다.
    보는것: '웹',
    availability: [{ kind: 'connected' }], toolKind: 'read', reversible: true,
    // **칸 1 정정.** 예전 끝 문장은 *"글을 쓰거나 보내거나 사는 일은 하지 않는다"* 였다.
    // 「글을 쓴다」는 `desktop.act:type` 이 **실제로 가진** 동사인데, 능력 문장들은 프롬프트에서
    // 한 줄로 뭉쳐 실리므로 그 부정에 손 경계가 없었다 — 모델은 "칸에 글자를 넣는다"와
    // "글을 쓰지 않는다"를 한 화면에서 읽었다(성질 1 의 유력 원인). 부정은 아래 `limits` 로
    // 내리고, 여기서는 **그 일을 하는 손을 가리킨다**(부정이 아니라 이관).
    //
    // ── 이관을 **되돌렸다**(2026-08-11 · 이 슬라이스) ─────────────────────────
    // 이관 자체가 결함이었다. 실측: 네이버를 브라우저 손으로 열어 놓고 글자를 치려니
    // **화면 손(픽셀)으로 돌아갔고 승인 카드 2장**이 떴다 — 「사용자 손 0회」가 깨진 자리가
    // 정확히 여기다. 비교군 둘(OpenClaw `browser-control`, Hermes `browser_type`)은
    // 브라우저가 **스냅샷 ref 위에서** 직접 친다. 좌표가 아니다.
    // 그래서 `type`·`press` 를 이 손이 갖는다. 경계는 `browser.js` 의 판정이 진다.
    // ★ 국면 5 슬라이스 1(2026-08-17): 「탭과 더보기를 펴고」가 일반 클릭 전체를 죽이고
    // 있었다(선빨강 실측 — 모델이 가능한 탭 클릭조차 0회 · 「막혀 있다」 답 원문). 능력
    // 문장은 코드 경계와 같은 글자여야 한다 — 코드는 클릭가능술어(browser.js)가 진다.
    capability: '보고 있는 화면을 다룬다 — 아래로 내리고, 관찰이 준 자리(ref)의 링크·버튼·탭·더보기를 누르고,'
      + ' 그 칸에 글자를 치고 검색 칸에서 엔터를 친다. 누른 뒤의 화면을 다시 관찰해 함께 돌려준다.'
      + ' 몇 번 내렸는지·왜 멈췄는지·어느 브라우저 자리에서 봤는지를 함께 남긴다.',
    operatorFact: '관찰이 준 자리(ref) 위에서 글자를 치고 검색을 끝낸다 —'
      + ' 마우스 좌표 대신 화면이 이름 붙여 준 자리를 짚는다.',
    행위: {
      scroll: { 말: '아래로 내리기', 별칭: ['스크롤'] },
      click: { 말: '화면에서 누르기(링크·버튼·탭·더보기)', 별칭: ['누르', '클릭'] },
      type: { 말: '화면 칸에 글자 치기', 별칭: ['글자를 치'] },
      press: { 말: '검색 칸에서 엔터 치기', 별칭: ['엔터를 치'] },
    },
    // 한계는 **동사 표식과 함께** 구조로 적는다(칸 1 S1~S3). 산문에 부정을 흘리면 그 부정이
    // 프롬프트에서 다른 손과 한 줄로 뭉쳐 손 경계를 넘는다 — 그게 성질 1 의 구조였다.
    limits: [{ 동사: 'click', says: '폼을 제출하는 버튼과 바깥 사이트로 나가는 링크는 누르지 않는다 — 페이지 안 걸음만 누른다' },
      { 동사: 'type', says: '비밀번호 같은 보안 칸에는 글자를 치지 않는다 — 그 칸은 사람이 직접 넣는다' },
      { 동사: 'press', says: '엔터는 검색 칸에서만 친다 — 상대에게 나가는 칸에서는 손이 물러나고 확인을 받는다' }],
    schema: {
      description: '보고 있는 화면을 다룬다.'
        ...
        + ' 보안 칸·파일 칸·검색이 아닌 칸의 엔터에서는 손이 물러나고 그 사실을 그대로 돌려준다.',
      parameters: {
        type: 'object',
        properties: {
...
export function demoDescriptors(opts = {}) {
  ...
  return 집계한계채우기(고른것.map((d) => 방채우기(d, opts.rooms ?? 기본방)));
}

/** 호출부가 방을 안 알려줄 때 쓰는 말. **거짓이 아닌 가장 좁은 표현**이다. */
const 기본방 = '작업 폴더';

// ── F-117 · 파일 손 선언이 자기 한계를 말하지 않아 터미널이 죽었다 ──────────────────
//
// 맨몸 대조군(scripts/terminal-qualification/맨몸판.mjs · 2026-08-15): 커널 겹을 다 뺀
// 기본 다섯 줄 루프에 같은 격리 방·같은 모델·같은 오너 발화를 넣고 팔만 바꿨다.
//   터미널만(중립 선언)                2·2·2·2   ← 부품 무죄
//   파일 손 + 터미널(중립 선언)        0·0·0·0   ← 문구를 고쳐도 죽어 있다 — 경쟁 자체가 죽인다
//   파일 손이 아래 사실을 말함 + 중립   1·2·3     ← 전부 살아남 (한 회차는 두 손을 섞어 씀 = 유도)
// 증거: docs/03-verification/evidence/terminal-2026-08-15/맨몸대조군-*.json
//
// 파일 손은 "홈 전체에서 읽는다"(참)만 말하고 **집계의 비용은 말하지 않았다.** 없는 사실은
// 모델이 짐작으로 메운다 — 안전해 보이는 손으로 하나씩 열다가 두 개에서 멈춘다(오너 실측 §0).
// **강제가 아니라 유도다**: 두 손의 실제 비용 구조라는 사실 하나를 채운다. 고르는 것은 모델이다.
// 범위를 좁히지 않는다 — "홈 전체 읽기"를 좁히면 F-46 계열(좁은 선언을 모델이 믿음)이 다시 열린다.
//
// 터미널 손이 **이 설치에 실제로 선언될 때만** 얹는다 — 없는 능력을 가리키지 않는다
// (:741 hasCronTool 전례 · 손 관리자 조건 ② 2026-08-15). 문구는 측정된 원문 그대로다.
const 집계사실 = ' 여러 파일을 한꺼번에 세거나 줄 수를 재거나 내용을 훑는 일은 이 손으로 하나씩 하면'
  + ' 오래 걸린다 — 그런 일은 터미널이 명령 하나로 끝낸다.';
function 집계한계채우기(list) {
  if (!list.some((d) => d.id === 'local.terminal')) return list;
  return list.map((d) => (d.id === 'local.file'
    ? {
      ...d,
      capability: d.capability + 집계사실,
      ...(d.schema ? { schema: { ...d.schema, description: d.schema.description + 집계사실 } } : {}),
    }
    : d));
}
