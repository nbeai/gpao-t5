...
const DESCRIPTORS = [
  ...
  defineTool({
    ...
    // 2026-08-18 P0 실측은 이 결정을 뒤집었다. `reach` 성공은 읽기 증명이 아니고 실제 POST도
    // 승인 전에 통과했다. 임의 셸 네트워크는 카드 뒤에서만 열고, 공개 웹 읽기는 web 손이 맡는다.
    capability: '이 컴퓨터에서 명령을 직접 실행한다. 위치를 모르면 직접 찾는다. 확인·검사·테스트·빌드는 바로 하고,'
      ...
      + ' **이 컴퓨터 안에서 바꾸지 않는 명령은 확인 카드 없이 바로 실행된다** — 목록·세기·찾기·읽기가 그렇다.'
      + ' 바깥 네트워크에 닿는 셸 명령은 읽기와 전송을 실행 전에 증명할 수 없어 정확한 명령을 보여 주고 한 번 확인받는다.'
