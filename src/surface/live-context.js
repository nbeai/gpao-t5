 ...
 */
/**
 * **로그인된 브라우저 관찰 — 기본 켬**(오너 기결정 2026-08-07 의 집행 재개 · PM 판정 2026-08-09).
 *
 * 이 자리는 두 번 뒤집혔다. 그 계보를 남긴다 — 무엇이 막고 있었고 왜 풀렸는지가 근거다.
 *
 *   막던 것   우리가 크롬 동의 시트를 **직접 누르는 우회 갈래**가 있었다. 봉인
 *             ("밝힐 때만 켠다")이 지킨 것은 플래그 값이 아니라 **그 갈래가 기본으로 열리는 것**
 *             이었다. 그래서 첫 시도(갈래를 둔 채 값만 뒤집기)는 정확히 물렸고 되돌렸다.
 *   푼 것     ① 갈래 제거(도달 불가 코드 — 프로브 실측) + **부재 봉인** 둘
 *             ② `--grant existing-profile` 데몬 장착 + **사람의 동의**로 붙기 성립 —
 *                탭 6개 실측(evidence/step6-compare-2026-08-09/M1-재측정-f54).
 *                그 성공은 **우리 클릭 0** 이었다 — 승인 UI 는 사람의 것이다.
 *   남은 것   다른 설치의 **첫 동의 UX**(누가 언제 그 한 번을 누르는가)는 장부행 —
 *             「첫 10분」(패키지 단계)과 묶는다. 이 기계의 성공은 "오너가 이미 눌렀다" 위에 있다.
 *
 * **끄는 길은 남긴다**(`GPAO_T5_BROWSER_PROFILE=0`). 기본값만 뒤집는 것이지 선택 제거가 아니다.
 * 셋이 한 세트로 봉인된다(PM 조건 4): **기본 켬 · 발화 조건(`브라우저이야기인가`) · 관찰 전용
 * 울타리.** 켬만 봉인하고 울타리를 안 물면 반쪽이다.
 */
export const 브라우저프로필허용 = (env = {}) => env?.GPAO_T5_BROWSER_PROFILE !== '0';
