...
export function liveDeps(processEnv = {}, deps = {}) {
  ...
  // P6-L2: 사용자가 연 폴더 기록. 세션 저장소와 같은 자리에 둔다(설정이 아니라 **사용자의 결정**이다).
  const rootsStore = deps.rootsStore ?? new LocalRootsStore(
    processEnv.GPAO_T5_DATA_DIR ?? new URL('.', import.meta.url).pathname,
  );
