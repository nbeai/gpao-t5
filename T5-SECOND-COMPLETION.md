...
- 정리 전 설치본 기능 기준은 source `f74711de`, annotated tag
  `t5-0.3.1-pre-clean-baseline`이다. 정리 후 기준 tag는 `t5-0.3.1-clean-baseline`이다.
...
- 정리 전 기준은 `refoundation/evidence/product-cleanroom-baseline-2026-08-28.json`, 완료 결과는
  `refoundation/evidence/product-cleanroom-closeout-2026-08-28.json`이 담당한다.
- Prompt·제품 의미·UI 디자인을 바꾸지 않고 caller 없는 UI·서버 432줄을 제거했다. 4차 휴면 18개와
  qualification-only 29개는 source 증거로 보존하되 설치 payload에서 제외했다. 전체 제품 CI는 통과했다.
- Cleanroom은 종료됐다. 다음 개발 Gate는 오너가 이 clean baseline에서 별도로 연다.
