...
- 작성: 2026-07-29
- 지위: `current_automation_core_implementation_plan`
- 적용 시점: P-OP-7 완료 후 현재 병렬 개발 대상. 정확한 브랜치·소유권·통합 순서는 인수인계 §0을 따른다.
...
→ 명시 지시 범위는 바로 영속 Job으로 만들고 결과·중지·되돌리기 제공
→ 추정 생성 또는 외부 전송·삭제·결제·새 권한만 필요한 결정을 확인
→ 영속 Job
→ 시점 도달
→ 격리 AgentRun
...
P-OP-7은 완료됐다. 자동화 제품 코드는 T-cell 정본 구현선과 파일 소유권을 겹치지 않는 별도 worktree에서
병렬 개발하고, 공통 계약 단위로 Codex 통합 감사 뒤 편입한다.
...
→ 인간 자동화 시나리오와 복구 검증
→ Codex 통합 감사
→ Automation Core Closure 편입
```

P-OP의 승인·실행·전달·복구 바닥은 완료된 기반으로 재사용한다. 자동화가 별도 승인 체계를 다시 만들지 않는다.
...
2. 모델이 추정한 생성·수정은 proposal이다. 사용자의 명시 요청과 안전 범위의 가역 수정은 바로 적용하고
   이전 버전을 보존한다.
3. proposal은 대상 버전/hash에 묶인다.
4. inferred proposal은 replay와 사용자 확인 뒤 active가 된다. explicit 저위험 요청은 같은 내용을
   다시 확인하지 않고 active가 되며 즉시 pause/rollback할 수 있다.
...
2. 대상·문면·범위가 명시 지시 또는 bounded grant와 달라지고 외부 효과가 생기면 멈추고 확인한다.
3. A3는 무인 실행하지 않는다.
4. A2 반복 실행은 대상·행동·범위·만료가 정확히 고정된 경우만 허용한다.
5. 외부 전달은 `deliveryPolicy`와 현재 승인 경계를 다시 통과한다.
6. “스킬이 존재함”은 외부 효과 권한이 아니다. 다만 읽기·정리·초안 같은 A0/A1 실행은 별도 승인 없이
   자동 수행한다.
...
### AC-0. 병렬 개발 경계와 기준선 확인

목표:

- T-cell 구현선과 파일 소유권 비중첩
- 공통 Authority·Ledger·Recovery 계약 재사용 확인
- 자동화 기준선과 인간 시나리오 기록

종료 조건:

- 현재 통합을 무효화하는 차단 결함 0
- Automation Closure 변경의 기준선·소유 파일·영향 범위 명시
...
2. 명시된 daily trigger, 정리 skill, 기본 agent를 자동으로 구성한다.
3. “매일 오전 9시 · 읽기/정리만 · 여기 표시 · 외부 전송 없음”을 짧게 알리고 중지/수정 경로를 준다.
4. 같은 내용을 승인 카드로 다시 묻지 않는다.
5. 앱 재시작에도 job이 유지된다.
6. 9시에 run 정확히 1회.
...
- 읽기·계산 skill replay
- 명시 범위의 가역 파일 생성은 자동 수행하고 결과·되돌리기 제공
- 외부 전송은 run 성공과 별개
- 승인 없는 발송 0
