...
- inferred 저위험 preference와 작업 방식은 replay·범위 검증 뒤 A0/A1 제한 영향으로 조용히 입장할 수
  있고 pause/rollback 가능해야 한다. 민감 정보·정체성·권한·외부 대상·project/profile/global 확장을
  추정해 새 장기 운영 모델로 만드는 경우만 자동 영향을 금지한다. 확인이 필요해도 학습 시점에 카드를
  띄우지 않고 실제 영향 경계에서 최소로 묻는다.
...
- 새 승인·확인 질문·검열·사용자 개입은 반드시 비가역 외부 효과, 새 권한, 확정되지 않은 대상 중
  하나의 실제 안전 경계와 연결되어야 한다. 연결되지 않으면 기능 추가가 아니라 마찰 회귀다.
- 검증된 외부 구현의 코드·실패 방어·검사를 직접 확인하고, T5 계약에 맞는 부분은 다시 발명하지 않고
  흡수한다. T5의 차별성은 검증된 기반을 버리는 데 있지 않고, 그 위에서 더 적은 개입으로 더 나은
  현실적 결과를 내는 데 있다.
...
따라서 정확도·안전 회귀가 없는데도 승인율, 불필요한 확인률, 완료 턴 수, 클릭 수, 전경 대기 시간이
증가하면 T-cell 개선으로 판정하지 않는다.
