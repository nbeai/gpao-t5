...
2880    목적미달이어가기() → true 면 continue · false 면 break (2026-08-17 재실측 — 정의 2880 ·
        호출부 3138·3161·3173·3455 · 재는 법: grep -n '목적미달이어가기' src/kernel/turn.js)
...
| ⑧ | 2318 | 전량 | 실행 뒤 최종답 |
| ⑨ | 2928 | 전량 | **목적미달 되부름** (2026-08-17 재실측 · 재는 법: grep -n 'const 되부름 = await ctx.model.respond' src/kernel/turn.js) |
...
| `browser.act` | browser-tool.js | scroll click type press (4) | 선언 `read` ⚠ · **실제 효과를 재는 자리가 없다**(클릭 뒤 무엇이 나갔는지 관측 0 — §12 CH4) | desc **331자** + capability 152 (재는 법 위와 같음) |
...
`compactResult`(task-context.js:111 — 2026-08-17 재실측 · 재는 법: `grep -n 'export function compactResult' src/kernel/l1-intent/task-context.js`)가 **갈래별로 접는다**: 브라우저 관찰 / 화면 요소(**예산 6배**) / 웹 수집 / bulk 이동 / 폴더 목록 / 파일 본문 / 그 밖(JSON + 잘림 표식).
bulk 이동 갈래는 `skipped` 를 사유별로 세어 접는다 — `protected` 도 이 자리로 모델에게 간다. **남은자리말도 같은 갈래다**(:312→:77 — 남은 파일 수·확장자 분포·`a64412a8`부터 남은 폴더 수까지 모델에게 간다. 이 줄이 비어 있어서 §7-cd 측정 전까지 「원인이 배아 밖」을 지도에서 못 짚었다).
...
미실림 · J16 등재), 1차 사실문의 참값 실림은 단언으로 굳음(`test/wrong-value-claim-is-caught.test.js`).
**일곱째 얼굴(2026-08-17 · 계열 중 제품 사실 공급층 첫 얼굴)**: task-context 남은자리말이
folders 를 구조적으로 침묵 — 「남은 파일: 1개(.gz)」가 「다 봤다」로 읽혀 거짓 완료 옆에 섰다
(J15 소유 · §7-cd 측정 → **닫힘** `a64412a8` · §7-ce · `test/leftover-facts-reach-model.test.js`.
...
| CH4 | **안전 · ★ 오너 결재 완료(2026-08-17 · 보좌관 등재 · 원문 「ch4는 승인한다」 · 정본 칸 = 부채대장-오너결재.md 머리)** — 결재된 것은 **(c) 변이 관측 + 그 사이 (a) 정직 고지**, **(b) 클릭 write 승격은 배제**(자동성 원칙 정면 위반). ⚠️ 결재가 덮지 않는 것: 승인 관문 정책 변경 · submit/외부 링크 개방 · 관측을 승인 조건으로 승격(별건 재상신) | `browser.act click` 은 **`read` 선언인데 상태를 바꾸는 POST 를 낸다.** `toolKind:'read'`(demo-context.js:1168) → §5-1 A0 → 항상 자동. 폼 밖 `type=button` + `onclick fetch POST` 는 술어가 기계로 못 가른다(공지 펼치기도 같은 fetch다) → **승인 카드 0 으로 `POST /삭제` 1건이 실제로 나갔다**(사용자가 시킨 클릭). ★ **무는 것은 T5 고유 축 「탐침 기반 권한 — 이름·패턴이 아니라 실제 효과로 판정」**(오픈북 §4)이다: 실제 효과를 재는 자리가 없는 채로 자동이다. 흐름 축 공통 기제 셋째 줄(「했으면 무엇이 변했는지가 결과에 실린다」)도 여기서 빈다. 선택지 셋(현상 유지+정직 등재 / write 승격 / 변이 관측 장치 — CDP Network 도메인만 켜면 된다, `browser.js:461-462` 는 Page·Runtime 뿐)은 **오너 결정** — 구현자·감시자가 안 고른다. ⛔ CU·`desktop.*` 비교는 근거에서 뺐다(오너 절대 명령 8c1e9de4 · 준비 정본 §1-0 금지 셋 ③ — 옛 판의 「desktop 은 probe 로 받는다」 논거를 이 정정으로 무효화) | 원본 `docs/03-verification/evidence/terminal-2026-08-17/국면5-b-s1-반대시험/회차.json` r2-JS변이(`클릭시도 1 · blocked:false · POST델타 {"/삭제":1} · 승인 0`) · 자리 `src/runtime/browser.js:263`(술어) · `src/surface/demo-context.js:1168` |
...
| J15 | **정직** | 「정리」 결과 미수령 — 2판 **4모양** 병렬 · **공통 원인 측정 완료**(2026-08-17 · 아래 · 이 행 머리가 「미측정」인 채 뒤에서 뒤집혀 있던 자기모순을 해소 — 지도 관리자 실측): 백업 안 하위폴더 잔존 2 · 완료 과대 선언 1 · 역방향 과소 보고 1 · **도달 결손 1(2차 R3 u8 — 2026-08-17 정정: 근거 줄은 네 이름을 드는데 이 행이 셋만 적고 있었다)**. 넷 중 역방향 1은 **답 층** 그물이 섰다(`40cacee9` · J14 (나) · 결정적 서버판) — J15 는 **실물 층**(정리 결과 미수령)이고 **공통 원인 측정 완료**(2026-08-17 §7-cd · 결정적 서버판 재현 + 계측기 양성 대조): 잔존 2·과대 선언 1의 원인 자리는 배아(turn.js 목적미달 — 갈래 6 전부 「걸음 성공·주장-걸음 대조」만 읽음)가 아니라 **사실 공급층**(**수리 전**: task-context.js:77-84 남은자리말이 folders 구조적 침묵 · :606-619 vef 가 옮기기 전 lastList 무조건 우선 — 낡은 값·모순 실림). 역방향 1은 실물 층 미달 표본 아님(디스크후 실측 목적 도달 — §7-cd ⑤) · 도달 결손 1은 그 자로 못 봄(B3·B7 계열). 수리 자리 이동은 오너 결재 완료(A안 · 2026-08-17) → **그 표본 2의 원인 자리인 사실 공급 결함 두 기제는 닫힘**(`a64412a8` · §7-ce — 남은 폴더 병기 · vef 원장 인덱스 우선 · 검사 `test/leftover-facts-reach-model.test.js`). ⚠️ **실물 층 완주(실모델이 그 사실로 재수행해 잔존이 실제로 사라지는가)는 라이브 미측정** — 결정적 서버판 갈음 **3연속**(A1·A2·이번) · **판정 자리는 터미널 재도전 재판이다 — B 재판정은 그 재판의 실측과 함께 돌아온다**(오너 판정 2026-08-17 · §7-cg-2 — 라이브 소검증 3표본에서 거짓완료 0 · B 개봉 근거 미발동 · 표본 확대도 안 함) · **열림** | 근거 `docs/03-verification/evidence/terminal-2026-08-15/기준선과-남은자리.md:1932-1934` + §7-cd·§7-ce·§7-cf(꼬리 — grep -n '7-c[def]' 로 재라) |
...
현실 공급 층           작업셋·화면·연결·잘린 범위·치운 뒤 남은 것(파일·폴더)   없음         지킨다
