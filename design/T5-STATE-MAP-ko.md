...
**막히는 조건 22종** — 대표: 크기 초과 · 문서 추출 실패 · `move` 목적지 존재 · `bulk_move` 조건 0개/통과 0개 · 보호 영역 · ScopeError · ENOENT(→ `다음수단: local.locate`).
...
**검사** 397파일 3,688건 · 평탄 구조 · 임시 방을 194파일이 각자 만든다 · 가짜 모델을 117파일이 각자 정의(공용 모듈 없음) · `demo-context` 를 **156파일**이 import.
...
| id | 위험 | 무엇 | 자리 |
|---|---|---|---|
| S1 | **안전** | `bulk_move` 만 개별 파일 보호 판정을 안 한다 — 점 없는 비밀 이름(`id_rsa`·`*.pem`·`credentials`·`*token*`) 이 카드 없이 옮겨진다 | local-file.js:995-1051 |
| S2 | **안전** | 비-macOS 에서 터미널이 샌드박스 없이 돈다 — probe 조차 생 셸. 아무것도 안 막히니 `read`→자동 | terminal-run.js:43 |
...
| F3 | **마찰** | 비밀 이름 정규식이 일반 자료도 잡는다(`token-정산.xlsx` 읽기 차단, 사유 미고지) | file-scope.js |
...
npm test                          # 3,744건 · 되돌아가지 않았다는 최소 조건
