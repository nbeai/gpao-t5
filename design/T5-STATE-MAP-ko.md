...
> 기준 커밋 — **베끼지 말고 재라**(§4-c ②): `git log -1 --format=%h -- design/T5-STATE-MAP-ko.md`.
> (이 줄을 쓴 시점 직전 값 `11bf7d46` · 2026-08-17. 앞서 `a64412a8` 로 적혀 있었는데 그 뒤
> J15 를 고친 커밋이 머리를 안 따라 올려 거짓이 됐다 — 지도 관리자 실측 2026-08-17. 그래서
> 숫자 대신 재는 법을 박는다.) 그 이후의 움직임은 세션 시작 점검이 잰다(§4-c ①). 그 이후의 움직임은 세션 시작 점검이 잰다(design/T5-FINAL-ASSEMBLY-ko.md §4-c ①).
