...
> 기준 커밋 16fd0e82 (2026-08-17) — 이 문서를 마지막으로 손질한 커밋. 그 이후의 움직임은 세션 시작 점검이 잰다(design/T5-FINAL-ASSEMBLY-ko.md §4-c ①).
...
규모      src 189파일 · 커널 82(20,468줄) · 런타임 64(19,116) · 표면 43(11,971) = 51,555줄
          ...
          검사 495파일(`ls test/*.test.js | wc -l`) · 건수는 `npm test` 실행값만이 잰다
          (마지막 단독 실측 4,323/4,323 — 2026-08-17 `npm test` 실행 · 도장 16fd0e82) · 스크립트는 `ls scripts`
...
술어 한 벌(:481)을 두 문(`완료주장검증` :719 · `절대재검증` :519)이 소비 · 검사
...
exit-verification.js   위 14조항(§6-3 열거가 정본 — 이 줄의 옛 「11」은 §6-3 이 13이던 때도 어긋나 있었다 · 2026-08-17 정정)
```
...
**검사** 495파일(`ls test/*.test.js | wc -l` · 2026-08-17 실측) · 건수는 `npm test` 가 잰다(마지막
단독 실측 4,323/4,323 — 2026-08-17 실행 · 도장 16fd0e82) · 평탄 구조 · 임시 방을 194파일이 각자 만든다 · 가짜 모델을 117파일이 각자 정의(공용 모듈 없음) · `demo-context` 를 **156파일**이 import.
...
미측정 · §7-cc-1 한계 ④)이 그 자리의 여섯째 얼굴로 등재됨(2026-08-17 · 지도 관리자 판정). 손 인벤토리 · **서버 실기동 한 턴으로 모델 노출 도구 캡처** · 통제 채널 · 기관 10 결손 · 부재 확인 7주제 · 확정 계열 4 · 캐시 접두 안정성 · 코드 파생 사실 · **미측정은 계획서 §2 를 인용**(값을 지어내지 않는다) · 체감 지표 사후 집계.
...
②  완료 정의 넷        work-contract(커널) / completion-contract(HTTP만) /
                     working-state(recentOutcome) / exit-verification(14조항 — §6-3 열거가 정본)
...
원장 종결             **함수 층 6/7**(7/7 아니다 — 08-12 세 문서 통일 때 이 줄만 안 딸려 왔다).
                      ...
                      만든 사실 위의 거짓 실패는 **안 문다** — `exit-verification.js:701·703`
                      (옛 인용 :466 은 2026-08-17 커밋이 그 문장을 고쳐 써 자리가 이동 · 재실측).
...
npm test                          # 건수는 실행이 잰다(마지막 단독 실측 4,320/4,320 · 2026-08-17) · 되돌아가지 않았다는 최소 조건
