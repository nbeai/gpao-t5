...
- 제품 코드와 gate 기준선은 변경하지 않음
- 수정 전 동시성 결함은 30/30 결정적으로 재현됐으므로, distinct/same-key 계약을 한 fixture에서 함께 검증하는 6회 반복으로 축소
- 두 방향 모두 매 회차 실제 `Promise.allSettled` 동시 시작과 원장 상태를 확인
- 집중 검사 계약 31건은 그대로 유지, AC-1 벽시계 약 1.34초 → 0.56초
- 공식 gate 성공 측정: CPU 36.8초, 38.0초, 38.3초. 마지막 두 회는 연속 PASS
- 첫 성공과 마지막 연속 측정 사이 전역 검사 1회가 실패했으나 즉시 전체 1,209건 재실행과 이후 gate 2회에서 재현되지 않음

## 자체 검증

- AC-1 집중 반대시험: 31건 통과
- 전체 회귀: 1,209건 통과, 실패 0
- 프로젝트 gate: 연속 PASS, CPU 38.0초·38.3초 / 40초, 벽시계 14.3초·14.5초
