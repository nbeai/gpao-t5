...
이번 P0-1은 새로운 종류의 어려움이 아니다. 직전 v2 중단에서 이미
“H05는 session ID가 없거나 `--resume` 성공을 증명하지 못하면 즉시 실패”를 종료 조건으로
고정했다. 그런데 새 preflight는 ready 프롬프트를 resume 성공의 대용물로 사용했고, 다시
**실제 사실 대신 옆의 값으로 통과를 만들었다.**

검증기가 자기 입력 불리언을 실제 재시작 증거로 읽는 P1-3도 같은 재발 유형이다. 직전 두 번의
중단에서 “명령줄·실행표가 성립하는 것과 제품 동작이 성립하는 것은 다르다”가 이미 확인됐다.
따라서 이 둘은 하지 않아도 됐던 재발이며, 오너 대기·감사 재실행·API 준비 지연을 다시 만들었다.
