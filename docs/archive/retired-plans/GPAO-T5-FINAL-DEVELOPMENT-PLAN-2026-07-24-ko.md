...
- 현재 지위: 최초 개발 계획의 역사 기록이다. 현재 최상위 정본이나 구현 명령으로 사용하지 않는다.
- 승계 정본: `docs/03-product-plan/GPAO-T5-VISION-AND-PERFORMANCE-PHILOSOPHY-2026-07-27-ko.md`
...
13. 코드 단위가 아니라 **사용자의 실제 목적 달성 경로**에서 검수했는가.
    예: "파일 이동"은 함수가 통과했는지가 아니라, 일반 사용자가 기존 파일을 잃지 않고 원하는 위치에
    정리됐는지, 막히면 무엇을 해야 하는지 이해했는지까지 본다.
14. T5가 무엇인지 잊지 않았는가.
    T5는 개발자용 콘솔이나 기능 많은 챗앱이 아니라, 일반인이 말로 일을 끝내는 AI-native OS다.
    각 기능은 "사용자를 덜 헤매게 하는가 / 실제로 목적에 도달하게 하는가 / 실패해도 안전하게 복구되는가"로
    검수한다.
