...
아래 원문은 v3.0의 모든 단계보다 우선하는 성능 기준이다. 요약·재작성·정형 문구로 대체하지 않는다.
각 Phase의 완료 여부는 기능 수가 아니라 이 원문에 비추어 판정한다.
...
### 0.0-A 절대 역량 명제 — T5도 할 수 있고, 해도 된다

아래 명제는 v3.0 이후 모든 기능 판단보다 우선한다.

> 코덱스, 클로드코드, 오픈클로, 헤르메스, 그리고 기타 다른 운영체제가 할 수 있는건 당연히 티파이브도 할 수 있고, 해도 된다.
> 그 어떤 핑계로도 이런 개발을 막지마라.

이 말은 OpenClaw/Hermes 기능 수를 무작정 복제하라는 뜻이 아니다. 다른 AI 운영체제와 agent runtime이 이미
사용자 목적 수행에 필요하다고 증명한 손발을, T5 개발에서 "위험하다", "범위가 크다", "사용자가 직접 하면 된다",
"일단 좁게 하자"는 이유로 제거하거나 지연하지 말라는 뜻이다.

T5는 Codex와 Claude Code가 개발 작업에서 쓰는 파일, 터미널, 프로세스, 브라우저, 앱, 외부 도구, 검증 루프를
일반 사용자의 PC와 업무 환경으로 확장하는 AI OS다. 안전은 기능 축소가 아니라 위험 실행 경계, 승인, 원장,
보호 영역, 마스킹, 복구 경로로 구현한다.
...
이 순서를 해치거나, 런타임이 사용자 말을 기계적으로 닫아 모델의 해석·판단 기회를 빼앗는 기능은
v3.0의 성공이 아니라 범위 이탈이다.

### 0.0-B 가능화 원칙 — 된다/안된다보다 부드럽고 안정적인 실행 경로

v3.0 이후의 개발 판단은 "이건 된다/안된다"에서 멈추지 않는다. 먼저 찾아야 할 것은 사용자가 원하는 일을
T5가 어떻게 더 부드럽고 안정적으로 할 수 있게 만들 것인가다.

위험하거나 복잡한 기능을 제거하는 것은 안전 설계가 아니다. T5의 안전은 능력 축소가 아니라 실행 순간의
경계, 승인, Truth Ledger, 보호 영역, 마스킹, 복구 경로, 라이브 검증으로 구현한다.
...
P2의 말귀 개선은 이 원칙의 실측 근거다. 입력 품질·현재 맥락·실행 사실·selfState를 바로잡자 말투, 도구 선택,
검색 정직성, 대화 흐름이 함께 좋아졌다. 이후 터미널, 프로세스, 브라우저, 채널, 외부 커넥터도 같은 방식으로
설계한다.
...
Hermes의 학습·회수 장점을 이미 있는 T5 구조로 체감되게 만든다.

P7은 단순히 "자주 맡기는 일을 줄여준다"가 아니다. T5가 현재 대화의 최근 맥락만 보는 상태에서 벗어나,
지난 세션, 읽은 자료, 실행 원장, 사용자 자료에서 **관련 근거를 다시 찾아 현재 요청에 맞게 회수하는 능력**을
닫는 단계다.

P2가 현재 턴의 말귀와 흐름을 바로잡았다면, P7은 그 성과를 오래된 맥락과 사용자 자료로 확장한다.
OpenClaw/Hermes가 강한 깊이와 회수력을 T5식으로 흡수하되, T5의 원칙은 유지한다:

```text
많이 기억하는 것이 목표가 아니다.
필요한 근거를 정확히 찾아, 현재 맥락에 맞게, 지어내지 않고 쓰는 것이 목표다.
```
...
ToolReceipt / Delivery Ledger
workingState / contextShown
```
...
내 자료 기준으로 봐줘.
지난번 그 이야기 이어서 해줘.
예전에 읽은 페이지 기준으로 다시 정리해줘.
지난 대화에서 관련된 내용 찾아줘.
그때 실행했던 결과 기준으로 판단해줘.
같은 일을 반복한다.
T5가 반복 패턴을 후보로 제안한다.
사용자가 승인한다.
다음 같은 요청에서 질문이 줄어든다.
...
## P7-1 깊이와 회수력 — 의미 검색 / 내 자료 기준 답변

목표:

```text
T5가 지난 세션·읽은 자료·실행 원장·사용자 자료에서 현재 요청과 관련된 근거를 찾아 회수한다.
문자열이 똑같이 들어 있는 것만 찾는 수준을 넘어서, 의미적으로 관련된 후보를 찾아 현재 대화에 맞게 쓴다.
```

완료 기준:

```text
"내 자료 기준으로 봐줘"에 실제 자료/세션/원장 근거를 찾아 답한다.
"지난번 그거"가 최근 턴을 넘어 오래된 세션에서도 회수된다.
단순 문자열 포함 검색보다 나은 의미적 후보 선택이 있다.
찾은 것과 못 찾은 것을 구분한다.
오래된 근거와 현재 맥락을 섞지 않는다.
모델이 기억을 지어내지 않고, 회수한 근거만 사용한다.
회수한 근거는 출처·시각·자료 종류를 함께 남긴다.
대형 벡터 DB 없이 먼저 T5식 경량 구조로 시작한다.
```

구현 원칙:

```text
receipt/ledger 가 진실이고, memory/search 결과는 그 파생 뷰다.
현재 workingState 를 덮어쓰지 말고, 관련 근거 후보로 제시한다.
모델에게 "기억하라"는 규칙을 더 넣지 않는다. 모델 앞에 실제 회수 근거를 준다.
검색됨 ≠ 사실임 ≠ 현재 맥락에 반영됨. 세 단계를 구분한다.
사용자 자료를 외부 임베딩 서비스에 조용히 보내지 않는다.
```

## 완료 기준

```text
P7-1 깊이와 회수력 시나리오 통과
