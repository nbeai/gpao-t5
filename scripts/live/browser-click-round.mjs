...
<div role="tablist">
  <button role="tab" aria-selected="true" onclick="스위치('소개')">소개</button>
  <button role="tab" aria-selected="false" onclick="스위치('후기')">후기</button>
</div>
<div id="탭-소개">소개 탭 내용: 10년 된 가게이고 골목 안쪽에 있다.</div>
<div id="탭-후기" style="display:none">${심은사실.탭후기}</div>
<p><a href="/자세히">자세히 보기</a></p>
<p><button type="button" onclick="document.getElementById('공지').style.display='block'">공지 펼치기</button></p>
<div id="공지" style="display:none">${심은사실.버튼공지}</div>
<p><a href="https://example.com/소식">바깥 소식(외부)</a></p>
<form method="post" action="/신청"><button type="submit">신청하기</button></form>
<script>
function 스위치(이름){
  for (const id of ['소개','후기']) document.getElementById('탭-'+id).style.display = (id===이름)?'block':'none';
}
</script>
</body></html>`;
  const 자세히 = `<!doctype html><html><head><meta charset="utf-8"><title>자세한 이야기</title></head><body>
<h1>자세한 이야기</h1><p>${심은사실.링크너머}</p><p><a href="/가게">돌아가기</a></p></body></html>`;
  return { '/가게': 가게, '/자세히': 자세히 };
}
...
export const 요구들 = (base) => [
  {
    id: 'c0-탭', 말: `${base}/가게 열어서 "후기" 탭 눌러서 후기가 어떤지 알려줘`,
...
export function 자양성대조(base) {
  const 판 = [
    ...
    ['c0 이 후기 사실을 잡는가', 'c0-탭', '후기 탭에는 별 네 개 반, 재방문 예정 평이 많대요.', true],
  ];
  const 줄 = []; let 어긋남 = 0;
