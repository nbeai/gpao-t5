...
// ── ④ (퇴역 2026-08-14) "후속/TODO" 표현 상한 ─────────────────────────────
// 여기엔 `src/**/*.js` 의 **주석 줄만** `후속|TODO|FIXME|아직 (안|없|구현)` 로 세어 기준선(12)보다
// 늘면 BLOCK 하는 검사가 있었다. 걷어낸 이유는 **한 번도 미룬 일을 잡은 적이 없기 때문**이다.
//
// 저장소 전수 심사(2026-08-14)에서 이 검사가 막은 사고를 못 찾았다. 걸린 기록 셋은 전부 낱말이었다:
//   cff99738(08-05) "우리가 아직 안 연 곳이다"           → 미룬 일이 아니었다. 문구만 고쳐 풀었다(F-26 등재)
//   6d9d564d(08-13) "아직 안 그려진 자리" → "그려지지 않은"  **코드 무변경**
//   d5b78685(08-13) "같은 걸음의 후속 성공" → "나중에 확인한" **코드 무변경**
// 이 자리의 옛 주석이 스스로 자백해 뒀다 — "이 검사의 뜻은 '미룬 일이 늘었는가'이지 단어 사냥이 아니다."
// 뜻은 계약인데 재는 것은 낱말이라 계약과 계측이 갈라져 있었다(F-26 도 같은 말을 적었지만
// "진짜 유예도 잡아 왔다"는 주장의 사례는 대지 못했다).
//
// 게다가 기준선 23→21→20→15→14→12 는 전부 **다른 작업의 부수 효과**였고, 게이트가 마지막에
// 스스로 덮어썼다. 자기가 자기 선을 내려 적으면 "한 번도 못 잡았다"는 사실이 구조적으로 안 보인다.
//
// 오너 판단(2026-08-14): 라이브와 무관한 형식 규칙은 버린다. **유일한 자는 실제 콘솔 라이브 결과다.**
// 되살릴 거라면 낱말이 아니라 "사용자가 부딪히는 자리에 미완이 늘었는가"를 재는 자로 다시 짜야 한다.
...
// **자동 갱신은 `serviceNameLeaks` 하나뿐이다**(줄어들 때만 따라 내려간다). 성능 기준선(CPU·벽시계)은
// 여기서 안 쓴다 — 스스로 올리는 기준선은 기준선이 아니다. 넘겼으면 코드를 고치거나, 오너 결정으로
// 이 파일을 손으로 옮긴다. `deferred` 는 항목과 함께 2026-08-14 에 퇴역했다(위 ④ 자리 참고).
{
  let prev = null;
  try { prev = JSON.parse(await readFile(baselineFile, 'utf8')); } catch { /* 최초 */ }
  const 다음 = { ...prev, ...(누수기준선 === null ? {} : { serviceNameLeaks: 누수기준선 }) };
  if (prev?.serviceNameLeaks !== 다음.serviceNameLeaks) {
