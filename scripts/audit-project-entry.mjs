...
// **경고는 실패가 아니지만 사라지지도 않는다.** 아래 「잊힘과 작업 중」 절 참고 — 줄을 없애면 이 수리가 곧 C5 다.
const 경고들 = [];
const warn = (message) => 경고들.push(message);
...
//     └ 2026-08-14(F-101 보탬 2) 에 이 줄이 한 번 더 갈렸다 — 바로 아래를 읽어라
//   · 작업 방이 아닌 자리에 열린 방은 **여전히 실패**한다(그물이 안 넓어진다)
// 바뀌는 것은 **깨끗한 작업 방 하나뿐**이다.
//
// ── **「잊힘」과 「작업 중」은 다른 것이다** (F-101 보탬 2 · 2026-08-14) ─────────────────
//
// 위 판은 그 둘을 **같은 이름으로 불렀다**: *본선에 안 들어간 커밋이 하나라도 있으면 실패*.
// 그건 「잊힘」의 정의가 아니라 **「작업 중」의 정의**다. 오너가 병렬 작업을 방 분리로 하라고
// 정했고(`scripts/lane.mjs`), 방에서 일하는 손은 커밋을 한다 — 그래서 **시킨 대로 일하는 한
// 이 감사는 초록이 될 수 없었다**(실측 2026-08-14: 방 12개 중 11개가 미병합 커밋을 들고 FAIL).
//
// 늘 빨간 신호는 신호가 아니다. 실제 피해: 인계 문서가 *"게이트는 지금 빨갛다"* 를 **평상 상태**로
// 적어 뒀고, 그 소음 속에서 **아무도 모르는 커밋 23개**가 하루 넘게 안 보였다.
//
// 그래서 무르게 하는 대신 **가른다**. 판정 축을 「커밋이 있나」에서 **「마지막 커밋 뒤 얼마나
// 지났나」**로 옮긴다 — 그게 원래 지키려던 「살아 있는데 **잊힌** 작업」이다.
//   · 방치된 방(오래 멎은 미병합 방) → **FAIL 유지.** 이게 이 자의 존재 이유다
//   · 작업 중인 방                    → 실패가 아니라 **경고**. **줄은 그대로 출력된다**
//   · 작업 방이 아닌 자리             → **FAIL 유지**(그물 안 넓힌다)
//
// ⚠️ **시계를 쓰는 것이 허용되는 유일한 이유**: 이 저장소 원칙은 *"시간은 학습의 근거가 아니다 —
// 시계는 lease·TTL 에만"* 이다. 「잊힘」은 정확히 **TTL** 이다(방이 살아 있다고 주장할 수 있는
// 기한). 다른 판정에 시계를 끌어들이지 않는다. 시각은 `git log -1 --format=%ct` 로 얻는다.
//
// **N = 24시간인 이유 — 임의값이 아니다. 두 사실이 같은 자리를 가리킨다:**
//  ① **밟은 분포에 실제로 골이 있다**(2026-08-14 실측, 미병합 방 11개의 마지막 커밋 나이):
//     `1h · 2h · 5h` 와 `30h · 30h · 31h · 31h · 33h · 33h · 34h · 42h`.
//     5시간과 30시간 사이는 **비어 있다** — 오늘 일하는 방과 어제 두고 온 방이 갈라진 자리다.
//     경계를 그 골 안에 둔다. 어느 쪽으로도 한 방을 반대로 부르지 않는다.
//  ② **하루는 이 저장소의 인계 주기다.** 세션이 바뀔 때마다 인계 문서를 다시 쓴다
//     (`GPAO-T5-CURRENT-SESSION-HANDOFF-ko.md`). 인계를 한 번 넘기고도 본선에 안 든 방은
//     **그 방을 알던 세션이 이미 없다** — 그게 「잊힘」의 기계적 정의다.
// 커밋이 하나라도 더 찍히면 시계는 다시 0 이므로, **진짜로 일하는 방은 절대 안 걸린다.**
const 방치_TTL초 = 24 * 60 * 60;

/** 그 머리의 마지막 커밋 시각(에폭 초). 못 읽으면 `null` — 모르면 안 봐준다(방치로 문다). */
function 마지막커밋(head) {
  try {
    const ct = Number(execFileSync('git', ['log', '-1', '--format=%ct', head], {
...
for (const { path, head } of worktrees) {
  ...
  if (작업방인가(path)) {
    ...
    if (남은 > 0) {
      // 「잊혔나」는 **노화**로 잰다(위 F-101 보탬 2). 커밋이 있다는 것만으로는 결함이 아니다.
      const ct = 마지막커밋(head);
      const 나이초 = ct === null ? Infinity : Math.max(0, Math.floor(Date.now() / 1000) - ct);
      const 나이 = ct === null ? '시각을 못 읽음' : `마지막 커밋 ${Math.floor(나이초 / 3600)}시간 전`;
      const 줄 = `${나이} · 본선에 안 들어간 커밋 ${남은}개): ${relative(root, path)}`;
      if (나이초 > 방치_TTL초) fail(`방치된 작업 방(${줄}`);
      // **경고도 반드시 남는다.** 이 줄이 오늘 미등재 커밋 23개를 찾아냈다 —
      // 「경고로 내린다」를 「조용히 없앤다」로 만들면 이 수리가 곧 C5 다.
      else warn(`작업 중인 방(${줄}`);
    }
    continue;
  }
...
// **경고는 통과·실패 어느 쪽에서도 먼저 나온다** — 실패가 없다고 사라지면 안 된다.
// stdout 으로 낸다: 실패는 stderr 라, 두 줄기를 따로 읽는 자(`gate.mjs` ⓪)가 섞지 않는다.
if (경고들.length) {
  console.log(`작업 중인 방 ${경고들.length}개 (실패 아님 — 아직 안 잊혔다. 본선 통합은 통합 담당 한 손이 한다):`);
  for (const message of 경고들) console.log(`  ⚠ ${message}`);
}
