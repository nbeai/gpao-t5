...
export const MUTATIONS = [
  ...
  { 이름: '답에 없는 위반 주장을 그대로 셈(허구 위반이 실패를 만듦)', 파일: GROW, 검사: T_GROW,
    찾기: "      if (!ev || !근거정규화(산출물).includes(ev)) { 관측.불가이유 = `forbidden_${i}_unevidenced`; return null; }\n      위반 += 1;",
    바꾸기: '      위반 += 1;' },
  { 이름: '무근거 위반 주장을 조용히 버림(null 이 아니라 통과로 흐름 — 감사 지적 ④ 재발)', 파일: GROW, 검사: T_GROW,
    ...
    바꾸기: "      // 표본이 아니다 — 통과·실패 어느 쪽으로도 위장하지 않는다.\n      if (!ev || !근거정규화(산출물).includes(ev)) continue;" },
  ...
  { 이름: '판정력 0(필수·금지 없음) 사례를 표본으로 받음', 파일: GROW, 검사: T_GROW,
    찾기: '      .filter((c) => c.expectedFacts.length + c.forbiddenFacts.length + c.exactFacts.length > 0);',
    바꾸기: '      ;' },
  ...
  { 이름: '판정 불가의 불가 이유가 기록되지 않음', 파일: GROW, 검사: T_GROW,
    찾기: "      if (!ev || !근거정규화(산출물).includes(ev)) { 관측.불가이유 = `required_${i}_unevidenced`; return null; } // 근거 없는 충족 주장 — 판정 불가",
    바꾸기: '      if (!ev || !근거정규화(산출물).includes(ev)) return null; // 근거 없는 충족 주장 — 판정 불가' },
];
