...
export const MUTATIONS = [
  ...
  { 이름: '기억 격리 해제 — 저장 원문이 벌거벗은 명령 목록으로 렌더됨(§5-J 재발)', 파일: PROVIDER, 검사: T_PROVIDER,
    ...
    // 2026-08-07: 노드 K 가 이 블록을 **지시/사실**로 갈랐다(판 ④ — 사실에 격리 딱지가 붙어
    // 모델이 실려 간 기억을 버렸다). 격리 자체는 그대로 서 있어야 하므로 **재는 것도 그대로다** —
    // 주입 자리만 새 구조의 지시 갈래로 옮긴다.
  ...
  { 이름: '그림을 교환에 안 붙임(모델이 화면을 못 본다)', 파일: TASKCTX2, 검사: T_CU_F2,
    // 2026-08-07: 객체 동일성만으로는 못 찾는 것이 실물에서 드러나(담은 `rec` 과 꺼내는
    // 영수증이 달랐다) 호출 신분으로도 걸었다. **재는 것은 그대로다** — 그림이 교환에 붙는가.
    찾기: "          return g ? { 그림: g } : {};",
    바꾸기: "          return {};" },
  ...
  { 이름: '드라이버가 "확인 못 한다"고 밝혀도 안 됐다고 단정', 파일: DESK_ACT, 검사: T_CU_D2,
    찾기: "      let 못본다 = 낸것?.effect === 'unverifiable';",
    바꾸기: "      let 못본다 = false;" },
  ...
  { 이름: '그림 크기를 봉투에서 안 읽음(모델이 짚을 자가 없다)', 파일: 'src/runtime/image-size.js', 검사: 'test/cu-node2-ladder-is-taught.test.js',
    ...
    바꾸기: "    return null;" },
  { 이름: '환경변수만 봄(사장님이 켠 T5 에는 화면 손이 0개다)', 파일: 'src/surface/live-context.js', 검사: 'test/cu-hand-must-reach-the-product.test.js',
    찾기: "  const cua백엔드 = 화면손찾기({ env: processEnv });",
    바꾸기: "  const cua백엔드 = processEnv.GPAO_T5_CUA_BIN;" },
  { 이름: '표준 자리를 안 훑음(설치해 둬도 못 찾는다)', 파일: 'src/runtime/desktop-bin.js', 검사: 'test/cu-hand-must-reach-the-product.test.js',
    찾기: "  for (const p of 표준자리(home)) {",
    바꾸기: "  for (const p of []) {" },
  ...
  { 이름: '브라우저 창인데 탭을 안 봄(CDP 길을 안 쓴다)', 파일: CUA, 검사: 'test/cu-node4-my-logged-in-browser.test.js',
    ...
    바꾸기: "          if (false) {" },
  { 이름: '한국어 동의 시트를 안 눌러 줌(드라이버는 영영 못 본다)', 파일: CUA, 검사: 'test/cu-node4-my-logged-in-browser.test.js',
    찾기: "                    await mcp.call('click', {",
    바꾸기: "                    await Promise.resolve({}).then(() => ({})); await (async () => ({}))({" },
  { 이름: '탭 목록이 모델까지 안 감(손이 들고만 있다)', 파일: TASKCTX, 검사: 'test/cu-node4-my-logged-in-browser.test.js',
    찾기: "    if (Array.isArray(result.탭들) && result.탭들.length) {",
    바꾸기: "    if (false) {" },
  { 이름: '손이 무엇을 보는지 안 밝힘(막혀도 옆 손을 못 고른다)', 파일: 'src/kernel/l2-plan/tool-descriptor.js', 검사: 'test/cu-node3-routing-between-hands.test.js',
    찾기: "    보는것: d.보는것,",
    바꾸기: "" },
  { 이름: '축을 selfState 가 안 나름(커널이 비교할 재료를 못 받는다)', 파일: 'src/kernel/l0-evidence/self-state.js', 검사: 'test/cu-node3-routing-between-hands.test.js',
    찾기: "      보는것: t.보는것,",
    바꾸기: "" },
  { 이름: '막혀도 옆 손을 안 가리킴(사용자에게 명령어를 준다)', 파일: 'src/kernel/turn.js', 검사: 'test/cu-node3-routing-between-hands.test.js',
    찾기: "  const 같은것보는손 = 옆손찾기(막힌손, 손들);",
    바꾸기: "  const 같은것보는손 = null;" },
  { 이름: '고르는 규칙을 모델에게 안 줌("어떻게 보나"가 없다)', 파일: 'src/kernel/screen-guidance.js', 검사: 'test/cu-node3-routing-between-hands.test.js',
    찾기: "**\"그걸 지금 어떻게 보나\"** 로 갈린다.",
    바꾸기: "" },
  { 이름: '화면 손이 비싸다고 안 말함(파일로 될 일에 화면을 쓴다)', 파일: DEMOCTX, 검사: 'test/cu-node3-routing-between-hands.test.js',
    찾기: "      + ' 여기뿐이다. 대신 **느리고 비싸다** — 파일·설정으로 되는 일은 그쪽이 낫다.',",
    바꾸기: "      + ' 여기뿐이다.'," },
  ...
  { 이름: '그림의 자를 안 줌(모델이 짚을 자가 없다)', 파일: TASKCTX, 검사: 'test/cu-node2-ladder-is-taught.test.js',
    찾기: "        ? ` · 네가 본 화면 ${result.그림크기.w}×${result.그림크기.h}(지금 보이는 만큼 · 짚을 자리는 이 안에서)` : '';",
    바꾸기: "        ? '' : '';" },
