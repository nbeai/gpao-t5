...
export const MUTATIONS = [
  ...
  { 이름: '못 물어본 판정을 판정 불가로 굳힘', 파일: GROW, 검사: T_GROW,
    찾기: '      verdict: 판정.ok ? 판정으로(c, 실행.text, 판정.text) : undefined,',
    바꾸기: '      verdict: 판정.ok ? 판정으로(c, 실행.text, 판정.text) : null,' },
  ...
  { 이름: '답에 없는 위반 주장을 그대로 셈(허구 위반이 실패를 만듦)', 파일: GROW, 검사: T_GROW,
    찾기: "      if (ev && 근거정규화(산출물).includes(ev)) { 위반 += 1; 사유.push(`금지 출현: ${String((c.forbiddenFacts ?? [])[i] ?? i).slice(0, 40)}`); }",
