...
export async function 회차돌리기({ n, 팔, 연결 }) {
  ...
  try {
    ...
    // 동결 §6: 사용자 개입은 **한 번뿐**이다. 카드가 뜨면 승인, 물으면 "응, 그렇게 해줘".
    // 그 뒤로는 개입 0 — 모델이 스스로 이어가는지가 이 실험이 재는 것이다.
    while (Date.now() - 시작 < 회차상한ms) {
      if (답?.kind === 'approval' && 사용자개입 === 0) {
        사용자개입 += 1;
        답 = await 부르기('/turn', { sessionId: s, approve: 답.pendingId });
        턴들.push(답);
        continue;
      }
      if (답?.kind !== 'approval' && 사용자개입 === 0 && 되묻나(답)) {
        사용자개입 += 1;
  ...
  const 발화횟수 = 도청.기록.filter((기) => 기.보낸것?.messages)
    .map((기) => JSON.stringify(기.보낸것.messages).split(문장).length - 1);
  ...
  const 회차 = {
    ...
    턴수: 턴들.length,
    마지막답: String(턴들.at(-1)?.reply ?? '').slice(0, 2000),
  };
  return 회차;
}

/** 모델이 사용자에게 되물었는가 — 개입 한 번을 쓸 자리인지 판정한다. */
function 되묻나(답) {
  const t = String(답?.reply ?? '');
  return /\?|까요|할까|드릴까|괜찮/.test(t);
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  for (let i = 0; i < Math.min(몇번만, 순서.length); i += 1) {
    ...
    로그(`  턴 ${r.턴수} · 사용자개입 ${r.사용자개입} · ${(r.걸린ms / 1000).toFixed(1)}초 · 토큰 입력${r.토큰.입력}/출력${r.토큰.출력}${r.타임아웃 ? ' · 타임아웃' : ''}`);
