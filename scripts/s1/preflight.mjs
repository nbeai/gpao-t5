...
export const 기준지문 = Object.freeze({
  ...
  // 2026-08-17 · **국면 5 슬라이스 1 일반 클릭**(88e4e889 → d6a89404 · 손 20 그대로 · 배치 그대로).
  // 움직인 것은 browser.observe·browser.act 두 스키마 설명뿐: click 정의역이 「탭·더보기만」에서
  // 「canOpen 의 링크·버튼·탭·더보기」로(경계: 폼 제출·바깥 이탈은 물러남 — browser.js 클릭가능술어
  // 한 벌이 수집·클릭 양쪽을 진다). 선빨강 실측(국면5-b-s1-선빨강-2회): 옛 문장이 모델의 손 선택
  // 자체를 죽이고 있었다(가능한 탭 클릭조차 0회 — 실측 원인 ③). ★ 시스템 프롬프트 지문은 한
  // 글자도 안 움직였다(0702451a 그대로 — 헌장·문안 무접촉). 검사: test/browser-general-click.test.js.
  // 2026-08-18 · **CH4 흐름 ① 막힘 행선지**(d6a89404 → eec33b15 · 손 20·배치 불변).
  ...
  // 2026-08-18 · **P0 터미널 승인 효과 계약**(3236d2ef → 1af56549 · 손 20·배치 불변).
  // 외부 효과를 오류 문구로 추측하지 않고 effects(network·write·signal) 집합으로 카드에 보인다.
  // 선언은 승인 전 권한이 아니며, 승인 뒤에도 그 집합만 연다. 시스템 프롬프트는 불변.
  스키마: '1af565490624b556',
  도구수: 20,
...
export const 허용파일 = [
  ...
  // P0 모델 왕복 회계 — 논리 respond마다 목적·상태·지연·토큰을 수치로 남긴다.
  // 원문·응답·도구 인자는 저장하지 않고 provider 내부 시도 횟수는 모르면 null이다.
  'src/runtime/model-call-accounting.js',
  // P0 background 왕복 누락 봉인 — T-cell 성장의 직접 modelFor 호출도 같은 회계 문을 지난다.
