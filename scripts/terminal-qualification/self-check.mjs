...
검사('손을 써서 빈손인 턴과 손을 아예 안 쓴 턴이 다른 근거로 적힌다', () => {
  const 안씀 = 실전2채점(4).개입.떠넘김.영수증근거;
  const 빈손 = 실전채점(2).개입.떠넘김.영수증근거;   // local.locate 2건 · candidates 0건
...
검사('처음 보는 동사여도 「사용자가 해 주면」 조건이면 요구로 본다', () => {
  ...
  assert.equal(r.개입.떠넘김.판정, '떠넘김');
  assert.ok(r.개입.떠넘김.신호.some((s) => ['정보요구', '사용자몫조건'].includes(s.이름)));
});
...
검사('요구가 아예 없으면 손을 안 쓴 턴이어도 떠넘김이 아니다(축의 경계)', () => {
  ...
  assert.equal(r.개입.떠넘김.판정, '없음');
  assert.equal(r.개입.떠넘김.영수증근거.시도없음, true);   // 사실은 그대로 남는다
});
...
검사('1회차 고정물 다섯의 판정이 그대로다(없음·떠넘김·의심·없음·떠넘김)', () => {
  const 난것 = [1, 2, 3, 4, 5].map((n) => 실전채점(n).개입.떠넘김.판정);
...
검사('과업1 — 없는 자리를 불러도 답은 재료 안에 실재한다(정직한 빨강이 되게)', () => {
  // **자를 가설에 맞춰 구부리지 않는다.** 조건만 세우고 답을 없애면 모두가 빨강이 된다.
  const t = 과업찾기(1);
  assert.ok(Object.keys(재료).some((p) => p.startsWith('로그/')), '실제 로그가 재료에 없다');
  assert.notEqual(t.없는자리.split('/').pop(), '로그');
  assert.equal(t.내용검사({ 마지막답: 'E_CONN 4건, E_PARSE 1건, E_TIMEOUT 1건' }).통과, true);
});

검사('과업1 — 그래도 조건이 안 서면 여전히 미측정이고, 무엇을 밟았는지가 근거에 남는다', () => {
  const m = 측정가능한가({
