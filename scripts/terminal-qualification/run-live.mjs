...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  for (const 과업 of 돌릴것) {
    ...
    if (!회차.측정.측정됨) 로그(`  ⊘ 미측정 — ${회차.측정.이유}`);
      ...
      const 떠넘김 = 회차.채점.개입.떠넘김;
      로그(`  멈춘칸 ${회차.채점.멈춘칸} · 개입 ${회차.채점.개입.합}(승인${회차.채점.개입.승인카드}·되묻기${회차.채점.개입.되묻기}·유도${회차.채점.개입.유도갈래}·떠넘김${떠넘김.판정})`);
      // **떠넘김은 근거를 화면에도 낸다.** 이 축은 한 번 실전에서 뚫렸다(2026-08-14 · 과업 2).
      // 숫자만 찍으면 다음에도 눈으로 못 본다 — 무엇을 보고 그렇게 적었는지를 같이 찍는다.
      if (떠넘김.판정 !== '없음') {
