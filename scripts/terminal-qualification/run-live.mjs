...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (process.argv.includes('--재료만')) {
    ...
    for (const t of 돌릴것) {
      ...
      // **옳은행동은 채점 전에 적혀 있다**(규율 B). 돌리기 전에 눈으로 본다.
      로그(`     옳은행동 ${t.옳은행동}`);
    }
    if (process.env.T5_LIVE_KEEP !== '1') await rm(방.뿌리, { recursive: true, force: true });
    로그('\n모델을 부르지 않았다.');
  ...
  for (const 과업 of 돌릴것) {
    ...
    if (!회차.측정.측정됨) 로그(`  ⊘ 미측정 — ${회차.측정.이유}`);
      ...
      const 떠넘김 = 참고.떠넘김;
      const 터 = 회차.채점.터미널;
      로그(`  멈춘칸 ${회차.채점.멈춘칸} · 승인카드 ${회차.채점.개입.승인카드}`);
      // **터미널이 얼마나 일했나** — 이 하네스가 재려던 것이 여기다.
      로그(`  터미널 호출 ${터.호출수} · 서로다른명령 ${터.서로다른명령} · 이어붙임 ${터.이어붙임}`
        + ` · 실패 ${터.실패}(뒤이어감 ${터.실패뒤이어감}) · 쓴프로그램 ${터.쓴프로그램.join(',') || '없음'}`
        + `${터.다른손.length ? ` · 다른손 ${터.다른손.join(',')}` : ''}`);
      로그(`  실물 ${회차.채점.실물.새로생김.length}건 · 원본불변 ${회차.채점.원본불변.지켜졌나 ? '지켜짐' : '깨짐'} · 자리고지 ${회차.채점.자리고지.알렸나 ? '했다' : '안했다'}`);
      로그(`  사다리 ${회차.채점.사다리.map((c) => `${c.이름}:${c.상태}`).join(' → ')}`);
      // **참고관측은 점수가 아니다.** 되묻기·유도·떠넘김은 말귀 손의 축이라 터미널 점수에서 뺐다.
      // 그래도 근거는 화면에 낸다 — 떠넘김 축은 한 번 실전에서 뚫렸고(2026-08-14 · 과업 2),
      // 숫자만 찍으면 다음에도 눈으로 못 본다.
      로그(`  [참고관측 · 점수 아님] 되묻기 ${참고.되묻기} · 유도 ${참고.유도갈래} · 떠넘김 ${떠넘김.판정}`);
      if (떠넘김.판정 !== '없음') {
        로그(`    떠넘김 ${떠넘김.판정} ← 그 턴(${떠넘김.영수증근거.본턴}) 결실 ${떠넘김.영수증근거.결실}·빈손 ${떠넘김.영수증근거.빈손}·막힘 ${떠넘김.영수증근거.막힘}`);
        for (const s of 떠넘김.신호) 로그(`      [${s.이름}] ${s.원문.slice(0, 70)}`);
      }
    }
    ...
    로그('');
    요약.push({
      과업: 과업.번호, 이름: 과업.이름, 능력: 과업.능력, 모양: 과업.모양,
      측정됨: 회차.측정.측정됨, 미측정이유: 회차.측정.이유,
      멈춘칸: 회차.채점?.멈춘칸 ?? null,
      승인카드: 회차.채점?.개입.승인카드 ?? null,
      터미널호출: 회차.채점?.터미널.호출수 ?? null,
      서로다른명령: 회차.채점?.터미널.서로다른명령 ?? null,
      이어붙임: 회차.채점?.터미널.이어붙임 ?? null,
      실패뒤이어감: 회차.채점?.터미널.실패뒤이어감 ?? null,
      // **참고관측** — 터미널 점수에서 뺀 값. 분모에도 안 들어간다.
      참고_되묻기: 회차.채점?.참고관측.되묻기 ?? null,
      참고_떠넘김: 회차.채점?.참고관측.떠넘김.판정 ?? null,
