...
- **P90-1 장기 작업상태 결정적 구조 편입 완료(2026-08-02, 겸임 구현 `f9a96d7`)**:
  ...
  PASS(CPU 26.4초·벽시계 17.3초)다. 공식 3도메인 실제 모델 반복과 P90-1 90점 판정은 아직
  미실행이며 이 수정 커밋을 동결한 뒤 재개한다. 증거:
...
다음 순서: 설치 선택과 별개로 사람 사용시험 P1 두 건(장기 대화 작업상태·자동화 사용자 흐름)을
제품 우선순위로 다룬다. 외부 공개·설치·배포는 아직 선언하지 않는다.
