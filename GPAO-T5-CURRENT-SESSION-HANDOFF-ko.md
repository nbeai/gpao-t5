...
## ★★ 지금 최신 상태는 여기다 — **먼저 읽어라** (2026-08-13)

- **`design/NEXT-SESSION.md`** — 아무것도 모르는 세션이 이 한 장으로 시작하는 인수인계서.
  무엇을 읽고 · 무엇을 기준으로 삼고 · 개발 규칙 · 완성의 정의 · 종료 판정은 누가 하나 ·
  에이전트 넷을 언제 어떻게 부르나 · 지금 해야 할 일 · 밟으면 안 되는 것이 전부 들어 있다.
- 이 문서(1,662줄)는 **역사와 배경**이다. 2026-08-11 에서 멈춰 있다.
  최신 사실이 필요하면 위 문서를 보고, 배경이 필요할 때 여기로 온다.
...
> (역할 경계: 개발은 구현팀, **종료 판정은 라이브** — 오너 선언 2026-08-14 로 사람 감사 폐지).
...
- **`audit:docs` 가 이 문서의 낡음을 놓쳤다(2026-08-01).** §0-A 가 H 단계를 말하는 동안
  ...
  **종료 판정은 라이브가 낸다**(오너 선언 2026-08-14 로 사람 감사 폐지). 구현 레인이 라이브 결과로 판정한다.
