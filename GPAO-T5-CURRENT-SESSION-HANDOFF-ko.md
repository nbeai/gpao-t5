...
- 비교 계측기 준비와 유료 회차 1은 종료됐다. 현재 상태는
  `TCELL_H_DIAG_1`이다 — **S0~S5 PASS · 서버 노출 경계 PASS · H0 완료 · H01~H07 1회 진단 완료 ·
  결함 계열 `P0` 2 · `P1` 2**(2026-08-01).
...
- **다음은 H 단계다.** 10개를 순서대로 훑는 장기 감사가 아니라 네 묶음으로 운영한다:
  H0 현재 진실 정리 → **T-cell H 봉인(H01~H07)** → **PC 손발 봉인(H08~H09)** →
...
- 현재 작업: T-cell S0~S5 구현 종료. **H 단계** — H01~H07 1회 진단 완료, 결함 계열 수정 중.
- 상태 토큰: `TCELL_H_DIAG_1` (§0-A·§9·§10 과 같은 값이어야 한다)
...
- **계열 ④ — 원인 확정, 구현 미착수**
  - 활성 경로: `openai` / `gpt-5.1` / `api.openai.com/v1` (비밀값 미열람)
  - 이 provider 는 **스트리밍 가능**하다(`model-provider.js` 에 `streamBody`·delta 파서 선언).
  - 25턴 전부 0이었던 이유는 게이트 하나다: `if (opts.onDelta && spec.streamBody && !opts.tools?.length)`.
    `modelSchemasFor()` 가 손이 있으면 **항상 통제 채널을 덧붙이므로** `tools.length > 0` 이
    상시 참이고 게이트가 매번 막는다. 빈 답 재시도만 도구 없이 나가 실제로 흐른다.
  - **함정(반드시 같이 처리)**: `OPENAI_WIRE.streamBody` 는 `opts` 를 받지 않는다.
    조건만 지우면 **스트리밍 요청에서 도구 스키마가 사라진다.** `opts` 전달과 `tool_call`
    조각 누적을 **함께** 완결해야 한다. 반쪽이면 "고른 줄 알았는데 실행 안 됨"이 된다.
  - 스트리밍 미지원 provider(`streamBody: undefined`)는 **가장하지 않고** 단발 경로를 유지한다.
    특정 provider 를 고쳤다고 전체 모델 계약을 고쳤다고 말하지 않는다.
- **H02 `proposal_short:boundary_sample` 미조사.** suite 문턱·표본 기준·`maxRounds` 완화 금지.
- 현재 수치: 회귀 **1520** · 돌연변이 스윕 **86** · `audit:plan`/`docs`/`workspace` PASS ·
  작업 트리 깨끗 · 잔여 프로세스 0.
...
상태 토큰: `TCELL_H_DIAG_1` (§0-A·§4·§10 과 같은 값이어야 한다)

### 다음 작업 — **이 순서대로**

1. **계열 ④ 완결** — `streamSse` 가 `tool_call` 조각을 누적하고 `opts` 를 받도록 완결한 뒤
   `!opts.tools?.length` 게이트를 걷는다. 반대시험은 일곱을 모두 건다: 도구 없는 일반 답 ·
   도구 사용 뒤 최종 답 · **첫 조각이 `complete` 보다 먼저** · 도구 실행 정확히 1회 ·
   미리보기와 지속된 최종 답 불일치·중복 0 · 미지원 provider 는 가장하지 않고 정상 최종 투영 ·
   provider 하나를 고치고 전체 계약을 고쳤다고 하지 않음.
2. **H02 `proposal_short:boundary_sample` 원인 종결** — 문턱·표본 기준·`maxRounds` 완화 금지.
3. **H01~H07 라이브 1회 재진단** — 결과는 H 진행표 §5-A 에만 기록한다.
4. **`P0` 0 과 네 결함 계열 소멸 확인.**
5. 그때에야 **동결된 6회 봉인**으로 간다.

각 계열은 **수정 전 반대시험 실패 → 구현 → 돌연변이 → 전체 회귀 → 실제 제품 경로 실측** 순서다.
...
```text
현재 작업: T-cell S0~S5 구현 종료. **H 단계** — H01~H07 1회 진단 완료, 결함 계열 수정 중.
현재 상태: `TCELL_H_DIAG_1` — S0~S5 PASS · 서버 노출 경계 PASS · H0 완료 · H01~H07 1회 진단 완료.
결함 계열 P0 2 · P1 2. ①② 는 `12948bd`, ③ 은 `3567d91` 로 자동검증 종결(라이브 재검증 전).
계열 ④(answer_delta 0)는 원인만 확정 — openai/gpt-5.1 은 스트리밍 가능한데 tools 가 항상
실려 게이트가 막는다. `streamBody` 가 opts 를 안 받는 함정을 함께 풀어야 한다. H02
`proposal_short` 미조사. 회귀 1520 · 스윕 86 · 게이트 PASS · 트리 깨끗 · 잔여 0.
다음 순서: 계열 ④ → H02 원인 → H01~H07 라이브 1회 재진단 → P0 0 확인 → 동결 6회 봉인.
```
