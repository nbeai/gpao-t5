...
8. OpenClaw·Hermes 라이브 비교는 준비 결함을 이유로 축소·포기하지 않고 정확하게 완주한다.
   ...
   구현·실행을 직접 인수한다. 이 전환과 기술 조정 부담을 오너에게 넘기지 않는다.
...
- 다음은 Claude의 **한 번짜리 고정 범위 감사**다. 네 항목과 기존 성립 회귀만 판정한다.
  새 발견은 통과 근거 허위, 안전 경계 붕괴, 핵심 성공 조건 무효화, 후속 판정 왜곡 중 하나일
  때만 차단으로 재개봉한다. 그 밖의 개선은 후속 관찰이다.
- 감사 통과 뒤에는 준비 문서를 다시 늘리지 않는다. 오너 자격 입력 한 번 → 유료 회차 1 →
  구조 유효성·실제 완료 신호·호출/토큰/비용 실측 순으로 곧바로 진행한다.
