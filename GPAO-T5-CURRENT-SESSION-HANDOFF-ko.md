...
- 비교 계측기 준비와 유료 회차 1은 종료됐다. 현재 상태는
  `TCELL_IMPL_S1_CODE_COMPLETE / AWAITING_LIVE_CREDENTIAL`이다.
...
- **S1 잔여 결함(감사 회부 · 경계 판정 요청)**: "취소해줘" 턴에 파일 되돌리기 승인 카드가 함께
  뜬다. 기억은 정상 철회되지만 사용자는 무관한 승인 카드를 본다. 원인은 기억이 아니라 도구
  추론 정규식(`src/kernel/l1-intent/intent.js:71` 의 `취소해` → local.file,
  `src/kernel/l1-intent/file-parse.js:14` 의 undo)이고, 그 자리는 계획 §5 가 이 슬라이스에
  금지한 구역(도구 선택 정책)이라 **손대지 않았다**. 경계 예외 여부는 감사·오너 판정 사항이다.
- **S1 라이브 대기**: H01·H04 실제 서버·`gpt-5.1` 3회는 자격 파일이 위생 정리로 사라져 미실행.
  오너가 `bash scripts/compare-live/enter-key.sh` 를 한 번 실행하면 즉시 진행한다.
...
- 현재 작업: T-cell S1(무마찰 가역 기억·한 턴 철회) 구현 완료와 라이브 확인
