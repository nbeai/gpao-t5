...
async function defaultClientFactory({ serverUrl, token, onUnauthorized }) {
  ...
  try {
    await client.connect(new StreamableHTTPClientTransport(new URL(serverUrl), {
      authProvider: { token, ...(onUnauthorized ? { onUnauthorized } : {}) },
    }));
  } catch (error) { await client.close().catch(() => {}); throw error; }
  return { listTools: (...args) => client.listTools(...args), callTool: (...args) => client.callTool(...args), close: () => client.close() };
}

export function makeRemoteMcpRuntime({ serverUrl, credential, onUnauthorized, clientFactory = defaultClientFactory } = {}) {
  ...
  async function client() {
    if (!clientPromise) clientPromise = clientFactory({
      serverUrl, token: async () => (await credential()).accessToken, onUnauthorized,
