  ...
  for (const entry of raw.packages) {
    ...
    for (const [version, release] of Object.entries(entry.versions)) {
      ...
      for (const [key, asset] of Object.entries(release.assets)) {
        if (!PLATFORM_KEYS.has(key) || !SHA256.test(asset?.sha256 ?? '')) throw new Error('invalid CLI asset');
...
async function defaultVerifyExecutable({ path, expectedVersion }) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => { child.kill(); finish(new Error('CLI version verification timed out')); }, 5000);
  });
}
...
export class ManagedCliStore {
  constructor({ root, catalog, platform = process.platform, architecture = process.arch, fetchImpl = fetch, verifyExecutable = defaultVerifyExecutable, maxBytes = DEFAULT_MAX_BYTES } = {}) {
    ...
    this.bin = join(root, 'bin'); this.versions = join(root, 'versions'); this.trash = join(root, 'trash');
  ...
  async ensure() {
    ...
    for (const path of [this.root, this.bin, this.versions, this.trash]) {
  ...
  binaryPath(id) { const entry = this.entry(id); return join(this.bin, executableName(entry.command, this.platform)); }
  ...
  async ensureVersion(id, version) {
    ...
    const bytes = await responseBytes(await this.fetch(asset.url, { redirect: 'follow' }), this.maxBytes);
    ...
    try {
      ...
      await this.verifyExecutable({ path: temporary, id, expectedVersion: version });
  ...
  async activate(id, version, type = 'installed') {
    ...
    const current = await this.status(id); if (current.activeVersion === version) return { state: 'already_installed', id, version, managedPath: active, sha256: asset.sha256 };
    ...
    return { state: type, id, version, previousVersion: current.activeVersion, managedPath: active, sha256: asset.sha256, downloaded };
  }
  ...
  async restore(id) { return this.serialize(async () => { await this.ensure(); if ((await this.status(id)).activeVersion) throw new Error('managed CLI is already installed');
    ...
    for (const name of names) { const source = join(this.trash, name); if (!(await regularFile(source))) continue; const version = await this.matchingVersion(id, source); if (!version) continue;
      ...
      return { state: 'restored', id, version, managedPath: this.binaryPath(id) }; }
...
export function makeCliAcquisitionTool({ store, authorizeEffect } = {}) {
  ...
  return {
    name: 'cli_prepare',
    description: 'Find and prepare a trusted T5-managed command only when the terminal lacks a suitable means. Candidates are pinned official single-file releases with exact platform and SHA-256 verification. Never use arbitrary URLs, package managers, sudo, global installs, or shell profile changes. Installation is a reversible local change; after preparation use the returned command through the terminal.',
