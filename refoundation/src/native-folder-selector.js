...
export function makeNativeFolderSelector({
  platform = process.platform, run = execute, windowsHelper = null,
} = {}) {
  if (platform !== 'darwin' && (platform !== 'win32' || !windowsHelper)) return null;
  return async function selectFolder() {
    const program = platform === 'darwin' ? '/usr/bin/osascript' : windowsHelper;
    const args = platform === 'darwin' ? ['-l', 'JavaScript', '-e', MACOS_SCRIPT] : [];
    ...
    if (!absolute || path.length > 4096 || /[\u0000-\u001f\u007f]/u.test(path)) {
