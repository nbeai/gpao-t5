...
export function makeScopedFileActivityService({ ledger, adapterFactory = null, onError = () => {} } = {}) {
  ...
  let generation = 0; let queue = Promise.resolve(); let degradedReason = null;
  ...
  async function status() {
    const state = await ledger.status(); const degraded = state.enabled && !runtimeRunning;
    return { available: Boolean(adapterFactory), ...state,
      enabled: state.enabled && runtimeRunning, desiredEnabled: state.enabled,
      degraded, degradedReason: degraded ? degradedReason ?? 'collector_not_running' : null,
      userSafeSummary: !state.configured ? '파일 활동 기록은 꺼져 있어요.'
        : state.enabled && runtimeRunning ? '허용한 폴더의 metadata 변화만 기록하고 있어요.'
          : degraded ? '파일 활동 기록을 다시 시작하지 못했어요. 기록 설정은 그대로 보존했어요.'
  ...
  async function startRuntime() {
    ...
    try {
      ...
      runtimeRunning = true; degradedReason = null;
    } catch (error) {
    ...
    watchTask = Promise.resolve().then(() => current.wait()).then(async (result) => {
        ...
        if (result?.state === 'stopped' && state.enabled) {
          queueMicrotask(() => { serialize(async () => {
            ...
            try { await startRuntime(); } catch (error) {
              degradedReason = error?.code ?? error?.message ?? 'collector_restart_failed';
              reportError(error);
            }
          }).catch(reportError); });
        } else {
          if (state.enabled) degradedReason = result?.reason ?? 'collector_failed';
      ...
      if (adapter === current && currentGeneration === generation) {
        ...
        if (state?.enabled) degradedReason = error?.code ?? error?.message ?? 'collector_failed';
      }
  ...
  return Object.freeze({
    ...
    async configure({ roots, recordedAt = new Date().toISOString() } = {}) {
      return serialize(async () => { await stopRuntime(); degradedReason = null;
        await ledger.configure({ roots, platform: process.platform, recordedAt }); return status(); });
    },
    async enable({ recordedAt = new Date().toISOString() } = {}) {
      return serialize(async () => {
        await ledger.setEnabled({ enabled: true, recordedAt });
        try { await startRuntime(); } catch (error) {
          await ledger.setEnabled({ enabled: false, recordedAt: new Date().toISOString() }); degradedReason = null; throw error;
        }
        return status();
      });
    },
    async pause({ recordedAt = new Date().toISOString() } = {}) {
      return serialize(async () => { await stopRuntime(); degradedReason = null; const state = await ledger.status();
