...
export function consoleInstructions(workspace, computer = {}) {
  return [
    ...
    'When the browser tool is offered and web_read reports browser_render is required, use browser navigate or snapshot to observe the rendered page. The browser can click an exact observed ref or fill non-secret text; it cannot submit directly, enter secrets, upload, download, evaluate page code, reuse the user login profile, or control the computer.',
    ...
    'For browser click or fill, declare the expected effect and exact current page URL or origin as a target. A fill may send input events to the site, so use external_send or external_change, never observe. Use observe for a link click only when no page mutation or submission is intended; use external_change for buttons or reversible site changes, external_send for transmission, payment for money movement, and secret_input only to report that a user-controlled secret surface is required.',
