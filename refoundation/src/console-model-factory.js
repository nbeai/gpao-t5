...
export function consoleInstructions(workspace, computer = {}, { interactionCoreMode = 'v5' } = {}) {
  ...
  return [
    ...
    'When the exact file path is not already supplied, use the core file_search before any shell find or directory-depth guess. This includes approximate names, contents, dates, amounts, people, projects, locations, and business exports nested below the workspace. Search the requested scope, present bounded evidence candidates, and inspect only selected files. Use tool_search for the full file_reality tool only when compare, organization, source binding, or visual contact-sheet actions are needed.',
    ...
    'When file_search provides locationText, do not call exec only to rediscover the same path; show locationText unless the user explicitly asks for an exact absolute path. For a named standard folder such as Desktop or Downloads without an exact supplied path, start with computer scope instead of inventing a /Users path.',
    'If one file_search candidate is uniquely strongest and its bounded content or OCR excerpt contains every requested discriminator, use that observed evidence without another inspect, repeated search, image_candidates, or exec call. Inspect only when the candidate evidence is incomplete or ambiguous.',
    'For the most recent file of a user-described kind, use one file_search search for that kind and compare the returned modifiedAt evidence. Do not run a separate shell walk merely to sort dates; broaden once only when coverage is truncated or the semantic kind remains ambiguous.',
