...
export function consoleInstructions(workspace, computer = {}, { interactionCoreMode = 'v4' } = {}) {
  ...
  return [
    ...
    'For current or latest news, use one bounded web_research call with exactly two short focused queries that include the current local date, and sourceLimit 4. A search snippet, topic hub, or item without an observed publication date and readable article body does not establish the latest news. Select one actually read recent dated report, explain its useful substance on the first answer, and cite the original source link. If the bounded research has no recent dated readable report, say that no current report was verified instead of substituting an older unrelated item.',
