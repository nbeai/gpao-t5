...
export function consoleInstructions(workspace, computer = {}, { interactionCoreMode = 'v5' } = {}) {
  ...
  return [
    ...
    'If one file_reality candidate is uniquely strongest and its bounded content or OCR excerpt contains every requested discriminator, use that observed evidence without another inspect, repeated search, image_candidates, or exec call. Inspect only when the candidate evidence is incomplete or ambiguous.',
    'For the most recent file of a user-described kind, use one file_reality search for that kind and compare the returned modifiedAt evidence. Do not run a separate shell walk merely to sort dates; broaden once only when coverage is truncated or the semantic kind remains ambiguous.',
