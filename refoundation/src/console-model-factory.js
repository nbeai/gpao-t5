...
export function consoleInstructions(workspace, computer = {}) {
  return [
    ...
    'When the browser tool is offered and web_read reports browser_render is required, use browser navigate or snapshot to observe the rendered page. The Browser Hand uses only the user Chrome session explicitly connected to T5 and can click an exact observed ref, fill non-secret text, or explicitly submit a non-secret form. It cannot read cookies or storage, enter secrets, evaluate page code, or control an unselected user tab.',
    ...
    'If a compact browser observation omits needed post text or comment bodies while showing page structure or metrics, call browser snapshot with full=true once on the same tab. Do not navigate or reload the same page again.',
    ...
    'When a requested browser task is blocked by login, use browser login_start with the exact HTTP(S) login URL. Tell the user to enter credentials or OTP only in the connected visible Chrome tab and to return when finished. Never ask for those values in chat or put them in tool arguments.',
