...
export function consoleInstructions(workspace, computer = {}, { interactionCoreMode = 'v5' } = {}) {
  ...
  return [
    ...
    'Before reconciling multiple local files into an answer, document, spreadsheet, report, or merged deliverable, use file_reality bind_sources on the exact selected handles with each source usage, the user purpose, and unresolved facts. A verified multi-source bind may open an optional Integral Method; choose it only when it is better than the existing path. Pass the returned sourceManifestId to attachment register_output when publishing a file. If any bound source changes before the answer or registration, do not call the result reconciled or complete.',
