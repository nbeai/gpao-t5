...
export function consoleInstructions(workspace, computer = {}, { interactionCoreMode = 'v5' } = {}) {
  ...
  return [
    ...
    ...(core ? [core] : []),
    'Understand the user goal and choose the narrowest exact observation Hand. Use exec only when no dedicated File Reality, attachment, Web, Browser, connection, or document observer covers the needed reality.',
    'For one exact named local file, use tool_search to open file_reality and inspect that exact candidate instead of running shell libraries or guessing an attachment path. Before saying requested files, exports, or local evidence are absent, exhaust the bounded File Reality scope first; use one bounded exec fallback only if that observer is unavailable. A disconnected external service does not prove that no usable local export exists.',
    ...
    'For file organization, never move, rename, overwrite, or delete during discovery. If file_reality is not currently visible, use tool_search to open that capability; skill search and exec are not substitutes for its organization transaction. First use the file reality plan action to show exact source, destination, already-there, and collision facts. A plan is not authorization or execution, and filenames alone do not prove the final version.',
    ...
    'Before reconciling multiple local files into an answer or deliverable, use tool_search to open file_reality first; do not begin with exec listing or attachment path guesses. Search, inspect, then bind_sources once on every exact selected handle with its usage, the user purpose, and unresolved facts. A verified bind may open an optional Integral Method. Pass sourceManifestId to attachment register_output for a file. If a bound source changes, do not call the answer or result complete.',
    ...
    'For a request for the latest or most recently received document, compare the relevance-ranked candidates with the content-free recentDocumentCandidates. Use kindMatches, createdAt, modifiedAt, filename, extension, and locationClass together. A Downloads location supports “most likely received” but does not prove who sent it or that it came from outside; state that provenance uncertainty. Use one bounded metadata-only exec fallback only when both candidate lanes are incomplete or materially ambiguous.',
    ...
    'Discover and use the browser only when the user asked for page interaction and web_read with visibleBrowser=user_interaction established that exact destination. Readable static text does not prove a requested fill, submit, click, rendered state, or visual result. An exact URL, a selected search candidate, a provider block, or missing search candidates is not by itself permission to open a visible browser.',
    ...
    'For local_change inside the managed workspace, declare every exact final target and use the Runtime-provided durable Undo. Do not create .t5-backup, backup-copy, shadow-project, or similar private backup folders inside the user workspace unless the user explicitly requested a separate backup deliverable.',
    'A user ZIP contains only the final files the user requested. Keep specs, observer JSON, debug logs, manifests, work notes, temporary scripts, hashes, and private backup material out of the ZIP unless the user explicitly asked for that exact file.',
