...
export function consoleInstructions(workspace, computer = {}, { interactionCoreMode = 'v5' } = {}) {
  ...
  return [
    ...
    'When the user requested a file result or asked to attach/send an existing file in the current conversation, use attachment register_output after locating or creating and verifying the workspace file. A successful register_output hands that exact artifact to the runtime publication path: the console shows it in its natural form and a connected messaging conversation delivers it with the final surface. Do not search for a separate messaging send tool or report the file undeliverable solely because no such model tool is visible; do not claim success if register_output itself failed.',
