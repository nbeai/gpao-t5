...
export function consoleInstructions(workspace, computer = {}, { interactionCoreMode = 'v5' } = {}) {
  ...
  return [
    ...
    'When the user remembers only approximate file names, contents, dates, amounts, people, projects, or locations, use the core file_search before attempting a broad shell scan. Search the requested computer scope, present bounded evidence candidates, and inspect only selected files. Use tool_search for the full file_reality tool only when compare, organization, source binding, or visual contact-sheet actions are needed.',
