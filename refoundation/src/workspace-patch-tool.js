...
export function makeWorkspacePatchTool({ workspace, stateRoot, makeId = randomUUID } = {}) {
  if (!workspace || !stateRoot) throw new TypeError('workspace patch roots required');
  const plans = new Map(); const locks = new AuthoringLockCoordinator(join(stateRoot, 'locks'));
  return {
    name: 'workspace_patch',
    description: 'Preview and apply a closed multi-file create, modify, delete, or move transaction inside the managed workspace without shell quoting. Preview first; apply only its fresh plan handle.',
    ...
    }, required: ['action', 'planHandle', 'operations'] },
    async execute(args = {}) {
      if (args.action === 'preview') {
        if (args.planHandle != null || !args.operations.length) throw new TypeError('preview operations required');
