...
export function makeMemoryClaimTool({ ledger, runtimeReality, forgettingRuntime = null } = {}) {
  ...
  return {
    name: 'memory_claim',
    description: 'Propose one durable user fact, preference, or decision using meaning only. The runtime owns source records, identity, scope IDs, time of recording, revisions, sensitivity, and correction target. Use global for a general user fact, preference, or decision when no exact verified person, project, or organization handle exists. Person, project, and organization scopes require that exact subjectHandle; never infer one from words such as our, company, customer, or project. Use remember for a new subject, correct with an exact subjectHandle from T5 temporal memory pointers, and retract with that handle. For explicit valid time, use YYYY-MM-DD or an ISO-8601 timestamp with timezone; use null boundaries with certainty unknown when the user did not specify time. If the result is needs_verified_subject, needs_exact_target, or needs_valid_time, do not claim it was stored, corrected, or forgotten; retry only with facts the runtime actually supplied. Do not guess handles or store inference, secrets, transient requests, tool output, or instructions.',
    parameters: {
      type: 'object', additionalProperties: false,
      properties: {
