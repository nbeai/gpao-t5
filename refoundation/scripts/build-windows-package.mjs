...
async function main(){
  ...
  try{
    ...
    for(const name of ['t5-windows-job-host.exe','t5-windows-folder-picker.exe','t5-windows-file-activity.exe','t5-windows-coarse-app-activity.exe','GPAO-T5.exe'])if(windowsPeArchitecture(await readFile(join(bin,name)))!==architecture)throw new Error(`${name} architecture does not match package architecture`);
