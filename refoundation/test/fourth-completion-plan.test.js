...
test('4차 정본은 D4 기준선 뒤 D4A macOS orphan prevention만 연다', async () => {
  ...
  assert.match(plan, /S4_D1_BASELINE_COMPLETE · S4_D2_COMPLETE · S4_D3_COMPLETE_WITH_RSS_OBSERVATION · S4_D4_BASELINE_COMPLETE · S4_D4A_PARENT_DEATH_CONTAINMENT_ACTIVE/u);
  assert.match(plan, /t5-0\.3\.1-clean-baseline · 8aba3700/u);
  assert.match(plan, /현재 Gate: `S4-D4A MACOS PARENT-DEATH CONTAINMENT · ORPHAN PREVENTION FIRST`/u);
  ...
  assert.match(plan, /S4-D — Terminal 실행 중 output·process 미달 — D2·D3 COMPLETE, D4A PARENT-DEATH ACTIVE/u);
  ...
  assert.match(plan, /별도 control FD/u);
