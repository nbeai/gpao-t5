...
test('4차 정본은 S4-F3 lock을 닫고 F4 publication rollback만 연다', async () => {
  ...
  assert.match(plan, /S4_F2_SCRATCH_PREPARE_REQUALIFIED · S4_F3_SORTED_LOCK_REVALIDATE_COMPLETE · S4_F4_PUBLICATION_ROLLBACK_ACTIVE/u);
  assert.match(plan, /t5-0\.3\.1-clean-baseline · 8aba3700/u);
  assert.match(plan, /현재 Gate: `S4-F4 TARGET PUBLICATION AND FAILURE ROLLBACK`/u);
