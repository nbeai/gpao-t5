...
test('4차 정본은 D5C 제품 격리를 닫고 S4-E read-only 기준선만 연다', async () => {
  ...
  assert.match(plan, /S4_D_TERMINAL_MANAGED_NON_PTY_COMPLETE · S4_D5A_IN_PROCESS_CANDIDATES_REJECTED · S4_D5C_PRODUCT_ISOLATION_COMPLETE · S4_E_READ_ONLY_BASELINE_ACTIVE/u);
  assert.match(plan, /t5-0\.3\.1-clean-baseline · 8aba3700/u);
  assert.match(plan, /현재 Gate: `S4-E MANAGED MUTATION CONFINEMENT · READ-ONLY BASELINE`/u);
