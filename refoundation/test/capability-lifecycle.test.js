...
test('제안→tested→recommended와 적용은 다른 Run으로 분리되고 archive·restore가 복구된다', async () => {
  ...
  try {
    ...
    const tool1 = makeCapabilityLifecycleTool({ ledger, runLedger: { read: async (id) => runs.get(id) }, stores, currentRunId: 'proposal-run', currentRunOrigin: 'user' });
    ...
    await assert.rejects(() => wake.execute({ action: 'apply', proposalId: proposed.proposalId, kind: null, id: null, lifecycleAction: null, baselineRunIds: [], candidateRunIds: [], rationale: null, unknowns: [], effect }), /user request Run/u);
...
test('store method 부재·사후 상태 불일치는 lifecycle 성공 사건을 만들지 않는다', async () => {
  ...
  try {
    ...
    const proposalTool = makeCapabilityLifecycleTool({ ledger, runLedger: { read: async (id) => runs.get(id) }, stores, currentRunId: 'p', currentRunOrigin: 'user' });
