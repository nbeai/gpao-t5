...
test('Google Workspace Drive는 사전등록 OAuth·offline consent·userinfo·protected tools 뒤에만 연결된다', async () => {
  const calls = []; const metadata = { issuer: 'https://accounts.google.com',
  ...
  const connection = makeGoogleWorkspaceDriveMcpConnection({ secretStore: memorySecretStore(),
    ...
    fetchImpl: async (url, init = {}) => {
      ...
      if (String(url).includes('oauth-protected-resource')) return new Response(JSON.stringify({
        resource: 'https://drivemcp.googleapis.com/mcp/v1', authorization_servers: ['https://accounts.google.com'] }));
      if (String(url).includes('oauth-authorization-server')) return new Response('', { status: 404 });
      if (String(url).includes('openid-configuration')) return new Response(JSON.stringify(metadata));
      if (String(url) === 'https://oauth2.googleapis.com/token') return new Response(JSON.stringify({ access_token: 'GOOGLE-ACCESS',
  ...
  assert.equal(inspected.stability, 'developer_preview');
  assert.match(inspected.userSafeSummary, /개발자 미리보기/u);
...
test('Google userinfo에 sub·email이 없으면 tools/list가 성공해도 ready가 아니다', async () => {
  const metadata = { issuer: 'https://accounts.google.com', authorization_endpoint: 'https://accounts.google.com/o/oauth2/v2/auth',
  ...
  const connection = makeGoogleWorkspaceDriveMcpConnection({ secretStore: memorySecretStore(), clientId: 'CLIENT',
    clientSecret: 'SECRET', callbackPort: 0, fetchImpl: async (url) => {
      ...
      if (String(url).includes('oauth-authorization-server')) return new Response('', { status: 404 });
      if (String(url).includes('openid-configuration')) return new Response(JSON.stringify(metadata));
      if (String(url) === 'https://oauth2.googleapis.com/token') return new Response(JSON.stringify({ access_token: 'ACCESS',
        scope: 'openid email profile https://www.googleapis.com/auth/drive.readonly' }));
