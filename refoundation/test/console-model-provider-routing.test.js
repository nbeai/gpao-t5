...
test('저장된 OpenAI·Claude·Gemini 연결은 각 공식 wire adapter로 선택된다', async () => {
  const room = await mkdtemp(join(tmpdir(), 't5-provider-routing-'));
  const file = join(room, 'model-connection.json');
  const keys = { openai: 'openai-secret', anthropic: 'anthropic-secret', gemini: 'gemini-secret' };
  const connections = [
  ...
  try {
    ...
    assert.deepEqual(answers, ['openai', 'claude', 'gemini']);
