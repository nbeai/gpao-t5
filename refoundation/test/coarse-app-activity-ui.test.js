...
test('CH2 UI는 기억·개인정보 설정에서만 title·URL 없는 app/AFK 통제를 보인다',async()=>{
  ...
  for(const value of ['내보내는 중…','다운로드를 시작했어요','내보내지 못했어요','현재 상태를 다시 확인했어요','현재 기록은 그대로 있어요'])
    assert.ok(memory.includes(value),value);
  assert.match(memory,/if \(!appState\.privateMode\) appActions\.appendChild\(toggleApp\)/u);
  assert.match(memory,/catch \{ fileActivityActionNotice = '[^']+'; \}[\s\S]*await SET_RENDER\.memory\(\)/u);
  assert.match(memory,/catch \{ appActivityActionNotice = '[^']+'; \} await SET_RENDER\.memory\(\)/u);
  assert.doesNotMatch(memory,/내보냈어요/u);
