...
test('실제 콘솔은 선택 원본을 결과 Artifact와 runtime provenance로 결속한다', async (t) => {
  ...
  const source = join(room, '7월매출.csv'); const source2 = join(room, '8월매출.csv'); const output = join(resultDir, '취합결과.csv');
  await writeFile(source, 'month,amount\n7월,1200000\n'); await writeFile(source2, 'period,sales\n8월,1350000\n');
  ...
  const server = makeConsoleServer({ stateDir: join(room, 'state'), workspace, computerFileRoots: [room],
    fileIndexSearch: async () => [source, source2], onError: (error) => errors.push(error?.stack ?? String(error)), modelFactory: () => ({ async respond(input) {
      call += 1;
      ...
      if (call === 3) { const found = JSON.parse(input.messages.findLast((item) => item.role === 'tool').content).result;
        const july = found.candidates.find((item) => item.displayName === '7월매출.csv');
        const august = found.candidates.find((item) => item.displayName === '8월매출.csv');
        return { text: '', toolCalls: [{ id: 'bind-source', name: 'file_reality', args: fileArgs({ action: 'bind_sources',
          sourceUses: [{ handle: july.handle, usage: '7월 확정 매출', columnMappings: [
            { sourceColumn: 'month', outputColumn: '월' }, { sourceColumn: 'amount', outputColumn: '매출' },
          ] }, { handle: august.handle, usage: '8월 확정 매출', columnMappings: [
            { sourceColumn: 'period', outputColumn: '월' }, { sourceColumn: 'sales', outputColumn: '매출' },
          ] }], purpose: '3분기 매출 취합', unknowns: ['9월 자료 미수신'],
      ...
      if (call === 5) { const registered = JSON.parse(input.messages.findLast((item) => item.role === 'tool').content).result;
        assert.equal(registered.sourceProvenance.state, 'verified'); assert.equal(registered.sourceProvenance.sources.length, 2);
        assert.equal(registered.sourceReconciliation.rowCount, 2);
        return { text: '', toolCalls: [{ id: 'complete', name: 'work_completion', args: { outcome: 'achieved', inputSettlements: [] } }] }; }
      return { text: '7월·8월 원본을 다시 확인해 취합결과를 준비했습니다. 9월 자료는 아직 반영되지 않았습니다.', toolCalls: [] };
    } }) });
  ...
  const response = await fetch(`${base}/turn`, { method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ sessionId: session.id, text: '7월매출.csv와 8월매출.csv를 근거로 결과/취합결과.csv를 등록하고 9월 미수신도 알려줘' }) });
  const result = await response.json(); assert.equal(response.status, 200, JSON.stringify({ result, errors }));
  assert.match(result.reply, /9월 자료는 아직 반영되지 않았습니다/u); assert.ok(manifestId);
  ...
  assert.match(artifact.humanReceipt.confirmed.join(' '), /원본 2개/u);
  assert.match(artifact.humanReceipt.confirmed.join(' '), /표준 열 2개.*전체 2행/u);
