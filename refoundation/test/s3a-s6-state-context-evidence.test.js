...
test('S6 evidence의 production source와 test-shadow artifact digest가 evidence exact commit과 일치한다', async () => {
  const value = JSON.parse(await readFile(evidencePath, 'utf8'));
  const commit = evidenceRevisionCommit('refoundation/evidence/s3-a-s6-state-context-shadow-2026-08-26.json');
  const sources = {
  ...
  for (const [name, path] of Object.entries(sources)) {
    const actual = digestAtCommit(commit, path);
