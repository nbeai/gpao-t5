...
test('Google Workspace OAuth app이 완전하면 공식 remote MCP가 우선하고 아니면 기존 Drive 경로를 유지한다', async () => {
  ...
  const bundled = JSON.parse(await readFile(new URL('../config/google-oauth.json', import.meta.url), 'utf8'));
  ...
  assert.match(source, /googleWorkspaceRemoteConnection[\s\S]*\?\? googleWorkspaceApiConnection \?\? googleDriveService/u);
  assert.match(source, /makeGoogleWorkspaceApiConnection/u);
  ...
  assert.equal(bundled.officialApiEnabled, true);
  assert.match(bundled.clientId, /\.apps\.googleusercontent\.com$/u);
});
