...
test('Alpha5·6와 최종 네 사용자 여정이 모두 닫힌 뒤에만 3차α source 완료다', () => {
  ...
  assert.match(plan, /S3_CA_CA1_COMPLETE_CA2_ACTIVE_LOCAL_POSITIVE_CONTROL/u);
...
test('최종 evidence는 실제 provider·외부 삭제·물리 플랫폼·수치 우위를 꾸미지 않는다', () => {
  ...
  assert.doesNotMatch(JSON.stringify({ alpha5, alpha6, repair, final }), /\/Users\/|C:\\Users\\|sk-[A-Za-z0-9]|-----BEGIN/u);
});
