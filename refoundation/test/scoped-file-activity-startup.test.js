...
test('앱 시작은 packaged helper가 있을 때만 CH1을 재개하고 종료 때 collector를 정산한다', async () => {
  ...
  assert.match(source,/makeMacOSFSEventsAdapter/u);assert.match(source,/resumeConfigured\(\)/u);
  ...
  assert.doesNotMatch(source,/makeWindowsUSNAdapter|windows_usn.*PASS/u);
});
