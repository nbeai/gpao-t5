...
function validatingFetch(calls) {
  return async (url, init = {}) => {
    ...
    const provider = String(url).includes('anthropic') ? 'anthropic'
      : String(url).includes('googleapis') ? 'gemini' : 'openai';
...
test('공식 API 키 세 종류는 검증 뒤 0600 저장되고 공개 목록에는 비밀이 없다', async () => {
  ...
  try {
    ...
    assert.deepEqual(listed.map((item) => item.provider), ['openai', 'anthropic', 'gemini']);
