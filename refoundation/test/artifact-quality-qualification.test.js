...
function passingObservations() {
  ...
  ].map((observation) => ({ ...observation, state: 'observed', observerRef: `fixture://${observation.observationId}` }));
}

test('QualityReceipt는 Semantic·Domain·Structural·Screen·Print를 독립 논리곱으로 자격한다', () => {
  const receipt = qualifyArtifactQuality({ contract: reconciliationContract(), observations: passingObservations() });
...
test('값과 화면이 맞아도 원문 고객 추적이 빠지고 인쇄 표가 가로 분할되면 완료가 아니다', () => {
  ...
  const receipt = qualifyArtifactQuality({ contract: reconciliationContract(), observations });
...
test('screen-only 결과도 다섯 lane을 보존하되 Print를 qualified로 꾸미지 않는다', () => {
  ...
  const receipt = qualifyArtifactQuality({ contract, observations });
  assert.equal(receipt.qualified, true);
