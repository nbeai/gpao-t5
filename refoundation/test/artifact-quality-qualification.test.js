...
function makeRuntimeQualifier(observations = passingObservations(), overrides = {}) {
  const byRequirement = new Map(observations.map((observation) => [observation.requirementId, observation]));
  return makeArtifactQualityQualifier({
    observationProducers: TRUSTED_PRODUCERS.map((producer) => ({
      ...producer,
      observe(context) {
        if (overrides[producer.kind]) return overrides[producer.kind](context);
        return structuredClone(byRequirement.get(context.requirement.requirementId));
      },
    })),
  });
}

async function qualify({ observations = passingObservations(), contract = reconciliationContract(), overrides } = {}) {
  return makeRuntimeQualifier(observations, overrides)({ contract });
}

test('QualityReceipt는 Semantic·Domain·Structural·Screen·Print를 독립 논리곱으로 자격한다', async () => {
...
test('값과 화면이 맞아도 원문 고객 추적이 빠지고 인쇄 표가 가로 분할되면 완료가 아니다', async () => {
  ...
  const receipt = await qualify({ observations });
...
test('화면 렌더는 인쇄 렌더나 OpenXML page setup을 대신하지 못한다', async () => {
...
test('screen-only 결과도 다섯 lane을 보존하되 Print를 qualified로 꾸미지 않는다', async () => {
  ...
  const receipt = await makeRuntimeQualifier(observations)({ contract });
  assert.equal(receipt.qualified, true);
  assert.equal(receipt.lanes.print.status, 'not_applicable');
  assert.equal(receipt.lanes.print.required, false);
});

test('requiredArtifactForms와 visualHierarchyGoals는 관측되지 않으면 완료가 아니다', async () => {
...
test('임의 facts와 self-declared producer는 lane을 통과하지 못한다', async () => {
...
test('trusted producer 문자열 복사나 call-time observation 주입은 실제 verifier를 대신하지 못한다', async () => {
  assert.throws(() => makeArtifactQualityQualifier({
    trustedProducers: TRUSTED_PRODUCERS,
  }), /identity strings cannot establish observation provenance/u);

  let observerCalled = false;
  const qualifier = makeArtifactQualityQualifier({
    observationProducers: TRUSTED_PRODUCERS.map((producer) => ({
      ...producer,
      observe() { observerCalled = true; return null; },
    })),
  });
  await assert.rejects(() => qualifier({
    contract: reconciliationContract(),
    observations: passingObservations().map((observation) => ({
      ...observation,
      producer: { kind: 'render_verifier', identity: 'qh2-render-runtime' },
    })),
  }), /runtime-owned/u);
  assert.equal(observerCalled, false);

  const unmeasured = await makeArtifactQualityQualifier()({ contract: reconciliationContract() });
  assert.equal(unmeasured.qualified, false);
  assert.equal(unmeasured.lanes.semantic.requirements[0].reason, 'producer_unavailable');
});

test('malformed observedUnitIds와 defect object는 throw 대신 해당 requirement를 failed로 닫는다', async () => {
  const observations = passingObservations();
...
test('page setup 구조와 schema·contract·artifact receipt 결속이 잘못되면 해당 requirement가 실패한다', async () => {
  const observations = passingObservations();
