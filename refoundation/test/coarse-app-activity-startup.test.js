...
test('앱 시작은 packaged CH2 helper만 재개하고 종료 때 collector를 정산한다',async()=>{
  ...
  assert.match(source,/T5_APP_ACTIVITY_HELPER/u);assert.match(source,/makeMacOSCoarseAppAdapter/u);
  assert.match(source,/appActivityService,/u);assert.match(source,/server\.closeAppActivity\(\)/u);
  assert.doesNotMatch(source,/makeWindowsCoarseAppAdapter|windows_foreground.*PASS/u);
});
