...
test('파일 지능 인간 콘솔은 합성 fixture·읽기 전용 모델·빈 messenger만 연다', () => {
  ...
  assert.match(source, /realUserFilesMayBeRead: realHomeBlind/u); assert.match(source, /externalWrites: realHomeBlind \? 1 : 0/u);
  ...
  assert.match(source, /protectedReadRoots: realHomeBlind \?.*Keychains.*stateDir.*: \[homedir\(\)\]/su);
  assert.match(source, /terminalEnvironment: \{ HOME: qualificationHome/u);
...
test('blind 여권사진 fixture는 정답을 보호 상태에만 두고 콘솔 출력에서 위치를 숨긴다', () => {
  ...
  assert.match(source, /computerFileRoots: realHomeBlind \? \[homedir\(\)\] : blindPassport \? \[syntheticHome\]/u);
...
test('real-home blind fixture는 무작위 전용 폴더 하나만 만들고 종료 시 정확히 정리한다', () => {
  ...
  assert.match(source, /computerFileRoots: realHomeBlind \? \[homedir\(\)\]/u);
