...
test('제품 진입점은 Channel Talk를 platform secret store와 설정 연결 목록에 실제 등록한다', async () => {
  ...
  assert.match(source, /const workspaceConnectionServices = \[googleWorkspaceRemoteConnection[\s\S]*\?\? googleWorkspaceApiConnection \?\? googleDriveService,[\s\S]*channelTalkConnection\]/u);
