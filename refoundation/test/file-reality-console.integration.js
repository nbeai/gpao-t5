...
test('실제 콘솔은 모호한 단서로 컴퓨터 후보를 찾고 선택한 파일만 다시 연다', async (t) => {
  ...
  const workspace = join(room, 'workspace'); const archive = join(room, 'archive');
  await Promise.all([mkdir(workspace), mkdir(archive)]);
    ...
    modelFactory: () => ({ async respond(input) {
      ...
      if (turn === 3) {
        ...
        return { text: '', toolCalls: [{ id: 'inspect-file', name: 'file_reality', args: {
          ...
          handles: [observed.candidates[0].handle], maxCandidates: null,
        } }] };
      }
      if (turn === 4) {
