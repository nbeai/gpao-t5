...
async function fixture({ existing = true, sourceProvenance = null, selectedVisual = false, selectedFile = false,
  ...
  await work.recordResultReady({ runId: writer.runId, sessionId: SESSION,
    ...
    surfaceResult: { kind: 'reply', artifacts: [{ attachmentId: registeredArtifact.attachmentId }] } });
...
test('G protected exec batch가 직접 등록한 Artifact는 generated output 영수증으로 materialize한다', async () => {
  const f = await fixture({ producedBatch: true });
