...
test('5차 HQ는 실제 Console 여덟 여정·최초 실패 수리·전체 회귀를 함께 닫는다', () => {
  assert.equal(evidence.status, 'PASS_MACOS_PRODUCT_SCOPE_WITH_EXTERNAL_BOUNDARY_OBSERVATION');
  assert.deepEqual(evidence.journeys.map((journey) => journey.id), [
    'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'H7', 'H8',
  ]);
  assert.equal(evidence.journeys.find((journey) => journey.id === 'H2')?.status,
    'PARTIAL_EXTERNAL_BOUNDARY');
  assert.ok(evidence.journeys.filter((journey) => journey.id !== 'H2')
    .every((journey) => journey.status.startsWith('PASS')));
  assert.equal(evidence.firstFailuresAndRepairs.length, 7);
