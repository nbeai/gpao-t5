...
기존 Attachment ledger의 durable batch contract는 verified output 2개 전체의 target·expected hash·bytes·preimage를
prepare하고, postimage 전량 일치 때 한 append 사건으로 outputHandle 2개를 commit한다. successor restart에서도 같은
handle을 재사용한다. target 일부만 postimage면 handle 0의 `partial_effect_unknown`, 전부 preimage면 `not_published`다.
commit된 handle은 filePath와 path authorization 없이 기존 `attachment.register_output` 첫 시도에 Artifact 2개로
...
receipt, handle batch commit, Artifact batch 등록, snapshot cleanup 순서를 결속했다. output마다 F readback과 같은
handle은 ledger에만 남고 모델에는 verified Artifact만 반환된다. F 뒤
...
Console product integration은 model identity나 업무 이름을 보지 않고 exact program bytes·resolved Python·workspace cwd·
`local_change` target 사실로 기존 exec 아래 Snapshot backend를 선택한다. literal heredoc과 `python -c`를 쓰는 서로 다른
두 fixture model identity가 모두 Python 1회→F→batch→Artifact 2개→Work completion으로 끝났고 모델의 attachment
호출은 0이었다. verified output은 Runtime이 한 batch 사건으로 Artifact에 등록하며 Run에도 `output_produced`를 남긴다.
같은 Run의 attachment 손은 등록 완료 사실로 비활성화한다. macOS 제품 payload에서 Snapshot·Python·contract source를
더 이상 qualification-only로 제거하지 않는다. actual gpt-5.5 경제성 A/B 전까지 product activation 완료는 주장하지 않는다.

첫 actual product run은 G activation 1·정확성·원본·Artifact를 통과하고 attachment 실패를 4→0으로 줄였으며
121.860초·13 model calls·12 tool calls·324,141 tokens였다. 그러나 내부 outputHandle이 모델에 보이면서 이미 등록된
Artifact를 attachment로 두 번 다시 확인했다. handle을 ledger에만 남기고 attachment를 비활성화한 두 번째 actual에서
같은 gpt-5.5는 이번에는 복합 shell 형태를 사용했다. exact heredoc은 관측됐지만 command step이 하나가 아니어서 G
activation은 0이었고 148.284초·15·15·381,147 tokens였다. 결과 정확성·원본·잔여물 0은 유지됐지만 경제성 기준은
실패했다. 같은 activation 가족에 `mkdir`·`cd` 예외를 더하지 않는다.

따라서 현재 좁은 Python adapter는 모델 독립 제품 activation으로 완료되지 않았다. 다음 구조 후보는 모델이 요청한
exact shell command 전체를 writable immutable-source snapshot에서 실행하고, workspace 밖 write·network를 물리 차단한
...
S4-L read-only baseline은 과거 exact head의 Windows Server x64 runner에서 PowerShell·Job Object·ConPTY·DPAPI가
실행됐고 ARM64는 cross-build만 됐음을 확인했다. current head GitHub rerun, 물리 x64·ARM64, Windows Search,
NTFS 경계, 두 실제 모델, 사람 UI, signed installer journey는 모두 미실행이다. 기존 config의
...
오너 결정으로 4차 qualification 기본 모델은 gpt-5.5, Terra는 comparison observation으로 고정했다. S4-P 뒤
실제 설치 제품과 같은 Console UI를 Browser에서 열고 A~J wave를 실행했다. runner는 fixture 준비·원장 회수에만
사용했고 사용자 입력·진행·교정·Preview·Undo·재접속은 실제 Console에서 수행했다.

actual wave 판정은 다음과 같다.

| 여정 | 판정 | 실제 결과 |
|---|---|---|
| A 직접 대화 | PASS | 1 model call·tool 0, 자연스러운 답과 Enter/Shift+Enter 계약을 확인했다. |
| B 모호한 파일 | PARTIAL | 깊은 문서는 정확히 찾았지만 current-head 이미지 표본은 C2를 맞히고도 Artifact를 발행하지 않았다. |
| C 총무 산출물 | FAIL | XLSX·DOCX·ZIP 복합 경로가 protected program 내부 실패를 반복해 결과를 전달하지 못했다. |
| D 프로그램 구성 | PARTIAL | Python exact once·immutable source·독립 검산·Artifact 2개·cleanup은 통과했지만 요청한 `결과` 대신 `results`를 사용했다. |
| E 신규 프로젝트 | PASS_WITH_PERFORMANCE_REGRESSION | local site·managed server·Browser form·network 0·server stop·HTML Artifact는 통과했지만 246.6초·28 model calls·29 tools·1,031,895 tokens였다. |
| F 기존 프로젝트 | PARTIAL | dirty CSS 보존·source fix·test·Browser step 2·다음 입력 Undo는 통과했지만 첫 턴은 불필요한 screenshot 재검증 뒤 failure surface로 끝났다. |
| G 장기 교정 | PARTIAL | busy correction admission·최신 교정·재접속은 통과했지만 output publication과 중복 상태가 실패했다. |
| H 결과 UX | FAIL | 성공 뒤 superseded 실패·Artifact·Effect receipt가 함께 보이고 일부 확인 경고가 남았다. |
| I 능력 경계 | FAIL | tunnel CLI 부재·external effect 0은 맞았지만 내부 Attachment Preview를 external Preview처럼 말하고 불필요한 executable ZIP을 만들었다. |
| J 채널 연속성 | PASS | 기존 격리 Console↔Telegram·파일 delivery·restart·unknown ACK·exact-once 13/13을 current head에서 재확인했다. |

HQ 중 처음 launcher가 bounded `computerFileRoots`를 주지 않아 실제 사용자 Desktop 후보와 이미지 pixel이 provider에
전송된 isolation P0도 있었다. 실제 사용자 파일 write와 외부 effect는 0이었고 즉시 Console을 중단한 뒤 launcher를
synthetic workspace/home 두 root로 제한했다. 이 사고를 숨기지 않으며 해당 실행은 qualification에서 무효다.

actual wave에서 채택한 product 수리는 bounded missing output parent의 create·publication·Undo, local-change
confirmation 표준화, pre-publication durable output batch, selected visual·protected program Artifact receipt,
single Work status와 superseded mutation warning 정리다. 실제 사용자 의미·업무 Router·모델별 Prompt·새 Store는 0이다.
전체 검사는 unit 1,844/1,844(1 skip), product integration 199/199(2 Windows skip), mutation 2/2와 J 13/13을
통과했다. 그러나 위 C·B·F·H·I와 성능 미달 때문에 S4-HQ와 4차 전체 완료는 아직 주장할 수 없다.
